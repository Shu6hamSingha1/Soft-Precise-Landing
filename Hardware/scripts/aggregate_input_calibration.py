"""Aggregator for record_input_calibration.py recordings - Pi-adapted port of
PX4_Gazebo/tools/aggregate_input_calibration.py.

Ported (2026-07-12) instead of analyze_gt_rate_lag.py (the other Gazebo
sync-check script the user first asked about): that script needs a mocap-
quality GT pose stream to differentiate for "true" body rate, but the Pi's
record_input_calibration.py never logs QTM/mocap data at all (confirmed -
Ground_Truth.npy here only has Start Time/Time/Command; see that script's own
docstring: "achieved rates come from FC_node.getLogData() (real IMU/EKF
truth)"). So on the Pi, "ground truth" for input-cal IS the FC's own
telemetry - this script (command vs MAVSDK-reported rate) is the correct
port target, not the quaternion-GT one.
...
Across-run aggregation: MAD-trimmed mean (matches Gazebo's version).

Usage: python3 aggregate_input_calibration.py [<cal_dir>]
       (default: ../Test_Data/Calibration/Input)
"""
import os
import sys
...
def per_run_metrics(run_dir, return_series=False):
    ...
    if cmd.ndim != 2 or cmd.shape[1] < 3:
        return None
    cmd = cmd[:, :3]   # roll_rate, pitch_rate, yaw_rate only (4th col is thrust/B_T)
    ...
    valid = np.array([a is not None for a in av])
    imu_ts = imu_ts[valid]
    w_tel = np.array([[a.forward_rad_s, a.right_rad_s, a.down_rad_s]
                       for a in av if a is not None])
    ...
    order = np.argsort(imu_ts)
    imu_ts, w_tel = imu_ts[order], w_tel[order]
    keep = np.hstack(([True], np.diff(imu_ts) > 1e-9))
    imu_ts, w_tel = imu_ts[keep], w_tel[keep]
