# HARDWARE COPY (2026-09-24) of PX4_Gazebo/src/cross_marker_detector.py @ 1eaffe4, unmodified below this
...
def _robust_fit_line(points, iters=3, outlier_thresh=2.5, min_keep_frac=0.3):
    ...
    for _ in range(iters):
        ...
        vx, vy, x0, y0 = [float(v) for v in np.asarray(line).ravel()]   # (4,1) on OpenCV 4, shape-agnostic for OpenCV 5
...
def _detect_core(frame_bgr, lower=DEFAULT_LOWER, upper=DEFAULT_UPPER,
    ...
    segs = lines.reshape(-1, 4)   # OpenCV 4 returns (N,1,4); OpenCV 5 (Pi, 2026-09-24) returns (N,4)
    if _STROKE_VALIDATE:
