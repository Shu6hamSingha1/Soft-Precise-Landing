...
class PlanarFeatureMap:
    ...
    def __init__(self, max_features=300, min_tracked=8, refill_below=60,
        ...
        # (confirmed live: confidence pinned at 0.000 for 14+s straight while roi_hits
        # stayed 30/30). No existing mechanism recovers from this -- img_data.py's
        ...
        # TIME-BASED, not frame-count (2026-07-25 correction): this module's caller runs
        # at very different effective rates on Pi hardware (measured ~15-35Hz, swinging
        # with scene/tracking health) vs Gazebo SITL, so a fixed frame-count threshold is
        # a poor proxy for elapsed wall-clock time and risks not firing before some OTHER
        # caller-side mechanism (e.g. hardware_landing.py's MARKER_LOSS_GRACE, default
        # 1.0s) already gives up. degenerate_reset_seconds is kept comfortably under that
        # grace window. _degenerate_streak (frame count) is kept ONLY as an informational
        # log field alongside the elapsed-seconds value actually gating the reset.
        ...
        # True iff the PRIMARY slot dropped all 4 corners THIS frame (ported from Gazebo
        # 2a5c21f -- lets img_data.py's rescue gate use map_confidence instead of the
        # marker-dependent confidence specifically for this frame, since map_confidence
        # doesn't zero out just because ArUco decode failed).
        self.primary_zero_corners = False
        # DECODE-STALENESS decay (2026-07-26, user): frames_since_decode already existed
        # (incremented every update(), reset to 0 by loop_closure_correct) but was only
        # ever read by should_trigger_decode's advisory scheduling - map_confidence itself
        # had NO term that decayed with how long the map has been running on pure KLT
        # prediction without a real ArUco re-anchor. Without this, a map that hasn't seen
        # a real decode in hundreds of frames can still report high map_confidence purely
        # from locally-smooth KLT tracking (drift can be smooth, not just noisy) - exactly
        # the case the RESCUE consumer (which reads map_confidence to decide whether to
        # trust a predicted corner position) most needs to be warned about. Decays
        # linearly to 0 at decode_stale_max_frames frames since the last correction,
        # recovers to 1.0 the instant loop_closure_correct runs again (frames_since_decode
        # reset to 0) - same linear-decay convention as rigid_fail_streak/rigid_persist
        # above. Only applied to map_confidence (not confidence), matching this module's
        # own established distinction: confidence answers "is the marker's own tracked
        # geometry trustworthy right now" (live corner-shape-based, not decode-recency-
        # based); map_confidence answers "can the map still infer position at all" - the
        # metric this staleness signal is actually about.
        # TIME-BASED (2026-07-26 correction, same cross-platform-rate lesson as
    ...
    def update(self, gray, quat_R=None):
        ...
        slot_survival_conf = max(0.0, 1.0 - _streak / max(self.slot_inlier_fail_streak_max, 1))
        # DECODE-STALENESS term (2026-07-26, see decode_stale_max_frames comment in
        # __init__): decays linearly to 0 as frames_since_decode grows, so pure-KLT
        # drift with no real re-anchor eventually can't masquerade as high confidence
        # just because local tracking still looks smooth. Recovers to 1.0 the instant
        # loop_closure_correct resets frames_since_decode to 0.
