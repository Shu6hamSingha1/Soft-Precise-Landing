...
| 17 | **Yaw loop sign-inverted under `HW_POS_FEEDBACK`: heading diverges** (found 2026-09-24) | `e_a(t)` grows in magnitude from its initial offset through the flight; `u_a` has the SAME sign as `e_a`; `s[3]` equals the telemetry NED yaw exactly | **Open, not fixed.** 31/33 flights diverged (e.g. −33°→−75°, +64°→+89°), with measured w_z following the command. Root cause: `hw_pos_feedback.py` takes `ry` from the MAVSDK FRD→NED quaternion (NED yaw), but the port kept PX4 `gt_feedback.py`'s default `PLASMC_GT_ALPHA_SIGN=+1`. PX4's `_yaw_of` returns ENU yaw, so there alpha=+ψ_ENU=−ψ_NED; on hardware alpha=+ψ_NED, the opposite sign. `w[2]`'s sign already matches PX4 (−ψ̇_NED), so only alpha is wrong. Do NOT fix with `PLASMC_GT_ALPHA_SIGN=-1`: that also flips `w[2]`. Negate `ry` (or alpha only) in `hw_pos_feedback.py` |
| 18 | **`PLASMC_AU_MAX_Z` caps BRAKING, not descent** (found 2026-09-24) | `a_u(t)[:,2]` pinned at exactly −0.200 while `sigma_z<0` (descending faster than desired) | **Open, not fixed.** The 2026-08-03 cap `if a_u[2] < -au_max_z: a_u[2] = -au_max_z` (default 0.2) claims to bound DOWNWARD accel. In NED/V coordinates negative a_u_z is UP (`I_a = R a_u − g e3`; logs: a_u_z=+0.54 gives I_a_z=−9.27), so it limits upward braking to 0.2 m/s² and leaves downward accel unlimited. It was pinned in 33/33 flights, for 49% of each descent on average, usually from 2.4-2.8 m down. With any hover shortfall (0.01 throttle ≈ 0.27 m/s² downward bias) the controller cannot arrest the descent. This, not the adaptive gains, is why the vertical loop "doesn't brake" (cf. the kappa_z non-adaptation thread). It also drives the vertical funnel breach (`|σ_z|/p_z` 1.2-2.1 in 25/33 flights). Hardware-only; PX4 never had it |
| 19 | **Translational integral (`izeta`) frozen for the whole flight under `HW_POS_FEEDBACK`** (found 2026-09-24) | `izeta(t)` identically 0 in every flight | **Open, not fixed.** The 2026-07-31 conditional-integration freeze gates on the CAMERA's `FEATURE_PTS_FRESH` (controller.py ~2168; the same gate on `is_e_n` ~1851). In HW_POS mode the measurement is the always-fresh EKF, and the camera usually doesn't see the marker, so the Ω·∫ζ term never acts. Same class as the 08-19 `CBF_CORNERS_STALE` bypass, which already treats corners as never stale when `self._gt_feedback is not None`. Latent sibling: the yaw terminal-hold gates on the camera's `FEATURE_IS_STALE` (~2486) without a bypass. It didn't fire this session (w_u_z was not zeroed) |
