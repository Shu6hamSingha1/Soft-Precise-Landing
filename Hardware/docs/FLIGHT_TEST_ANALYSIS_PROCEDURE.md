...
| 14 | **Battery-sag thrust deficit: the voltage hover table stops at 0.395 but true hover keeps rising** (2026-09-24) | Estimate the true hover throttle per run as `median(thr_cmd + a_z_down/26.56)` over the controlled descent (thr from `Local_Logs['control_output'][:,3]`, a_z from the odometry velocity gradient, tilt<8°, Z>0.6 m). Compare it with the `Battery voltage: ... -> hover throttle` console line | **Fixed 2026-09-24, DEPLOYED to the Pi 2026-09-25 (sha256-verified), not yet flown.** `hardware_landing.py` now interpolates a measured table (`_HOVER_TABLE_V/_T`, binned medians of today's estimates). It warns below `HW_HOVER_WARN_VOLTAGE`=22.0 V and clamps below 20.67 V. In-sample error vs the per-flight estimates: mean −0.003 (old table −0.035, worst −0.124). Over 33 flights, corr(V, true hover) = −0.87. True hover ≈0.39 at 23.0 V, 0.405 at 22.0, 0.43 at 21.5, 0.47 at 21.0, ≈0.50 at 20.7, while `_voltage_corrected_hover_throttle` caps at 0.395 below 22.19 V. Descent speed at 0.5–1 m ≈ 0.82 + 13.7·deficit (R²=0.61), so 0.2–0.5 m/s on fresh packs and 1.5–2.5 m/s below 21.3 V. All 3 packs were flown down to 20.6–20.9 V; PX4 raised `EMERGENCY: Low battery level` 16×. **Judge no config flown below ~22.0 V.** |
| 15 | **GT-depth touchdown latches above ground on a momentary vertical stall** (2026-09-24) | `TOUCHDOWN-DETECT (analytic-feedback): depth=0.5–0.7m`, well above the ~0 of real contacts | **Fixed 2026-09-24, DEPLOYED to the Pi 2026-09-25 (sha256-verified), not yet flown.** `controller.py` GT path now also requires depth ≤ `PLASMC_TD_GT_DEPTH_MAX` (0.25 m), held continuously for `PLASMC_TD_GT_HOLD_S` (0.2 s). Unit-tested on both logged false-latch traces (no latch, even +1 s hover), a synthetic 3 s stall at 0.6 m (no latch), a genuine contact (latch +0.31 s) and a 0.15 m bounce (latches after settling). Runs `14-57-58` (d=0.591) and `15-18-18` (d=0.634) (data-folder names). The default window `PLASMC_TD_FRAMES=3` / `PLASMC_TD_WINDOW=3` is ~53 ms at 56 Hz, so any zero-crossing of v_z that has "progressed ≥0.3 m" passes. The handoff to PX4 LAND was safe. Candidate fix: widen to ~0.3–0.5 s (`PLASMC_TD_FRAMES`/`PLASMC_TD_WINDOW`, ≈17–28 frames), or also require depth below a floor |
| 16 | **Near-ground lateral divergence under `HW_POS_FEEDBACK`** (2026-09-24) | `s_e_n` oscillation (period ~3–4 s) grows as Z falls; below ~1 m `a_u_xy` reaches 3–7 m/s², tilt 20–34°, lift is lost, and the vehicle hits the ground at 2–3 m/s | **Open, cause not isolated.** Seen at good voltage too (folders `14-55-42`, `15-17-19` with 5.5 m drift, `15-47-27`, `15-48-14`). Same family as the 08-26 lateral kappa runaway. Hypotheses: 1/Z loop-gain growth plus GPS/EKF position error entering `s=(p−p_marker)/Z` (marker snapshotted from the pre-arm GPS pose). Pull the `.ulg` (`vehicle_local_position`, estimator source) before tuning gains |
| 17 | **Yaw loop sign-inverted under `HW_POS_FEEDBACK`: heading diverges** (found 2026-09-24) | `e_a(t)` grows in magnitude from its initial offset through the flight; `u_a` has the SAME sign as `e_a`; `s[3]` equals the telemetry NED yaw exactly | **Fixed 2026-09-24, DEPLOYED to the Pi 2026-09-25 (sha256-verified), not yet flown.** `hw_pos_feedback.py`: alpha = wrap(−_asign·ry) (ENU convention; `w[2]` untouched). `ry` is now relative to a target heading snapshotted with the marker position before arming (`PLASMC_HW_TARGET_YAW_SNAPSHOT=1`; =0 means north), so the loop holds the launch heading instead of slewing to north. Offline: vehicle 52°/target 20° gives alpha −32°. Closed-loop kinematic sim: old code runs to ±180° from −33°/+64°, new code converges to the target. 31/33 flights diverged (e.g. −33°→−75°, +64°→+89°), with measured w_z following the command. Root cause: `hw_pos_feedback.py` takes `ry` from the MAVSDK FRD→NED quaternion (NED yaw), but the port kept PX4 `gt_feedback.py`'s default `PLASMC_GT_ALPHA_SIGN=+1`. PX4's `_yaw_of` returns ENU yaw, so there alpha=+ψ_ENU=−ψ_NED; on hardware alpha=+ψ_NED, the opposite sign. `w[2]`'s sign already matches PX4 (−ψ̇_NED), so only alpha is wrong. Do NOT fix with `PLASMC_GT_ALPHA_SIGN=-1`: that also flips `w[2]`. Negate `ry` (or alpha only) in `hw_pos_feedback.py` |
| 18 | **`PLASMC_AU_MAX_Z` caps BRAKING, not descent** (found 2026-09-24) | `a_u(t)[:,2]` pinned at exactly −0.200 while `sigma_z<0` (descending faster than desired) | **Fixed 2026-09-24, DEPLOYED to the Pi 2026-09-25 (sha256-verified), not yet flown.** Now a symmetric `|a_u_z| ≤ PLASMC_AU_MAX_Z`, default 3.0 (was a one-sided 0.2 on the braking side). The 2026-08-03 cap `if a_u[2] < -au_max_z: a_u[2] = -au_max_z` (default 0.2) claims to bound DOWNWARD accel. In NED/V coordinates negative a_u_z is UP (`I_a = R a_u − g e3`; logs: a_u_z=+0.54 gives I_a_z=−9.27), so it limits upward braking to 0.2 m/s² and leaves downward accel unlimited. It was pinned in 33/33 flights, for 49% of each descent on average, usually from 2.4-2.8 m down. With any hover shortfall (0.01 throttle ≈ 0.27 m/s² downward bias) the controller cannot arrest the descent. This, not the adaptive gains, is why the vertical loop "doesn't brake" (cf. the kappa_z non-adaptation thread). It also drives the vertical funnel breach (`|σ_z|/p_z` 1.2-2.1 in 25/33 flights). Hardware-only; PX4 never had it |
| 19 | **Translational integral (`izeta`) frozen for the whole flight under `HW_POS_FEEDBACK`** (found 2026-09-24) | `izeta(t)` identically 0 in every flight | **Fixed 2026-09-24, DEPLOYED to the Pi 2026-09-25 (sha256-verified), not yet flown.** `_feat_fresh` is forced True when `self._gt_feedback is not None` at both sites (`is_e_n`, `izeta`), and the yaw-hold `FEATURE_IS_STALE` trigger is skipped in that mode. Watch the first flights: the Ω·∫ζ term is now live for the first time on hardware. The 2026-07-31 conditional-integration freeze gates on the CAMERA's `FEATURE_PTS_FRESH` (controller.py ~2168; the same gate on `is_e_n` ~1851). In HW_POS mode the measurement is the always-fresh EKF, and the camera usually doesn't see the marker, so the Ω·∫ζ term never acts. Same class as the 08-19 `CBF_CORNERS_STALE` bypass, which already treats corners as never stale when `self._gt_feedback is not None`. Latent sibling: the yaw terminal-hold gates on the camera's `FEATURE_IS_STALE` (~2486) without a bypass. It didn't fire this session (w_u_z was not zeroed) |
| 20 | **Lateral actuation lag ~0.2-0.6 s** (measured 2026-09-24) | Lag-scan of commanded `I_a_xy` against achieved horizontal accel (2nd derivative of NED position) | **Characterisation, not a bug.** Direction is correct: the command points at the marker (median within ±30°) and the `s` bearing rotates with yaw as the V-frame should. Delivered magnitude is 30-180% of commanded (median ~0.8). The analytic feedback itself is accurate (h_z vs −v_z/Z: 20 ms lag, scale 0.99). A fixed ~0.4 s actuation lag with 1/Z gain growth explains the near-ground lateral oscillation in #16. The #17 yaw spin (~0.2 rad/s) adds rotating-frame coupling |
...
- **2026-09-24 (second pass):** Controller-internals review of the same session added #17 (yaw alpha sign, NED vs ENU), #18 (AU_MAX_Z caps braking), #19 (izeta frozen by the camera freshness gate) and #20 (lateral actuation lag). #14's framing is refined: battery sag supplies the downward bias, and #18 is why the controller could not correct it.
- **2026-09-24:** Analysed the 39-attempt HW_POS_FEEDBACK gain-sweep session (`Test_Data/Landing/2026-09-24`). Added catalog #14 (battery-sag hover deficit, the dominant confound), #15 (3-frame GT touchdown false latch) and #16 (near-ground lateral divergence). Method notes: `Local_Logs` time starts at 0, so align it as `lt - lt[0] + Control_Data t[0]`. A wrapped command line in the transcript can hide a run's config (15:14:48 was K_R=1.0). The `.ulg` clock ran ~21 min ahead of the Pi that day (Pi 15:07 ↔ `15_28_53.ulg`).
