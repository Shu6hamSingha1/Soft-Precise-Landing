function [o, cs] = flow_surface(V_s, V_h, B_w_c, I_R_C, zeta_r, dzeta_r, s_dot_meas, t, P, cs)
%FLOW_SURFACE  Optic-flow funnel + combined sliding surface (tex Sec. III-A.2).
%   Builds the desired flow h_d (measured s_dot + clean-IMU rotation FF + descent),
...
%   Returns struct o with fields used by blocks.asmc; cs carries integrator/filters.
    ...
    e3 = [0;0;1];
    % s_dot_meas (smooth4-filtered measured centroid rate) is passed in so the SAME
    % rate feeds both h_d here and the position-funnel dr_bar_e [tex eq. s dot, measured].
    ...
    h_d_noS = rot + h_d_ff;                      % transport+descent (no s_dot)
    V_h_d   = [s_dot_meas; 0] + h_d_noS;
    V_h_e   = V_h - V_h_d;
    ...
    % d/dt(h_d) with s_ddot DROPPED (transport+descent only), smooth4 + hard cap
    if cs.k == 1, rdh = zeros(3,1); else, rdh = (h_d_noS - cs.h_d_noS_prev)/P.dt; end
    cs.h_d_noS_prev = h_d_noS;
