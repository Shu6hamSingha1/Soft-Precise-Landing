...
% Computing smooth difference of 'zeta_1'.
    ...
    dzeta_1d = - K.zp*zeta_1(:,idx) - K.zi*izeta_1(:,idx) - K.zd*dzeta_1;
...
% Computing smooth difference of 'h_d'.
    ...
    c = cross(V_dw, V_s(1:3,idx)) + cross(V_w, cross(V_w, V_s(1:3,idx))) ...
        + 2 * cross(V_w, V_h(:,idx)) - (dot(V_h(:,idx) + cross(V_w, V_s(1:3,idx)), e3))*V_h(:,idx) - 0.0*V_dh_d;
...
% Computing desired Thrust force of Camera
    B_T_cd(idx) = - m*(I_a_cd(3,idx))/(cos(theta_cd)*cos(phi_cd));
...
% Computing the smooth first difference and integral
    ...
    dE_cd = - K.ep * E_e(:,idx) - K.ei * iE_e(:,idx) - K.ed * dE_e(:,idx);
    ...
    B_w_cd(:,idx) = W_d*dE_cd;
   
% *************************************************************************
% Attitude Rate Control
% *************************************************************************
% Computing position of Camera wrt Target Origin
    w_e(:,idx) = B_w_c - B_w_cd(:,idx);    

% Computing the smooth first difference of desired Inner Loop Output
    if idx == 1
        iw_e(:,idx) = dt*w_e(:,idx)/2;
        raw_dw_d(:,idx+3) = zeros(size(B_w_cd));
        raw_dw_e(:,idx+3) = zeros(size(w_e));
    else
        iw_e(:,idx) = iw_e(:,idx-1) + dt*(w_e(:,idx-1) + w_e(:,idx))/2;
        raw_dw_d(:,idx+3) = (B_w_cd(:,idx) - B_w_cd(:,idx-1))/dt;
        raw_dw_e(:,idx+3) = (w_e(:,idx) - w_e(:,idx-1))/dt;
    end
    dw_d(:,idx) = smooth4(raw_dw_d(:,end-3:end));
    dw_e(:,idx) = smooth4(raw_dw_e(:,end-3:end));

    B_dw_cd(:,idx) = - K.wp * w_e(:,idx) - K.wi * iw_e(:,idx) ... 
        - K.wd * dw_e(:,idx) + dw_d(:,idx);

% Computing Inner Loop Control Output        
    B_tau_cd(:,idx) = J * B_dw_cd(:,idx) + cross(B_w_cd(:,idx), J * B_w_cd(:,idx));
    % B_tau_cd(:,idx) = J * B_dw_cd(:,idx) + cross(B_w_c, J * B_w_c);

    if norm(B_tau_cd(:,idx)) > 1e02 || any(isnan(B_tau_cd(:,idx)))
        break; % Break out of the outer loop
    end
...
% *************************************************************************   
    if idx > delay         
        u = [B_tau_cd(:,idx - delay); B_T_cd(idx - delay)];
    else
        u = zeros(4,1);
    end
    ...
    % for j=1:4 
    %     if T(j) > T_max
    %         disp(['Thrust on Motor ', num2str(j), ' has breached the ', ...
    %             'upper bound.'])
    %         T(j) = T_max;
    %     elseif T(j) < 0
    %         disp(['Thrust on Motor ', num2str(j), ' has breached the ', ...
    ...
    % System Control Inputs after Input Saturation 
        u = M*T;

% Solving UAV Fight Dynamics using Control Inputs Using RK-5 Method
    x_c = RK5(@(t, x) UAVDyn(t, x, u), t0, x_c, dt);
...
% *************************************************************************
    ...
    D_DS(:,idx) = [V_h_d(:,idx); I_a_cd(:,idx); E_crd; dE_cd; B_w_cd(:,idx); B_dw_cd(:,idx)];

% *************************************************************************
% Termination Condition
% *************************************************************************
    if norm(I_p_c - x_t(1:3,idx)) <= 0.01
        fprintf("Landed\n");
        break;
    end
...
% x = strtrim(input('Save? (Y/N): ', 's'));
% if ~isempty(x) && upper(x(1)) == 'Y'
%     [ok,msg] = copyfile("bestParam.mat","bestParam_backup.mat");
%     if ~ok
%         warning("Could not copy: %s", msg)
%     end
%     save("bestParam.mat", 'K');
%     fprintf('Param file updated.\n');
% else
%     fprintf('Param file not updated.\n');
% end