function cb_tau_finish()
% kR=[3;3;0.5] symmetric (24/25; only Liss IC3 fails, X-ring vel~0.25-0.27).
% Per-axis kR split is a dead end (kR_y<3 breaks Static IC3/IC5 + Circ IC4/IC5
% in the full 5x5). tau is the only collateral-free lever that helped L3
% (0.7->0.8: 0.27->0.25). Push tau harder, keep responsiveness elsewhere.
    trajs={"Static","Linear","Sinusoidal","Lissajous","Circular"};
    ICs={[0;0;-5],[2;2;-5],[2;-2;-5],[2;2;-7],[2;2;-3]}; icn={'IC1','IC2','IC3','IC4','IC5'};
    taus=[0.9 1.0 1.2 1.5];
    for ki=1:numel(taus)
      tau=taus(ki); SP=0; LD=0; worst=0;
      fprintf('=== tau=%.2f kR=[3 3 0.5] ===\n',tau);
      for ti=1:5
        line=sprintf('%-11s',trajs{ti});
        for ii=1:5
          d=run_one(ICs{ii},trajs{ti},tau);
      ...
      fprintf('  >> tau=%.2f  SP=%d/25 land=%d/25 worstResid=%.3f\n\n',tau,SP,LD,worst);
    end
end
function d=run_one(ic,traj,tau)
