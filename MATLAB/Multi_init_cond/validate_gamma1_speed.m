%% Speed-envelope check for Gamma(1)-only candidate
% Baseline envelope: Linear 1.25, Sin 1.75, Circ 1.00, Liss 1.50
% Test whether Gamma = diag([0.1875, 0.25, 0.5]) alone preserves envelope.
...
for t = 1:numel(trajList)
    ...
    for m = mults
        row = row + 1;
        [nl,mt,mxy,maxxy,failIC] = run_5ic_speed(p0,numRuns,trajType,m,ovr_gamma);
...
save(fullfile(fileparts(mfilename('fullpath')),'Datasets','validate_gamma1_speed.mat'), ...
     'results','mults','trajList');

fprintf('\n======== GAMMA(1)-ONLY SPEED ENVELOPE ========\n');
for t = 1:numel(trajList)
