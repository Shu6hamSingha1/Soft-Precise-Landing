...
% Control Parameter Tuner - Added a for loop with multiple termination
% conditions.
% Outer Loop --> s --> h --> a --> E
...
% startTimestamp = datetime('2025-11-07 14:52:00');

for n=1:10000000
    clearvars -except n startTimestamp threshold
    ...
    % *************************************************************************
        if norm(I_p_c - x_t(1:3,idx)) <= threshold
            fprintf("Threshold has been reduced to %.5f\n", threshold/2);
            threshold = threshold/2;
    
            if idx > 100
                fprintf("Landed\n");
                filename = sprintf('%s.mat', datestr(timestamp, 'yyyymmdd_HHMMSS'));
                data = fullfile('Dataset', filename);
                save(data);
            end
        end
