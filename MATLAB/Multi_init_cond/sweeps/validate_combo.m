%% VALIDATE COMBINED CANDIDATE (2026-04-14 fresh deep-sweep winners)
% New baseline (already locked): p_20=[25;25;8], zd=diag([1.3,1.3]).
% Fresh winners from sweep_deep_rerun.log:
%   p_20(3) ×0.50 -> 4.0   (Pareto: aggT 8.94->8.16, maxXY 0.0454->0.0375)
%   zd       ×0.75 -> 0.975 (dominant xy: maxXY 0.0454->0.0244)
%
...
% Candidate overrides
% p_20 idx=3 x0.5 scales only the vertical entry: [25;25;8] -> [25;25;4]
% zd (scalar scale) halves both diag entries: diag([1.3,1.3]) -> diag([0.975,0.975])
ovr_p20 = struct('p_20', [25; 25; 4]);
ovr_zd  = struct('zd',   diag([0.975, 0.975]));
ovr_combo = struct('p_20', [25; 25; 4], ...
                   'zd',   diag([0.975, 0.975]));
