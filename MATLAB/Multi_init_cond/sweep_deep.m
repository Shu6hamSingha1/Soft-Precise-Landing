...
param_list = {
    ...
    'p_20',    [12;12;5],              1, 'outer';   % lateral
    'p_20',    [12;12;5],              3, 'outer';   % vertical
    'p_2inf',  [1.5;1.5;2],            1, 'outer';
    'p_2inf',  [1.5;1.5;2],            3, 'outer';
    % Sliding-mode law
    'Omega',   diag([0.005,0.005,0.01]), 1, 'outer'; % lateral
    'Omega',   diag([0.005,0.005,0.01]), 3, 'outer'; % vertical
    'Gamma',   diag([0.25,0.25,0.5]),  1, 'outer';
    'Gamma',   diag([0.25,0.25,0.5]),  3, 'outer';
    'E',       diag([2.5,2.5,0.5]),    1, 'outer';
    'E',       diag([2.5,2.5,0.5]),    3, 'outer';
    % Adaptive law
    'N',       diag([0.02,0.02,0.05]), 0, 'outer';
    'P',       diag([1.5,1.5,5]),      0, 'outer';
    'kappa_0', [0.1;0.1;0.2],          0, 'outer';

    % ============== INNER ==============
    % Attitude PID (roll/pitch)
    'ep',        diag([5.0, 5.0]),     0, 'inner';
    'ei',        diag([0.1, 0.1]),     0, 'inner';
    'ed',        diag([0.1, 0.1]),     0, 'inner';
    % Body-rate PID
    'wp',        diag([5.0, 5.0, 5.0]),0, 'inner';
    'wi',        diag([0.01,0.01,0.1]),0, 'inner';
    'wd',        diag([0.1, 0.1, 0.2]),0, 'inner';
    'ff',        diag([0.1, 0.1, 0.1]),0, 'inner';
    % Yaw adaptive SMC
    'Omega_a',   1.5,   0, 'inner';
    'Gamma_a',   0.3,   0, 'inner';
    'n_a',       0.05,  0, 'inner';
    'p_a',       2,     0, 'inner';
    'kappa_a_0', 0.1,   0, 'inner';
    'E_a',       2.5,   0, 'inner';
};
