function cb_skept_07()
% KEEP s_ddot (tau=0.8) + REDUCED chi_r=0.7 (kills the high-chi_r_x divergence). Full 5x5.
% Where does it stand, and which cells fail (precise vs soft, per-axis)? p_2inf=0.5, p_r_inf=1.0.
    ...
    fprintf('  >> tau0.8+chi_r=0.7  SP=%d/25 land=%d/25 worstResid=%.3f\n',SP,LD,worst);
...
function d=run_one(ic,traj)
    ...
    IC_OVERRIDE=ic; NOISE_OVERRIDE=0; TRAJ_OVERRIDE=traj; COMBINED_BARRIER=1; C_SIMPLE=1; CB_DROP_SDDOT=0; CB_SDDOT_TAU=0.8; P2INF_XY_OVERRIDE=[0.5;0.5]; CHI_R_OVERRIDE=[0.7;0.7]; PRINF_OVERRIDE=[1.0;1.0];
