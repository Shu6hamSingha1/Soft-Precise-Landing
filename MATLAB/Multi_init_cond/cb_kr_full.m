function cb_kr_full()
% Inner-loop fix validated on hard cells -> full 5x5 at kR/kOmega x2 and x3 (low-tau keep-s_ddot).
% Net improvement? Does it break previously-passing cells (esp. Linear IC5, the old kR-raise victim)?
    for s=[2 3]
      ...
      for ti=1:5
        ...
        for ii=1:5
          d=run_one(ICs{ii},trajs{ti},s);
          ok=d.land&&d.xy<=0.08&&d.vel<=0.20; SP=SP+ok; LD=LD+d.land; worst=max(worst,d.resid);
          ...
          if ~ok; fails{end+1}=sprintf('%s %s xy=%.3f vel=%.3f',trajs{ti},icn{ii},d.xy,d.vel); end
        end
        fprintf('%s\n',line);
      end
      fprintf('  >> kR x%d (low-tau keep-s_ddot)  SP=%d/25 land=%d/25 worstResid=%.3f\n',s,SP,LD,worst);
      for f=1:numel(fails); fprintf('   FAIL %s\n',fails{f}); end
    end
end
function d=run_one(ic,traj,s)
