...
\begin{equation} \label{Eq. (1)}
    \dot{\boldsymbol{r}}_{\text{t}} = \boldsymbol{v}_{\text{t}} - \boldsymbol{v}_{\text{b}} - \boldsymbol{\omega}_{\text{b}} \times \boldsymbol{r}_{\text{t}} = - \boldsymbol{v} - \boldsymbol{\omega} \times \boldsymbol{r}_{\text{t}} - \boldsymbol{\omega}_{\text{t}} \times \boldsymbol{r}_{\text{t}}
\end{equation}
where $\boldsymbol{\nu} = [\boldsymbol{v}, \boldsymbol{\omega}]^{\top} = [\boldsymbol{v}_{\text{b}} - \boldsymbol{v}_{\text{t}}, \boldsymbol{\omega}_{\text{b}} - \boldsymbol{\omega}_{\text{t}}]^{\top}$ is the relative spatial velocity.

\subsection{Target Image Features}
In this section, a camera is utilized in the UAV system to obtain the target image features to coordinate the UAV motion.
...
To control the yaw rotation of the quadrotor based on image features, we can use the object orientation $\alpha$ defined by:
...
To generate suitable control commands for the UAV flight dynamics \eqref{quadrotor dynamics: equation}, we can use optic flow, which is defined as:
...
Consider the output for the outer loop control to be image feature vector $\boldsymbol{s}$ as defined below:
\begin{equation}
    \boldsymbol{s} = \begin{bmatrix} \hat{x} & \hat{y} & \alpha \end{bmatrix}^{\top} = \begin{bmatrix} \frac{x_{\text{t}}}{z_{\text{t}}} & \frac{y_{\text{t}}}{z_{\text{t}}} & \frac{1}{2} \text{tan}^{-1}\left( \frac{2 \mu_{11}}{\mu_{20} - \mu_{02}}\right)
    \end{bmatrix}^{\top}
\end{equation}
Referring to Appendix, the time-derivative of $\boldsymbol{s}$ is given as:
\begin{align}
    \dot{\boldsymbol{s}} =& \begin{bmatrix} \dot{\hat{x}} \\ \dot{\hat{y}} \\ \dot{\alpha} \end{bmatrix}
    = \begin{bmatrix}
    L_{\boldsymbol{v}} \boldsymbol{h}_{\boldsymbol{v}} + L_{\boldsymbol{\omega}} \boldsymbol{\omega}_{\text{b}} \\
    \boldsymbol{l}_{\boldsymbol{\omega}}^{\top} \boldsymbol{h}_{\boldsymbol{\omega}}
    \end{bmatrix}
\end{align}where 
\begin{align*}
    L_{\boldsymbol{v}} &= \begin{bmatrix} - 1 & 0 & \hat{x} \\ 0 & -1 & \hat{y}\end{bmatrix},\\
    L_{\boldsymbol{\omega}} &= \begin{bmatrix} \hat{x} \hat{y} & - (1 + \hat{x}^2) & \hat{y} \\
    1 + \hat{y}^2 & - \hat{x} \hat{y} & - \hat{x}\end{bmatrix}, \\
    \boldsymbol{l}_{\boldsymbol{\omega}} 
...
\end{align*}
Now, the goal is to design a visual feedback outer-loop controller to provide suitable angular velocity command $\boldsymbol{u}_o$. The command $\boldsymbol{u}_o$ will be fed to an inner-loop controller such that the output $\boldsymbol{s}$ converges to a predefined and arbitrarily small neighborhood of $\boldsymbol{s}^* = [\hat{x}^*,\, \hat{y}^*,\, \alpha^*]^{\top}$ without violating the visibility constraints, where $\boldsymbol{s}^*$ is constant. Hence, we obtain:
\begin{equation} \label{perturbed system dynamics for outer loop: equation}
    \dot{\boldsymbol{s}}_e = \dot{\boldsymbol{s}} - \dot{\boldsymbol{s}}^* = \mathcal{B}_o \boldsymbol{u}_o + \boldsymbol{c}_o + \boldsymbol{d}_o
\end{equation}
where
\begin{align*}
    \boldsymbol{u}_o = \boldsymbol{\omega}_{\text{b}}^*, 
    \mathcal{B}_o =\begin{bmatrix} L_{\boldsymbol{\omega}} \\ \boldsymbol{l}_{\boldsymbol{\omega}}^{\top} \end{bmatrix},
    \boldsymbol{c}_o = \begin{bmatrix} L_{\boldsymbol{v}} \boldsymbol{h}_{\boldsymbol{v}} \\ 0 \end{bmatrix}, 
    \boldsymbol{d}_o = \begin{bmatrix}  O_{2 \times 1} \\ -\boldsymbol{l}_{\boldsymbol{\omega}}^{\top} \boldsymbol{\omega}_{\text{t}} \end{bmatrix}.
\end{align*}
...
\textit{Assumption 1}: Given that $\boldsymbol{u}_o(t)$ is bounded, the disturbance $\boldsymbol{d}_o(t)$ in \eqref{perturbed system dynamics for outer loop: equation} is a uniformly bounded function with an unknown upper bound vector $\tilde{\boldsymbol{d}}_{o}$, i.e. $\lvert \boldsymbol{d}_o(t) \rvert \leq \tilde{\boldsymbol{d}}_{o}$.

Next, consider the output for the inner loop control to be optic flow vector $\boldsymbol{q}$ as defined below:
\begin{equation}
    \boldsymbol{q} = \begin{bmatrix} h_r & \boldsymbol{\omega}_{\text{b}}^{\top} \end{bmatrix}^{\top} = \begin{bmatrix} \frac{v_z}{z_{\text{t}}} & \boldsymbol{\omega}_{\text{b}}^{\top}
    \end{bmatrix}^{\top}
\end{equation}
Referring to Appendix, the time-derivative of $\boldsymbol{q}$ is given as:
\begin{equation}
    \dot{\boldsymbol{q}} = \begin{bmatrix} \dot{h}_r \\ \dot{\boldsymbol{\omega}}_{\text{b}} \end{bmatrix} = \begin{bmatrix}
        \frac{\dot{v}_z}{z_{\text{t}}} + \left( h_r + \hat{y} \omega_{\text{b}x} - \hat{x} \omega_{\text{b}y} \right) h_r \\ \dot{\boldsymbol{\omega}}_{\text{b}}
    \end{bmatrix}
\end{equation}
...
the UAV flight dynamics \eqref{quadrotor dynamics: equation} such that the output $\boldsymbol{q}$ converges to an arbitrarily small neighborhood of $\boldsymbol{q}^* = \begin{bmatrix} h_r^* & \boldsymbol{\omega}_{\text{b}}^{*\top} \end{bmatrix}$, where $h_r^*$ is a constant. Hence, we obtain:
\begin{equation} \label{perturbed system dynamics for inner loop: equation}
    \dot{\boldsymbol{q}}_{e} = \dot{\boldsymbol{q}} - \dot{\boldsymbol{q}}^* = \mathcal{B}_i \boldsymbol{u}_i + \boldsymbol{c}_i + \boldsymbol{d}_i
\end{equation}
\begin{align*}
    \boldsymbol{u}_i &= \begin{bmatrix} T_{u} & \boldsymbol{\tau}_{u}^{\top} \end{bmatrix}^{\top}, 
    \mathcal{B}_i = \begin{bmatrix} \frac{1}{m z_{\text{t}}} & O_{1 \times 3} \\ O_{3 \times 1} & J^{-1} \end{bmatrix} = \mathcal{B}_{i 1} \mathcal{B}_{i 2}, \\
    ...
    \mathcal{B}_{i 1} &= \begin{bmatrix} \frac{1}{z_{\text{t}}} & O_{1 \times 3} \\ O_{3 \times 1} & I_{3 \times 3} \end{bmatrix}, \mathcal{B}_{i 2} = \begin{bmatrix} \frac{1}{m} & O_{1 \times 3} \\ O_{3 \times 1} & J^{-1} \end{bmatrix}.
\end{align*}
Note that the matrix $\mathcal{B}_{i2}$ and the function $\boldsymbol{c}_i(t)$ is known and can be estimated for feedback linearization. Whereas, matrix $\mathcal{B}_{i1}(t)$ and function $\boldsymbol{d}_i(t)$ are unknown.

\textit{Assumption 2}: Given that $\boldsymbol{u}_i(t)$ is bounded, the disturbance $\boldsymbol{d}_i(t)$ in \eqref{perturbed system dynamics for inner loop: equation} is a uniformly bounded function with an unknown upper bound vector $\boldsymbol{d}_{i(\text{max})}$, i.e. $\lvert \boldsymbol{d}_i(t) \rvert \leq \boldsymbol{d}_{i(\text{max})}$. Moreover, for any arbitrary vector $\boldsymbol{x} \in \mathbb{R}^{4}$, there exists constant but unknown positive definite diagonal matrices $\mathcal{B}_{i (\text{min})}$ and $\mathcal{B}_{i (\text{max})}$ such that $0 < \boldsymbol{x}^{\top} \mathcal{B}_{i (\text{min})} \boldsymbol{x} \leq \boldsymbol{x}^{\top}  \mathcal{B}_{i1}(t) \boldsymbol{x} \leq \boldsymbol{x}^{\top} \mathcal{B}_{i (\text{max})} \boldsymbol{x}$.

\begin{figure}[hbt!]
\centering
    \includegraphics[width=0.5\textwidth]{Figures/Block Diagram_v1.pdf}
    \caption{Block diagram illustrating the control pipeline for soft landing of a UAV on a mobile target, highlighting the robust adaptive translational control pipeline implemented in the outer-loop controller.} 
...
In order to incorporate visibility constraint in the proposed control formulation, the outer-loop output error $\boldsymbol{s_e}(t) = [s_{e 1}(t), \, s_{e 2}(t), \, s_{e 3}(t)]^{\top}$ is constrained as $\boldsymbol{s}_{e}(t) \in (- \boldsymbol{p}_o(t), \, \boldsymbol{p}_o(t))$, $\forall \, t \geq 0$ with $\boldsymbol{p}_o(t) = [p_{o1}(t), \, p_{o2}(t), \, p_{o2}(t)]^{\top}$.
...
$\lim_{\text{t} \to \infty} \boldsymbol{p}_o(t) = \boldsymbol{p}_{o(\infty)} > \boldsymbol{0}_{3 \times 1}$, where predefined positive constants $\boldsymbol{p}_{o(0)} = [p_{o(0) 1}, \, p_{o(0) 2}, \, p_{o(0) 3}]^{\top}$, $\boldsymbol{p}_{o(\infty)} = [p_{o(\infty) 1}, \, p_{o(\infty) 2}, \, p_{o(\infty) 3}]^{\top}$, $\Xi_o = \text{diag}[\xi_{o1}, \, \xi_{o2}, \, \xi_{o3}]$ represent the maximum allowable overshoot, the steady-state residual set and the minimum convergence rate of $\boldsymbol{s}_{e}(t)$, respectively.

A non-linear transformation is formulated to map the constrained output error $\boldsymbol{s_e}(t)$ onto a new unconstrained variable $\boldsymbol{\zeta}_o(t) = [\zeta_{o1}(t),\, \zeta_{o2}(t),\, \zeta_{o3}(t)]^{\top}$ such that:
\begin{align}
    \label{error transformation for outer loop: equation}
    \boldsymbol{s}_{e}(t) &= \mathcal{S}_o(\boldsymbol{\zeta}_o) \boldsymbol{p}_o(t), \, \mathcal{S}_o(\boldsymbol{\zeta}_o) = \text{diag}[S_o(\zeta_{ok})], \nonumber \\
...
\begin{equation} \label{inverse transformation function for outer loop}
    \zeta_{ok}(t) = S_o^{-1}(s_{e k}, p_{ok}) = ln \left(\frac{p_{ok} + s_{e k}}{p_{ok} - s_{e k}} \right) \text{ for } k = \{1, 2, 3\}.
\end{equation}
...
\begin{equation} \label{transformed error derivative for outer loop: equation 2}
    \dot{\boldsymbol{\zeta}}_o(t) = \mathcal{G}_o(t) \big[\dot{\boldsymbol{s}}_{e}(t) - \mathcal{S}_o(\boldsymbol{\zeta}_o) \dot{\boldsymbol{p}}_o(t)\big],    
\end{equation}
where
\begin{align*}
...
In order to maintain the boundedness during landing, the inner loop output error $\boldsymbol{q_e}(t) = [q_{e 1}(t), \, q_{e 2}(t), \, q_{e 3}(t), \, q_{e 4}(t)]^{\top}$ is constrained as $\boldsymbol{q}_{e}(t) \in (- \boldsymbol{p}_i(t), \, \boldsymbol{p}_i(t))$, $\forall \, t \geq 0$ with $\boldsymbol{p}_i(t) = [p_{i 1}(t), \, p_{i 2}(t), \, p_{i 3}(t), \, p_{i 4}(t)]^{\top}$.
...
A non-linear transformation is formulated to map the constrained output error $\boldsymbol{q_e}(t)$ onto a new unconstrained variable $\boldsymbol{\zeta}_i(t) = [\zeta_{i 1}(t),\, \zeta_{i 2}(t),\, \zeta_{i 3}(t),\, \zeta_{i 4}(t)]^{\top}$ such that:
\begin{align}
    \label{error transformation for inner loop: equation}
    \boldsymbol{q}_{e}(t) &= \mathcal{S}_i(\boldsymbol{\zeta}_i) \boldsymbol{p}_i(t), \, \mathcal{S}_i(\boldsymbol{\zeta}_i) = \text{diag}[S_i(\zeta_{ik})], \nonumber \\
...
\begin{equation} \label{inverse transformation function for inner loop}
    \zeta_{ik}(t) = S^{-1}_i(q_{e k}, p_{i k}) = ln \left(\frac{p_{i k} + q_{e k}}{p_{i k} - q_{e k}} \right) \text{ for } k = \{1, 2, 3, 4\},
\end{equation}
...
\begin{equation} \label{transformed error derivative for inner loop: equation 2}
    \dot{\boldsymbol{\zeta}}_i(t) = \mathcal{G}_i(t) \big[\dot{\boldsymbol{q}}_{e}(t) - \mathcal{S}_i(\boldsymbol{\zeta}_i) \dot{\boldsymbol{p}}_i(t)\big],    
\end{equation}
where
\begin{align*}
...
\begin{align}
    ...
    & ~ + \mathcal{G}_i^{-1}(t) \mathcal{X}_i\,\boldsymbol{\zeta}_i(t) \big] \big\} + \mathcal{B}_{i (\text{min})} \, \tilde{\mathcal{D}}_i(t) \boldsymbol{\theta}_i(t), 
\end{align}
where 
\begin{align*}
    \tilde{\mathcal{D}}_i(t) &= \mathcal{B}_{i (\text{min})}^{-1} \,\mathcal{G}_i(t) \begin{bmatrix}  \mathcal{B}_i(t)  \, \mathcal{G}_i^{-1}(t) \,-\, \mathcal{G}_i^{-1}(t) & \mathcal{D}_i(t) \end{bmatrix}, \\
    \boldsymbol{\theta}_i(t) &= \begin{bmatrix}
    \mathcal{G}_i(t) \big[- \boldsymbol{c}_i(t) + \mathcal{S}_i(\boldsymbol{\zeta}_i) \dot{\boldsymbol{p}}_i(t) \big] - \mathcal{X}_i\,\boldsymbol{\zeta}_i(t) \\ \boldsymbol{g}_i(t)
    \end{bmatrix}, \\
...
\end{align*}
There exists an unknown positive constant $\tilde{d}_i = \lVert \mathcal{B}_{i (\text{min})} \rVert_2^{-1} \Big[\text{max}\left(|\lVert \mathcal{B}_{i (\text{max})} \rVert_2 - 1|,|\lVert \mathcal{B}_{i (\text{min})} \rVert_2 - 1|\right) + \lVert \boldsymbol{d}_{i(\text{max})} \rVert_2 \Big]$ such that $\lVert\tilde{\mathcal{D}}_i(t)\rVert_2\leq\tilde{d}_i$.
...
\begin{align} \label{control law for inner loop: equation}
    \boldsymbol{u}_i(t) =&~ \mathcal{B}_{i2}^{-1} \mathcal{G}_i^{-1}(t) \big[ \boldsymbol{u}_{i (\text{sw})}(t) + \boldsymbol{u}_{i (\text{eq})}(t) \big], \nonumber \\
    ...
    \boldsymbol{u}_{i (\text{eq})}(t) =&~\mathcal{G}_i(t)\big[- \boldsymbol{c}_i(t) + \mathcal{S}_i(\boldsymbol{\zeta}) \dot{\boldsymbol{p}}_i(t) \big] - \mathcal{X}_i\,\boldsymbol{\zeta}_i(t),
\end{align}
...
\begin{align}
    ...
    \dot{\boldsymbol{\sigma}}_i(t) =&~ - \mathcal{G}_i(t) \mathcal{B}_{i1}(t) \mathcal{G}_i^{-1}(t) \big[\Gamma_i \boldsymbol{\sigma}_i(t) \nonumber \\ 
    & ~ + \lVert \boldsymbol{\theta}_i(t) \rVert_2 \, \text{sat}[ \mathcal{E}_i^{-1}\boldsymbol{\sigma}_i(t)] \boldsymbol{\kappa}_i(t)\big] + \mathcal{B}_{i (\text{min})} \, \tilde{\mathcal{D}}_i(t) \boldsymbol{\theta}_i(t),
\end{align}
...
\textit{Theorem 1}:
Consider the nonlinear system \eqref{perturbed system dynamics for outer loop: equation} such that the initial value of the outer loop output error $\boldsymbol{s}_e(0)$ is within the constrained region, i.e., $- \boldsymbol{p}_{o (0)}(t) < \boldsymbol{s}_{e}(0) < \boldsymbol{p}_{o (0)}(t), \, \forall \, t \geq 0$. Then, under \textit{Assumption 1}, system \eqref{perturbed system dynamics for outer loop: equation} under the proposed control law \eqref{control law for outer loop: equation} accomplishes exponential convergence of $\boldsymbol{\zeta}_o(t)$ to a UUB given by
\begin{equation}
    \psi_o {=} \sqrt{\frac{\lambda_{\text{max}}(\mathcal{P}_o)}{\left(\phi_1 - \phi_2\right)}} \lVert \tilde{\boldsymbol{d}}_{o} \rVert_2
\end{equation}
where $\phi_1 \triangleq \frac{\text{ min}\big(2 \lambda_{\text{min}}\left(\Gamma_o \right),\lambda_{\text{min}}\left(\mathcal{P}_o \right)\big)}{\text{max}\big(1,\lambda_{\text{max}}\left(\mathcal{N}_o^{-1} \right)\big)}>0$, $0<\phi_2<\phi_1$. Moreover, the control law \eqref{control law for outer loop: equation} ensures that the trajectory of the system \eqref{perturbed system dynamics for outer loop: equation} lies within the prescribed constraint set $- \boldsymbol{p}_{o}(t) < \boldsymbol{s}_{e}(t) < \boldsymbol{p}_{o}(t), \, \forall \, t \geq 0$.

As a first step, observe that $\zeta_{ok}(0) = S_{ok}^{-1} [s_{ek}(0)/p_{o(0)k}]$ is well-defined for $\lvert s_{ek}(0) \rvert < p_{o(0)k}$ for $k = \{1, 2, 3\}$. The dynamics of $\boldsymbol{\zeta}_o(t)$ then evolves according to the system \eqref{unconstrained system for outer loop: equation} under the control law \eqref{control law for outer loop: equation} of the PLASMC strategy.
...
\begin{equation} \label{Lyapunov function for outer loop: equation}
    V_o(t) = \frac{1}{2}\boldsymbol{\sigma}_o^{\top}(t)\boldsymbol{\sigma}_o(t) + \frac{1}{2} \left[\boldsymbol{\kappa}_o(t) - \tilde{\boldsymbol{d}}_{o}\right]^{\top} \mathcal{N}_o^{-1} \left[\boldsymbol{\kappa}_o(t) - \tilde{\boldsymbol{d}}_{o}\right],
\end{equation}
where the unknown positive constant vector $\tilde{\boldsymbol{d}}_{o}$ satisfies $\lvert \boldsymbol{d}_o(t) \rvert \leq \tilde{\boldsymbol{d}}_{o}$.
...
\label{Lyapunov derivative for outer loop: equation 1}
   \dot{V}_o(t)=&~\boldsymbol{\sigma}_o^{\top}(t) \dot{\boldsymbol{\sigma}}_o(t) + \left[\boldsymbol{\kappa}_o(t) - \tilde{\boldsymbol{d}}_{o}\right]^{\top} \mathcal{N}_o^{-1} \dot{\boldsymbol{\kappa}}_o(t)\nonumber\\
   ...
   ~ & +\left[\boldsymbol{\kappa}_o(t) - \tilde{\boldsymbol{d}}_{o}\right]^{\top} \Big[\mathcal{G}_o(t) \lvert \boldsymbol{\sigma}_o(t) \rvert - \mathcal{P}_{o} \boldsymbol{\kappa}_o(t)\Big].
\end{align} 
...
\label{Lyapunov derivative for outer loop: equation 2}
   \dot{V}_o(t) \leq&- \boldsymbol{\sigma}_o^{\top}(t) \Gamma_o \boldsymbol{\sigma}_o(t) - \big[ \tilde{\boldsymbol{d}}_{o} - \lvert \boldsymbol{d}_o(t) \rvert\big]^{\top} \mathcal{G}_o(t) \lvert \boldsymbol{\sigma}_o(t) \rvert \nonumber\\
   ~ & - \boldsymbol{\kappa}_o^{\top}(t) \mathcal{P}_{o} \boldsymbol{\kappa}_o(t) + \tilde{\boldsymbol{d}}_{o}^{\top} \mathcal{P}_{o} \boldsymbol{\kappa}_o(t)
\end{align}
Given that $\lvert \boldsymbol{d}_o(t) \rvert \leq \tilde{\boldsymbol{d}}_{o}$, it can be shown that $\left[\tilde{\boldsymbol{d}}_{o} -\lvert \boldsymbol{d}_o(t) \rvert\right]^{\top} \mathcal{G}_o(t) \lvert \boldsymbol{\sigma}_o(t) \rvert \geq 0$.
Moreover, since $\mathcal{P}_{o}$ is positive-definite matrix, $\boldsymbol{\kappa}_o^{\top}(t) \mathcal{P}_{o} \boldsymbol{\kappa}_o(t) - \tilde{\boldsymbol{d}}_{o}^{\top} \mathcal{P}_{o} \boldsymbol{\kappa}_o(t) > \left\{\left[\boldsymbol{\kappa}_o(t) - \tilde{\boldsymbol{d}}_{o}\right]^{\top} \mathcal{P}_{o} \left[\boldsymbol{\kappa}_o(t) - \tilde{\boldsymbol{d}}_{o}\right] - \tilde{\boldsymbol{d}}_{o}^{\top} \mathcal{P}_{o} \tilde{\boldsymbol{d}}_{o} \right\}/2$.
Hence,
\begin{align}
\label{Lyapunov derivative for outer loop: equation 3}
    \dot{V}_o(t)~{\leq}~&-\lambda_{\text{min}}(\Gamma_o) \lVert \boldsymbol{\sigma}_o(t) \rVert_2^2 + \frac{1}{2}\tilde{\boldsymbol{d}}_{o}^{\top} \mathcal{P}_{o} \tilde{\boldsymbol{d}}_{o} \nonumber \\ 
    &- \frac{1}{2} \left[\boldsymbol{\kappa}_o(t) - \tilde{\boldsymbol{d}}_{o}\right]^{\top} \mathcal{P}_{o} \left[\boldsymbol{\kappa}_o(t) - \tilde{\boldsymbol{d}}_{o}\right] \nonumber \\ 
    {\leq}~& - \lambda_{\text{min}}(\Gamma_o) \lVert \boldsymbol{\sigma}_o(t) \rVert_2^2 - \frac{1}{2} \lambda_{\text{min}}(\mathcal{P}_o) \lVert \boldsymbol{\kappa}_o(t) - \tilde{\boldsymbol{d}}_{o} \rVert_2^2  \nonumber \\
    & + \frac{1}{2} \lambda_{\text{max}}(\mathcal{P}_o) \lVert \tilde{\boldsymbol{d}}_{o} \rVert_2^2\nonumber\\
    {\leq}~& -\phi_1 V_o(t) + \frac{1}{2} \lambda_{\text{max}}(\mathcal{P}_o) \lVert \tilde{\boldsymbol{d}}_{o} \rVert_2^2,
\end{align}
...
\begin{flalign}
\label{Lyap_deriv_6}
\dot{V}_o(t)&\leq-\phi_2 V_o(t)-(\phi_1-\phi_2) V_o(t)  + \frac{1}{2} \lambda_{\text{max}}(\mathcal{P}_o) \lVert \tilde{\boldsymbol{d}}_{o} \rVert_2^2 \nonumber\\
&\leq-\phi_2 V_o(t)\qquad\forall\, V_o(t)\geq \Theta_o=\frac{\lambda_{\text{max}}(\mathcal{P}_o) \lVert \tilde{\boldsymbol{d}}_{o} \rVert_2^2}{2(\phi_1-\phi_2)}.
\end{flalign}

Invoking \textit{Lemma 1}, any trajectory enters the ball $\mathcal{B}_{ro} = \left \{ \left ( \boldsymbol{\sigma}_o, \, \boldsymbol{\kappa}_o \right ) \in \mathbb{R}^6 | V_o \leq \Theta_o \right\}$ exponentially in finite-time and stays in $\mathcal{B}_{ro}$ for all future time. Note that the size of the ball can be made suitably small through a suitable selection of the control parameters $\lambda_{\text{min}}(\mathcal{P}_o)$, $\lambda_{\text{max}}(\mathcal{P}_o)$, $\lambda_{\text{max}}(\mathcal{N}_o^{-1}) $ and $\lambda_{\text{min}}(\Gamma_o)$, so that by recognizing that $V_o(t) \geq \lVert \boldsymbol{\sigma}_o(t) \rVert_2^2/2$, one has the ultimate bound for $\boldsymbol{\sigma}_o(t)$ as $\lVert \boldsymbol{\sigma}_o(t) \rVert_2 \leq \psi_o {=} \left[\lambda_{\text{max}}(\mathcal{P}_o)/\left(\phi_1 - \phi_2\right)\right]^{\frac{1}{2}} \lVert \tilde{\boldsymbol{d}}_{o} \rVert_2$. This bound is uniform as it is independent of initial conditions. 
Consequently, it follows from \eqref{sigma for outer loop: equation} that the output error variable $\boldsymbol{\zeta}_o(t)$ converges to a small bound around the origin so that from \eqref{error transformation for outer loop: equation}, $- \boldsymbol{p}_{o}(t) < \boldsymbol{s}_{e}(t) < \boldsymbol{p}_{o}(t)$ is satisfied. Thus the prescribed performance constraint is ensured $\forall \, t \geq 0$. This ends the proof.

\textit{Theorem 2}:
Consider the nonlinear system \eqref{perturbed system dynamics for inner loop: equation} such that the initial value of the inner loop output error $\boldsymbol{q}_e(0)$ is within the constrained region, i.e., $- \boldsymbol{p}_{i (0)}(t) < \boldsymbol{q}_{e}(0) < \boldsymbol{p}_{i (0)}(t), \, \forall \, t \geq 0$. Then, under \textit{Assumption 2}, system \eqref{perturbed system dynamics for inner loop: equation} under the proposed control law \eqref{control law for inner loop: equation} accomplishes exponential convergence of $\boldsymbol{\zeta}_i(t)$ to a UUB given by
\begin{equation}
    \psi_i = 2 \sqrt{\frac{\lambda_{\text{max}}(\mathcal{B}_{i(\text{min})})\lambda_{\text{max}}(\mathcal{P}_i)}{\left(\phi_3 - \phi_4\right)}} \tilde{d}_{i},
\end{equation}
where $\phi_3 \triangleq \lambda_{\text{min}}\left(\mathcal{B}_{i(\text{min})} \right) \frac{\text{ min}\big(2 \lambda_{\text{min}}\left(\Gamma_i \right),\lambda_{\text{min}}\left(\mathcal{P}_i \right)\big)}{\text{max}\big(1,\lambda_{\text{max}}\left(\mathcal{B}_{i(\text{min})} \right) \lambda_{\text{max}}\left(\mathcal{N}_i^{-1} \right)\big)}>0$, $0<\phi_4<\phi_3$.
Moreover, the control law \eqref{control law for inner loop: equation} ensures that the trajectory of the system \eqref{perturbed system dynamics for inner loop: equation} lies within the prescribed constraint sets $- \boldsymbol{p}_{i}(t) < \boldsymbol{q}_{e}(t) < \boldsymbol{p}_{i}(t) \, \forall \, t \geq 0$.

As a first step, observe that $\zeta_{i k}(0) = S_{i k}^{-1} [q_{e k}(0) / p_{i (0) k}]$ is well-defined for $\lvert q_{e k}(0) \rvert < p_{i (0) k}$ for $k = \{1, 2, 3, 4\}$. The dynamics of $\boldsymbol{\zeta}_i(t)$ then evolves according to the system \eqref{unconstrained system for inner loop: equation} under the control law \eqref{control law for inner loop: equation} of the PLASMC strategy.
...
\begin{align} \label{Lyapunov function for inner loop: equation}
    ...
    &+ \frac{1}{2} \left[\boldsymbol{\kappa}_i(t) - \tilde{d}_{i} \boldsymbol{1}\right]^{\top} \mathcal{B}_{i (\text{min})} \mathcal{N}_i^{-1} \left[\boldsymbol{\kappa}_i(t) - \tilde{d}_{i} \boldsymbol{1}\right],
\end{align}
...
\label{Lyapunov derivative for inner loop: equation 1}
    \dot{V}_i(t)=~&\boldsymbol{\sigma}_i^{\top}(t) \dot{\boldsymbol{\sigma}}_i(t) + \left[\boldsymbol{\kappa}_i(t) - \tilde{d}_{i} \boldsymbol{1}\right]^{\top} \mathcal{B}_{i (\text{min})} \mathcal{N}_i^{-1} \dot{\boldsymbol{\kappa}}_i(t)\nonumber\\
    ...
    & + \boldsymbol{\sigma}_i^{\top}(t) \mathcal{B}_{i (\text{min})} \, \tilde{\mathcal{D}}_i(t) \boldsymbol{\theta}_i(t) \nonumber\\
    & + \left[\boldsymbol{\kappa}_i(t) - \tilde{d}_{i} \boldsymbol{1}\right]^{\top} \mathcal{B}_{i (\text{min})} \mathcal{N}_i^{-1} \Big[\lVert \boldsymbol{\theta}_i(t) \rVert_2 \mathcal{N}_{i} \lvert \boldsymbol{\sigma}_i(t) \rvert \nonumber\\
    & - \mathcal{N}_{i} \mathcal{P}_{i} \boldsymbol{\kappa}_i(t) \Big].
\end{align} 
Since the diagonal matrices $\mathcal{G}_i(t)$, $\mathcal{B}_{i1}$ and $\Gamma_i$ are positive-definite such that $\boldsymbol{\sigma}_i^{\top} \mathcal{B}_{i (\text{min})} \boldsymbol{\sigma}_i \leq \boldsymbol{\sigma}_i^{\top}  \mathcal{B}_{i1}(t) \boldsymbol{\sigma}_i$, it can be shown that $\boldsymbol{\sigma}_i^{\top} \mathcal{B}_{i (\text{min})} \Gamma_i \boldsymbol{\sigma}_i \leq \boldsymbol{\sigma}_i^{\top}  \mathcal{G}_i(t) \mathcal{B}_{i1}(t) \, \mathcal{G}_i^{-1}(t) \Gamma_i \boldsymbol{\sigma}_i$.
...
Therefore, $\lvert \boldsymbol{\sigma}_i(t) \rvert^{\top} \mathcal{B}_{i (\text{min})} \boldsymbol{\kappa}_i(t) \leq \boldsymbol{\sigma}_i^{\top}(t) \mathcal{G}_i(t) \mathcal{B}_{i1}(t) \, \mathcal{G}_i^{-1}(t) \text{sat}[ \mathcal{E}_i^{-1}\boldsymbol{\sigma}_i(t)] \boldsymbol{\kappa}_i(t)$.
...
\label{Lyapunov derivative for inner loop: equation 2}
    \dot{V}_i(t)~{\leq}~& - \boldsymbol{\sigma}_i^{\top} \mathcal{B}_{i (\text{min})} \Gamma_i \boldsymbol{\sigma}_i - \lVert \boldsymbol{\theta}_i(t) \rVert_2 \lvert \boldsymbol{\sigma}_i(t) \rvert^{\top} \mathcal{B}_{i (\text{min})} \boldsymbol{\kappa}_i(t) \nonumber \\
    & + \lVert\tilde{\mathcal{D}}_i(t)\rVert_2 \lVert \boldsymbol{\theta}_i(t) \rVert_2 \lvert \boldsymbol{\sigma}_i(t) \rvert^{\top} \mathcal{B}_{i (\text{min})} \boldsymbol{1} \nonumber \\
    & + \left[\boldsymbol{\kappa}_i(t) - \tilde{d}_{i} \boldsymbol{1}\right]^{\top} \mathcal{B}_{i (\text{min})} \left[\lVert \boldsymbol{\theta}_i(t) \rVert_2 \lvert \boldsymbol{\sigma}_i(t) \rvert - \mathcal{P}_{i} \boldsymbol{\kappa}_i(t) \right] \nonumber \\
    \leq ~ & - \boldsymbol{\sigma}_i^{\top} \mathcal{B}_{i (\text{min})} \Gamma_i \boldsymbol{\sigma}_i \nonumber \\
    & - \left( \tilde{d}_{i} - \lVert\tilde{\mathcal{D}}_i(t)\rVert_2 \right) \lVert \boldsymbol{\theta}_i(t) \rVert_2 \lvert \boldsymbol{\sigma}_i(t) \rvert^{\top} \mathcal{B}_{i (\text{min})} \boldsymbol{1} \nonumber \\
    & - \boldsymbol{\kappa}_i^{\top}(t) \mathcal{B}_{i (\text{min})} \mathcal{P}_{i} \boldsymbol{\kappa}_i(t) + \tilde{d}_{i} \boldsymbol{1}^{\top} \mathcal{B}_{i (\text{min})} \mathcal{P}_{i} \boldsymbol{\kappa}_i(t)
\end{align}
Given that $\lVert\tilde{\mathcal{D}}_i(t)\rVert_2 \leq \tilde{d}_i$, it can be shown that $\left( \tilde{d}_{i} - \lVert\tilde{\mathcal{D}}_i(t)\rVert_2 \right) \lVert \boldsymbol{\theta}_i(t) \rVert_2 \lvert \boldsymbol{\sigma}_i(t) \rvert^{\top} \mathcal{B}_{i (\text{min})} \boldsymbol{1} \geq 0$.
Moreover, since both $\mathcal{B}_{i (\text{min})}$ and $\mathcal{P}_{i}$ are positive-definite matrices, $\boldsymbol{\kappa}_i^{\top}(t) \mathcal{B}_{i (\text{min})} \mathcal{P}_{i} \boldsymbol{\kappa}_i(t) - \tilde{d}_{i} \boldsymbol{1}^{\top} \mathcal{B}_{i (\text{min})} \mathcal{P}_{i} \boldsymbol{\kappa}_i(t) > \left\{\left[\boldsymbol{\kappa}_i(t) - \tilde{d}_{i} \boldsymbol{1}\right]^{\top} \mathcal{B}_{i (\text{min})} \mathcal{P}_{i} \left[\boldsymbol{\kappa}_i(t) - \tilde{d}_{i} \boldsymbol{1}\right] - \tilde{d}_{i}^2 \boldsymbol{1}^{\top} \mathcal{B}_{i (\text{min})} \mathcal{P}_{i} \boldsymbol{1} \right\}/2$.
Hence,
\begin{align}
\label{Lyapunov derivative for inner loop: equation 3}
\dot{V}_i(t) ~ \leq ~ &-\lambda_{\text{min}}(\mathcal{B}_{i(\text{min})})\lambda_{\text{min}}(\Gamma_i) \lVert \boldsymbol{\sigma}_i(t) \rVert_2^2 \nonumber\\
& - \frac{1}{2}\left[\boldsymbol{\kappa}_i(t) - \tilde{d}_{i} \boldsymbol{1}\right]^{\top} \mathcal{B}_{i (\text{min})} \mathcal{P}_{i} \left[\boldsymbol{\kappa}_i(t) - \tilde{d}_{i} \boldsymbol{1}\right] \nonumber\\
& +\frac{1}{2}\tilde{d}_{i}^2 \boldsymbol{1}^{\top} \mathcal{B}_{i (\text{min})} \mathcal{P}_{i} \boldsymbol{1} \nonumber \\
\leq ~ &-\lambda_{\text{min}}(\mathcal{B}_{i(\text{min})}) \Big[ \lambda_{\text{min}}(\Gamma_i) \lVert \boldsymbol{\sigma}_i(t) \rVert_2^2 \nonumber\\
...
\leq~ &-\phi_3 V_i(t) + 2 \lambda_{\text{max}}(\mathcal{B}_{i(\text{min})}) \lambda_{\text{max}}(\mathcal{P}_{i}) \tilde{d}_{i}^2,
\end{align}
where $\phi_3 \triangleq \lambda_{\text{min}}\left(\mathcal{B}_{i(\text{min})} \right) \frac{\text{ min}\big(2 \lambda_{\text{min}}\left(\Gamma_i \right),\lambda_{\text{min}}\left(\mathcal{P}_i \right)\big)}{\text{max}\big(1,\lambda_{\text{max}}\left(\mathcal{B}_{i(\text{min})} \right) \lambda_{\text{max}}\left(\mathcal{N}_i^{-1} \right)\big)}>0$.
