...
\end{equation}
where $\boldsymbol v_{\mathrm{rel}}\in\mathbb R^3$ denote the target translational velocity relative to the UAV expressed in $\mathcal V$, $\boldsymbol\omega_{\mathrm t}$ is the unknown target angular velocity
...
\caption{\textbf{Robustness to five initial conditions for Case~5.} (a) UAV/target trajectories: the shaded region is the $0.08$~m precision region. Touchdown markers: $\blacktriangle$ soft-precise and $\times$ aborted. (b) Camera-frame image-plane trajectories: the gray marker is the desired configuration, while the asymmetric stub encodes image orientation.
The inset magnifies the terminal position and orientation alignment about the desired marker. The legend reports The legend gives the terminal marker-centre offset $\|\delta{}^{\mathcal C}\hat{\boldsymbol r}\|_2$.}
...
(a) Outcomes over the five target-motion cases (S: soft-precise, H: hard/imprecise, A: aborted).
