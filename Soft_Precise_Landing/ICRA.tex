...
$\pm40\%$ variation about the nominal motion, at $\mathrm{IC}_2$. $19$ of
these $20$ runs achieve soft-precise touchdown with unchanged controller
parameters; the exception, the circular case at $\gamma_v=1.4$, exits the
camera field of view before reaching the surface. Across the $44$
soft-precise landings obtained over all $45$ initial-condition,
motion-profile, and speed-sweep tests, the worst-case terminal horizontal
error and relative touchdown speed are $6.06$~cm and $0.091$~m/s,
respectively.
...
VISTA is compared with the performance-constrained PBVS (PBVS--PPC) method~\cite{lin2022}, adaptive-observer PBVS (PBVS--AEDO)~\cite{zhang2026}, prescribed-performance IBVS (IBVS--PPC)~\cite{lin2023}, and feed-forward IBVS (FF--IBVS)~\cite{cho2022}. All methods are evaluated from the common initial condition $\mathrm{IC}_2=[2,2,5]^\top$~m under identical target trajectories and disturbance conditions and use the same geometric attitude-control layer.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]
{Figures/generated/comparison_qual_combined.pdf}
\caption{\textbf{Closed-loop comparison from $\boldsymbol{\mathrm{IC}_2=[2,2,5]^\top}$~m.}
...
(b) Normalized relative touchdown energy $\eta_E=(v_f/0.20)^2$. Only landed runs are shown, and $\log_{10}(\eta_E)<0$ denotes soft touchdown.
(c) UAV landing trajectories for Case~5. Touchdown markers denote $\blacktriangle$ for soft-precise, $\Diamond$ for precise only, $\bigcirc$ for soft only, $\bigtriangledown$ for neither soft nor precise, and $\times$ for aborted.
(d) Normalized marker-centre trajectories for Case~5. Reaching $|{}^{\mathcal C}\tilde r_x|=1$ or $|{}^{\mathcal C}\tilde r_y|=1$ corresponds to the respective FoV boundary.}
\label{fig:controller_comparison}
\end{figure}

Fig.~\ref{fig:controller_comparison}(a) first summarizes the overall closed-loop outcome across the five target-motion cases. VISTA reaches the surface with soft-precise touchdown in all five cases, whereas PBVS--AEDO reaches the surface in only three cases and still produces hard or imprecise touchdowns. The remaining baselines abort before surface contact. Fig.~\ref{fig:controller_comparison}~(b) further distinguishes the touchdown quality of the surface-reaching runs using the normalized relative touchdown energy. VISTA remains below the softness threshold in all five cases, whereas the surface-reaching PBVS--AEDO runs exceed it.

To clarify the underlying closed-loop behavior, Fig.~\ref{fig:controller_comparison}~(c) visualizes the UAV trajectories for the representative Case~5. The corresponding marker-centre evolution in Fig.~\ref{fig:controller_comparison}~(d) shows that VISTA maintains the target within the FoV, whereas PBVS--PPC and FF--IBVS reach the camera boundary. PBVS--AEDO retains visibility but still fails the soft-precise criterion, showing that visibility preservation alone is not sufficient for successful soft-precise landing.
...
The closed-loop analysis establishes uniform ultimate boundedness of the coupled visual sliding dynamics and image-orientation error, preservation of the prescribed image-position and image-velocity envelopes under the derived coupling condition, and conditional forward invariance of the visibility set under the nominal one-step predictor. In simulation, VISTA achieved soft-precise touchdown in $44$ of $45$ initial-condition, motion-profile, and $\pm40\%$ target-speed-sweep combinations without retuning. None of the four representative baselines achieved soft-precise touchdown across the corresponding comparisons, underscoring the non-triviality of satisfying alignment, touchdown-severity, orientation, and visibility requirements simultaneously. Future work will address hardware validation under sensing/actuation delays, predictor mismatch, and moving-platform uncertainties.
