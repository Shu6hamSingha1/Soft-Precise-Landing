...
This section presents the rigid-body dynamics of the quadrotor, defines the image parameters that drive the proposed MDF-ASMC, and formalizes the soft-precise landing problem in the presence of model uncertainty and exogenous perturbations.
...
\label{quadrotor dynamics: equation}
 ...
 \end{align*}
Here $m$ is the body mass, $J$ is the body inertia matrix, $\,^\mathcal{B} \boldsymbol{F}_u = [0,0,-\,^\mathcal{B} T_u]^\top$ with $\,^\mathcal{B} T_u\ge 0$ the rotor thrust magnitude along $-Z_\text{b}$, $\,^\mathcal{B} \boldsymbol{\tau}_u$ is the control torque, $\,^\mathcal{B} \boldsymbol{F_{\text{g}}}$ is the gravitational force, and $\,^\mathcal{B} \boldsymbol{F_{\text{d}}},\,^\mathcal{B} \boldsymbol{\tau_{\text{d}}}$ are the unknown bounded body-frame disturbance force and torque introduced in Section~\ref{background: section}-\ref{problem statement: section}. The combined thrust-torque actuation is $\boldsymbol{u}=[\,^\mathcal{B}T_u,\,^\mathcal{B}\boldsymbol{\tau}_u^\top]^\top\in\mathbb{R}^4$.

\subsection{Image Parameters} \label{image parameters: section}
In place of the inertial pose of \eqref{quadrotor dynamics: equation}, MDF-ASMC drives the control loop from image parameters computed from the target image as shown in Fig.~\ref{block diagram: figure}. The target is observed by a downward-facing monocular camera whose optical axis is aligned with $Z_\text{b}$. Image parameters are then derived from image feature points identified on the target image. These parameters are expressed in a \emph{virtual camera frame} $\mathcal{V}$, obtained from the camera frame $\mathcal{C}$ by rotating out the UAV roll $\phi$ and pitch $\theta$ so that its image plane is always parallel to the inertial horizontal plane (Fig.~\ref{frames planes: figure}). The pinhole projection on $\mathcal{V}$ and the closed-form virtual-feature map are given in Section~S1-A of the supplement.

\subsubsection{Virtual Image Pose} \label{virtual image pose: section}
Let $\,^\mathcal{C}\boldsymbol{\hat{r}}_i$, $i\in\{1,\dots,N\}$ with $N\ge 3$, denote $N$ distinguishable image feature points on the camera image plane (Fig.~\ref{frames planes: figure}). De-rotated through $\,^\mathcal{V}R_\mathcal{C}$, they re-project as virtual image feature points $\,^\mathcal{V}\boldsymbol{\hat{r}}_i=[\,^\mathcal{V}\hat{x}_i,\,^\mathcal{V}\hat{y}_i]^\top$, and their mean $\,^\mathcal{V}\boldsymbol{\hat{r}}=N^{-1}\sum_{i=1}^{N}\,^\mathcal{V}\boldsymbol{\hat{r}}_i$ is the virtual image point. The homogeneous augmentation yields the \emph{virtual image position}
...
\begin{enumerate}
    \item \emph{Unknown depth scale.} The relative target depth $\,^\mathcal{V}z_\text{t}$ along the optical axis is unobservable from the monocular image alone. Through the pinhole projection it manifests as the unknown time-varying scale $\beta=1/\,^\mathcal{V}z_\text{t}$ inside the input matrix $B_h$ of the optic-flow dynamics \eqref{h dot: equation}, so the controller cannot pre-cancel it.
    \item \emph{Aerodynamic perturbations near touchdown.} The body-frame disturbance force and torque $\,^\mathcal{B}\boldsymbol{F_{\text{d}}},\,^\mathcal{B}\boldsymbol{\tau_{\text{d}}}$ aggregate wind gusts, the aerodynamic ground effect~\cite{sanchez-cuevas2017} that dominates within $\sim$1~m of the landing surface, and residual unmodeled rotor dynamics. Rotated into $\mathcal{V}$, they propagate to the optic-flow disturbance $\boldsymbol{d}_h$ of \eqref{h dot: equation}.
...
Under Assumption~1, the control problem is to design a control law for the combined thrust-torque quadrotor input $\boldsymbol{u}=[\,^\mathcal{B}T_u,\,^\mathcal{B}\boldsymbol{\tau}_u^\top]^\top$ in \eqref{quadrotor dynamics: equation} that regulates the image parameters $\boldsymbol{s}$, $\alpha$, and $\boldsymbol{h}$ and keeps the target visible throughout the descent. The control law developed in Section~\ref{control design: section} achieves the functional requirement of soft-precise landing in closed loop.

\section{MDF-ASMC CONTROL DESIGN} \label{control design: section}
The proposed MDF-ASMC is a \emph{dual-funnel} design (Fig.~\ref{block diagram: figure}). The optic-flow funnel generates the commanded acceleration through a leakage-type ASMC law, and the target image funnel sizes a state-dependent attitude cone that kinematically clips it. Together, the two funnels constitute the outer loop, while the inner loop tracks the commanded attitude constructed from the outer-loop output and the virtual-compass heading.

\begin{figure*}[hbt!]
\centering
    \includegraphics[width=\textwidth]{Figures/block_diagram_v3.pdf}
    \caption{MDF-ASMC pipeline. Image feature points $\,^\mathcal{C}\boldsymbol{\hat{r}}_i$ from the monocular camera feed the Virtual Image Point PID, which produces the desired optic flow $\boldsymbol{h}_\text{d}$. The optic-flow error $\boldsymbol{h}_\text{e}=\boldsymbol{h}-\boldsymbol{h}_\text{d}$ drives the ASMC and yields the inertial commanded acceleration $\,^\mathcal{I}\boldsymbol{a}_\text{d}$. The funnel-margin cone clamp projects $\,^\mathcal{I}\boldsymbol{a}_\text{d}$ to the visibility-safe $\,^\mathcal{I}\tilde{\boldsymbol{a}}_\text{d}$, which together with the desired yaw $\psi_\text{d}$ forms the desired attitude $R_\text{d}$ and thrust $\,^\mathcal{B}T_u$. The SO(3) inner loop (Section~\ref{control design: section}-\ref{inner loop: section}\ref{so3 inner loop: section}) closes the loop by tracking $R_\text{d}$ and producing the body-frame torque $\,^\mathcal{B}\boldsymbol{\tau}_u$.}
    \label{block diagram: figure}
\end{figure*}

\subsection{Outer Loop: Dual Funnel and ASMC} \label{outer loop: section}
The outer loop transforms image-parameter feedback into the commanded acceleration through three sub-blocks: Section~\ref{control design: section}-\ref{outer loop: section}\ref{virtual image point control: section} (Virtual Image Point PID producing $\boldsymbol{h}_\text{d}$), Section~\ref{control design: section}-\ref{outer loop: section}\ref{optic flow control: section} (optic-flow funnel and leakage-type ASMC), and Section~\ref{control design: section}-\ref{outer loop: section}\ref{acceleration conditioning: section} (target image funnel and acceleration conditioning).
...
\end{equation}
where $\oslash$ is componentwise division, $\hat{\boldsymbol{r}}_\text{d}=\boldsymbol{0}$ is the desired centroid on the optical axis, and $|\bar{r}_{\text{e}_k}|=1$ marks the FoV boundary. With desired image position $\boldsymbol{s}_\text{d}=[0,0,1]^\top$ and position error $\boldsymbol{s}_\text{e}=\boldsymbol{s}-\boldsymbol{s}_\text{d}$, $\dot{\boldsymbol{s}}_\text{d}=\boldsymbol{0}$ gives $\dot{\boldsymbol{s}}_\text{e}=\dot{\boldsymbol{s}}$ from~\eqref{s dot: equation}. The \emph{Virtual Image Point PID} specifies the desired $\dot{\boldsymbol{s}}_{\text{e},xy}$
...
Define the optic-flow error $\boldsymbol{h}_\text{e}=\boldsymbol{h}-\boldsymbol{h}_\text{d}$. The cascaded outer-/inner-loop design admits the following standard time-scale-separation hypothesis.
...
Differentiating and using the optic-flow dynamics~\eqref{h dot: equation} under Assumption~2 yields
\begin{equation} \label{h_e_dot_1: equation}
    \dot{\boldsymbol{h}}_\text{e} = \beta\,\boldsymbol{u}_h + \boldsymbol{c}_h + \boldsymbol{d}_h,
\end{equation}
with $\boldsymbol{u}_h=-\,^\mathcal{I}R_\mathcal{V}^\top\,^\mathcal{I}\boldsymbol{a}_\text{d}$ the inertial commanded acceleration expressed in $\mathcal{V}$, $\beta=1/\,^\mathcal{V}z_\text{t}$ the depth scale, and $\boldsymbol{c}_h \triangleq \tilde{\boldsymbol{c}}_h - \dot{\boldsymbol{h}}_\text{d}$ absorbing the desired-flow derivative into the kinematic coupling of \eqref{h dot: equation}. Here $\boldsymbol{c}_h$ is known and available for feedback linearization, while $\beta$ and $\boldsymbol{d}_h$ are unknown and bounded as formalized in Section~\ref{background: section}-\ref{problem statement: section}.

The design follows the prescribed-performance control (PPC) paradigm \cite{lin2022}. A time-decaying performance function bounds the transient error magnitude, and a smooth coordinate change lifts the constrained error onto an unconstrained variable on which a stabilising law can be designed without inequality constraints. The optic-flow funnel constrains $h_{\text{e}_k}\in(-p_{2_k}(t),p_{2_k}(t))$, $k\in\{1,2,3\}$, with performance function $\boldsymbol{p}_2(t) = e^{-\Xi_2 t}(\boldsymbol{p}_{2_0}-\boldsymbol{p}_{2_\infty})+\boldsymbol{p}_{2_\infty}$, where $\boldsymbol{p}_{2_0},\boldsymbol{p}_{2_\infty},\Xi_2=\text{diag}(\xi_{2_k})\succ 0$ set the maximum overshoot, steady-state residual, and minimum convergence rate. A hyperbolic-tangent transformation maps the constrained error onto the unconstrained coordinate $\boldsymbol{\zeta}_2$:
\begin{align}
...
A funnel-saturation guard clamps $|S_2(\zeta_{2_k})|\le 1-\epsilon_S$ to keep $\mathcal{G}_2$ bounded as $|S_2|\to 1$ (Section~S2-A).
...
To ensure the target remains visible during soft-precise landing, the image feature points $\,^\mathcal{C}\boldsymbol{\hat{r}}_i$ are used directly in the cone clamp (Fig.~\ref{block diagram: figure}). The target image funnel and its funnel-margin cone clamp realize a state-dependent attitude cone by projecting the commanded lateral acceleration $\,^\mathcal{I}\boldsymbol{a}_{\text{d},xy}$ of \eqref{h_e_dot_1: equation} onto a cone whose half-angle tracks the instantaneous funnel margin, generalizing the fixed roll/pitch saturation of \cite{xie2016} to a state-dependent constraint. The target image funnel is the rectangular envelope
...
\end{equation}
with $\theta_\text{curr}(t)=\arccos(R_{33}(t))$ the body-$z$ tilt from inertial vertical and $\theta_\text{cap}=60^\circ$ the maximum allowable tilt. The $60^\circ$ cap fixes the vertical thrust component at $\cos\theta_\text{cap}=0.5$, so $\,^\mathcal{B}T_u$ stays within twice the hover thrust. This preserves gravity-compensation margin against actuator saturation. The empirical justification from the speed-envelope sweep is in Section~S3-E of the supplement. At the funnel boundary, $d_\text{min}^\text{fov}\to 0$ forces $\theta_\text{cone}\to\theta_\text{curr}$, so no tilt beyond $\theta_\text{curr}$ can push an image feature point $\,^\mathcal{C}\boldsymbol{\hat{r}}_i$ past $\boldsymbol{p}_1(t)$. The projection is
...
\end{equation}
with $\epsilon_\text{c}>0$ a numerical floor disjoint from the ASMC widths $\varepsilon_k$ of \eqref{adaptive control law: equation}. The image feature points $\,^\mathcal{C}\boldsymbol{\hat{r}}_i$ stay inside $\boldsymbol{p}_1(t)\subseteq\boldsymbol{p}_1(0)\preceq\mathcal{R}/2$, enforcing visibility kinematically for all $t\ge 0$. The cone-clamp projection residual is absorbed into $\boldsymbol{d}_h$ of \eqref{h_e_dot_1: equation}.
...
The inner loop comprises a virtual-compass yaw ASMC (Section~\ref{control design: section}-\ref{inner loop: section}\ref{yaw control: section}) and a geometric SO(3) attitude tracker (Section~\ref{control design: section}-\ref{inner loop: section}\ref{so3 inner loop: section}). The yaw ASMC drives the image-orientation error to a UUB. The SO(3) tracker produces the body-frame torque $\,^\mathcal{B}\boldsymbol{\tau}_u$.
...
\end{equation}
In order to drive $\alpha_\text{e}$ in \eqref{alpha_e_dot: equation} to a UUB without any \emph{a priori} knowledge of the upper bound of $d_{\alpha}$, a leakage-type ASMC law, with sliding surface $\sigma_{\alpha}=\alpha_\text{e}+\chi_{\alpha}\!\int\alpha_\text{e}\,d\tau$, is formulated to produce the desired yaw rate $\dot{\psi}_\text{d}$:
...
\end{equation}
This \emph{virtual-compass integrator} makes $\psi_\text{d}$ evolve purely from the image-based yaw error, enabling magnetometer-free operation.
...
\end{equation}
For any $0<\varphi_2<\varphi_1$ this implies $\dot{V}\le-\varphi_2 V$ whenever $V\ge\Theta\triangleq \tfrac{3\beta_{\min}\lambda_{\max}(\mathcal{P})\tilde{d}^2}{2(\varphi_1-\varphi_2)}$. A standard GUUB lemma (Section~S2-B) drives $V$ into $\{V\le\Theta\}$ in finite time. Using $V\ge\|\boldsymbol{\sigma}\|^2/2$ gives the UUB bound \eqref{UUB bound: equation}, and the inverse PPC transformation \eqref{optical flow error transformation: equation} enforces $-\boldsymbol{p}_2(t)<\boldsymbol{h}_\text{e}<\boldsymbol{p}_2(t)$ for all $t\ge 0$. The line-by-line algebra of \eqref{Vdot bound: equation} is given in Section~S2-B of the supplement. $\hfill\blacksquare$

\textit{Corollary 1 (Closed-loop target visibility).} Under the conditions of Theorem~1, the Virtual Image Point PID \eqref{s_e_dot_d: equation} renders $\hat{\boldsymbol{r}}_\text{e}$ UUB by an affine function of $\vartheta$, and the cone-clamp projection \eqref{cone clamp: equation} enforces the visibility constraint kinematically for all $t\ge 0$. The ISS cascade and the geometric visibility argument are in Section~S2-B.

\textit{Theorem 2 (Adaptive Yaw Ultimate Boundedness).}
Consider the image orientation kinematics \eqref{alpha_e_dot: equation} (linear in $\dot{\psi}_\text{d}$ by \textit{Remark~3}) with initial condition $\alpha_\text{e}(0)\in(-\pi/2,\pi/2]$. Then, under \textit{Assumptions~1--2}, the yaw ASMC law \eqref{yaw control law: equation} exponentially drives $\sigma_\alpha$ into the UUB
...
\end{equation}
For any $0<\varphi_2^\alpha<\varphi_1^\alpha$ this implies $\dot{V}_\alpha\le-\varphi_2^\alpha V_\alpha$ whenever $V_\alpha\ge\Theta_\alpha\triangleq\tfrac{\rho_\alpha\tilde{d}_\alpha^2}{2(\varphi_1^\alpha-\varphi_2^\alpha)}$. The GUUB lemma of Section~S2-B drives $V_\alpha$ into $\{V_\alpha\le\Theta_\alpha\}$ in finite time. Using $V_\alpha(t)\ge\sigma_\alpha^2/2$ gives the UUB bound \eqref{yaw UUB bound: equation}. The sliding-surface definition then renders $\alpha_\text{e}$ UUB by an affine function of $\vartheta_\alpha$, and the virtual-compass integrator \eqref{psi d integrator: equation} yields a bounded $\psi_\text{d}$ for the SO(3) tracker. The detailed steps for \eqref{V alpha dot bound: equation} are given in Section~S2-C of the supplement. $\hfill\blacksquare$
