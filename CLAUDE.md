...
- **Target-visibility (Python, `src/visibility_projection.py`, 2026-09-09):** conditions the SMC's `I_a` command. Tier-1 = one convex QP every cycle (`e63751e2`): minimal-intervention outward-only lean `y` keeping the measured cross-marker centre inside `φ = R/(2f)·(1−CBF_BUFFER_FRAC)` on the *real* camera plane, with the deliverability ball `‖y‖ ≤ √(A_CAP²/a_z² − 1)` (= the `arccos(a_z/A_CAP)` lean cap / thrust sphere) **folded into the QP** so `I_a` is actuator-feasible by construction, plus a penalised slack `CBF_VIS_RHO` for graceful degradation instead of infeasibility. `I_a[2]` is a fixed input. A `τ·d` moving-target lead term (`d` = the front-end's de-rotated optic flow `h_xy`, identity-mapped, conditioned by `condition_drift` — `rel_resid` gate / median-3 / LPF / clamp; `CBF_DRIFT_TAU` default **0.15**, baked 2026-09-09 on CBF-behaviour grounds — `τ=0` and `τ=0.15` both hold the stationary safe set 100% (0 sensor exits; the SP delta was SITL noise), while reactive-only `τ=0` leaves a moving-target gap (278 sensor-exit frames / 24 rover reps); `τ·d` closes it. `=0` restores reactive-only) makes the *same* QP serve stationary and rover with no branching. Tier-2 `descent_ease` (`CBF_DESCENT_EASE=1`) scales only the downward part of `I_a[2]` on a measured time-to-edge, self-releasing. Replaced `cbf_visibility.py` + the joint-QP/deliverability-sphere/`CBF_AZ_COST_GAIN`-relief/two-phase-δ/`rho_fov`-cone machinery (→ `Obsolete/`). Spec: `docs/CBF_visibility.pdf`. The `controller.py` lean/thrust caps remain as redundant guards (cover the raw path + the degenerate `A_CAP ≤ g` case).
