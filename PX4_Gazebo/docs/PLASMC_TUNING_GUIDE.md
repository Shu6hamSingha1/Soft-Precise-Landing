...
- **`gamma_s>1.0` + `KP=12` SWEPT (NC56-60, 2026-06-10, IC1 n=5):** `gamma_s=1.2` → **0 TL** (vs baseline's 2) but **1/5 hover** (over-centers → σ_z weak); `gamma_s≥1.4` degrades (1.4 catastrophic 8.7 m) — ceiling = descent-weakening, not a demand-breach. **`KP=12+E=1.5` → tightest landings of any cell (0.34 m best ever, no t=0 LK collapse) but 1/5 the touchdown lateral breach fires harder (κ_xy=0.85, a_u_xy=521) → next = `KP=12 + κ_xy cap`.** Neither cleanly beats baseline at n=5. The binding TL cause remains **PERCEPTION** (4/6 of TLs = close-range touchdown marker-loss). The **lateral κ-runaway = a touchdown funnel breach** (1/Z, κ_xy UNCAPPED) — UNBOUNDABLE by gain; fix = convergence-ordering (gate descent on `s_e_n`) + κ_xy cap, see [[feedback_lateral_kappa_runaway]]. Ultimate lever: **code-level perception** (pyramidal LK 2→3) + the IC-rig compass-drift fix.
