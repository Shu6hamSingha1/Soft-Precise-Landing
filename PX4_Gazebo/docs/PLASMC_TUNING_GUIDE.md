...
- **🔴 MOVING rover_cross landing does NOT work yet** (2026-09-01/02): 0/5 static, all moving reps
  ...
  - Control-side terminal stall (as diagnosed pre-09-09; the joint-QP half is now retired):
    single-frame centroid spike `s_x` 0.16→0.91→0.16 → `dh_d` ±11 → `theta_desired` 92° → CBF FoV
...
- **✅ CBF 90° convention question — RESOLVED 2026-09-09.** The reverted `Rz_p90b` "fix" and the
  long-open "90° SMC↔CBF convention" doubt are closed by the `visibility_projection` rebuild: the
  correct Jacobian is `Le = −(Lw@M)` (sign NEGATED vs the old `Lw@M`), fixed against an independent
  pinhole+attitude oracle — the old machinery AND the old `validate_cbf.py` oracle shared the wrong
  sign, which is why synthetic checks passed while SITL regressed. Verified in the IC2-5 A/B PASS.
...
- **⭐ (09-09) VISIBILITY CBF → ONE CONVEX QP + moving-target lead** (`e63751e2`, `28e4417b`) — Tier 1
  ...
  **folded in** → `I_a` feasible by construction), slack `s` penalised by **`CBF_VIS_RHO`**=2000
  ...
  entry); **`CBF_DRIFT_TAU` default 0.15** (BAKED 2026-09-09 — see next entry). The SAME QP serves
  stationary and rover — no branching. Solver = projected Newton + 1-D circle refine. `controller.py`
  lean/thrust caps kept as redundant guards. Spec `docs/CBF_visibility.pdf` (rewritten).
- **(09-09) moving-target lead `d` — CONDITIONED** (`e1b094e8`) — raw `h_xy` is unusable as the lead
  ...
  **`CBF_DRIFT_RESID_GATE`** (0.45 = the flow layer's own `rel_resid` gate; untrusted solve → `d=0`)
  ...
  Sweep-trace replay: `|d|max` 16→0.5 every rep, `p50` unchanged. **`CBF_DRIFT_TAU` BAKED 0→0.15**
  (2026-09-09), judged on **CBF behaviour, not SP**: on the IC2-5 stationary confirm gate
  (`test_data/DriftTauConfirm/20260909-200537`) `τ=0` and `τ=0.15` **both held the safe set 100%**
  (0 frames with the centre past the physical FoV edge; maxC/φ 1.02 vs 1.03) — the SP delta there
  (P+S 11 vs 14, an IC4 2.80 m/s TD) was SITL noise (most non-precise reps had `vis_active=0`, CBF
  inert). On a **moving** target `τ=0` is reactive-only and one step behind continuous target motion
  → **278 frames / 24 rover reps where the centre left the sensor** (vs 0 stationary); it corrects
  the marker back AFTER it leaves the FoV, not keeps it in. `τ·d` closes that gap. **Follow-up
  hardening (not a blocker):** terminal-overfill `h_xy` corruption (coherent, `rel_resid`-blind)
  becomes a phantom lead at `τ>0` — fix is an overfill gate on `d` (`d=0` when `MARKER_EXTENT_PX`
  shows the marker fills the frame). `CBF_DRIFT_TAU=0` restores reactive-only. New knobs
  `CBF_VIS_RHO` /
...
```
Bounding rules: **P** bounds κ cleanly (`κ_eq∝1/P`); **E** bounds κ but softens tracking (one-knob-one-job: P for κ, E for stiffness); **N, Γ** can't fix it (math-excluded); **the visibility projection masks it** (if `vis_active(t)` fires in normal ops the control law is failing — on a clean approach it is a pure pass-through); **funnel width = gain** (never widen a funnel for a transient). The **current binding limit is stochastic LK/ArUco perception** (~2 m/s dynamic-range ceiling → 1–2 TARGET_LOST per 5 reps even at a perfect IC) — *not* gain-tunable. Lag (38 ms roll / 287 ms yaw) is the **last** explanation, not the first.
...
- **`docs/CONTROLLER_PARITY.md`** (MATLAB↔Python diff + intentional divergences) · **`docs/CBF_visibility.pdf`** (visibility mechanism, 9-Sep rewrite) · **`docs/FUNNEL_CBF_DESIGN.md`** (SEN_FUNNEL §9; CBF part superseded) · **`docs/PERCEPTION_FLOW_FINDINGS.md`** (perception layer + LK dynamic-range lever) · **`docs/SH_REFERENCE.md`** (bash patterns).
