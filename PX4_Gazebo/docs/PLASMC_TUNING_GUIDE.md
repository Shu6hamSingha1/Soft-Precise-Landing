...
- **Gain-side levers MOSTLY exhausted, but two were NOT swept** (correction 2026-06-10): `gamma_s>1.0` (outer-funnel contraction = lateral-convergence speed) + `KP=12` (re-check at E=1.5 + SEN_FUNNEL; record shows 3.20 m). **Sweeping now (NC56+, IC1).** The binding TL cause remains **PERCEPTION** (6-TL analysis: 4/6 perception — 3 close-range *touchdown* marker-loss @ 1–3 m + 1 far drift; 2/6 control κ-runaway). The **lateral κ-runaway = a touchdown funnel breach** (1/Z, κ_xy UNCAPPED) — UNBOUNDABLE by gain; fix = convergence-ordering (gate descent on `s_e_n`) + κ_xy cap, see [[feedback_lateral_kappa_runaway]]. Ultimate lever: **code-level perception** (pyramidal LK 2→3) + the IC-rig compass-drift fix.
