...
3. Terminal velocity (soft gap) ≈ 38 ms lag → the genuine remaining lever is the uXRCE-DDS rate-path (see [[feedback_dds_lag_fix_blocker]]), not control gains. *(⛔ 2026-07-02: the uXRCE-DDS path was RULED OUT by user — lag stands as a design constant; and the Z_REG artifact later re-scoped the terminal-velocity story anyway.)*
