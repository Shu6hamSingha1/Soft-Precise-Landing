...
class Controller(Thread):
    def __init__(self, ref_rad_opt_flow, des_img_feature, time_keeper=time, controller=None, record='n',
        ...
        # Dead-zone on the κ-ODE GROWTH term |σ| (per-axis; 0 = OFF, bit-identical).
        #   dκ_i/dt = θ_i·N_i·G_i·max(|σ_i|−dz_i, 0) − N_i·P_i·κ_i
        # The growth term uses raw rectified |σ|, so measurement noise n on σ=h−h_d gives
        # E[|σ|] ≈ 0.8·σ_noise > 0 even at zero true error → a standing positive forcing on
        # κ that the leakage can only DRAIN (proportional), settling at a floor
        # κ_floor ≈ θ·G·0.8·σ_noise / P rather than zero. Under GT-FB σ_noise≈0.002 so κ
        # decays; under live perception it ramps to ~0.04–0.09 xy through the descent
        # (~1.6× that with AU_LEAD's HF gain) → κ ratchets terminally. A dead-zone sized to
        # the measured noise band zeroes the growth term whenever the true error is inside
        # the noise, killing the floor without touching real-disturbance response
        # (dz ≪ any tracking error that matters). Characterisation:
        # feedback_adaptive_law_noise_behavior. z kept 0 (κ_z is terminal-braking, rate-
        # limited, and its |σ| is loom not lateral-flow noise — don't starve it).
        self._kappa_dz = pa("KAPPA_DZ", 0.0, 0.0, 0.0)
    ...
    def _kappaSolver(self, _, kappa, X):
        ...
        # Dead-zone on the growth term (PLASMC_KAPPA_DZ_*, per-axis; default 0 -> exactly
        # np.abs(sigma), bit-identical). Rejects rectified-noise wind-up: see __init__.
        abs_sigma = np.maximum(np.abs(sigma) - self._kappa_dz, 0.0)
        return theta * (self._N @ self._G[-1] @ abs_sigma) - self._N @ self._P @ kappa
