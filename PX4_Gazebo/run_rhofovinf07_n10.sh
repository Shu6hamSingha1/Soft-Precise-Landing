#!/usr/bin/env bash
# Interpolation point: RHOFOVINF × 0.7 (terminal pixel envelope [80,80] → [56,56]).
# ×0.5 gave great softness (vh_end 0.80) but missed precision (xy_min 0.110).
# ×0.7 is the midpoint test — does it keep some softness benefit while
# recovering precision?
set -u
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TIMESTAMP="$(date +%Y%m%d-%H%M%S)"
BUNDLE_DIR="$HOME/ws/Test_Data/RhoFovInf07/${TIMESTAMP}"; mkdir -p "$BUNDLE_DIR"
IC="0.0,0.0,5.0"; N_REPS="${N_REPS:-10}"

export LANDING_REF_RAD_OPT_FLOW=-0.70
export IMG_FILTER_WIN=7
export PLASMC_RHOFOVINF_SCALE=0.7
...
for r in $(seq 1 "$N_REPS"); do
  ...
  echo "=== RhoFovInf×0.7  rep=$r/$N_REPS ==="
...
print(f"RHOFOVINF × 0.7  (n={len(rows)})")
