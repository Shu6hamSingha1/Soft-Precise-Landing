#!/usr/bin/env bash
# dh_d/dw KF no-regression IC2-5 gate (2026-06-20). KF is now DEFAULT-ON; confirm it
# doesn't regress vs KF-off. Arms per IC: kfon (default) vs kfoff (explicit off).
set -u
...
run_cell () {
  ...
  local base="$PROJ/test_data/DwKF_${ic}_${arm}"; mkdir -p "$base"
  ...
  echo "=== [$ic/$arm] DONE: $valid valid in $launch ==="
}
for ic in IC2 IC5 IC3 IC4; do
  run_cell "$ic" kfon
  run_cell "$ic" kfoff   env PLASMC_DHD_KF=0 PLASMC_DW_KF=0
done
echo "ALL CELLS DONE"
