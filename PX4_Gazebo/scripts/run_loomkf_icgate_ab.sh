#!/usr/bin/env bash
# Loom + derivative-KF IC2-5 GATE A/B (2026-06-20). On the NEW combined-barrier default.
# Tests whether FLOW_LOOM_DECOUPLE + PLASMC_DHD_KF + PLASMC_DW_KF help on top of the baked
# combined surface, gating them for bake. Arms per IC: base (combined only) vs loomkf.
set -u
...
run_cell () {
  ...
  local base="$PROJ/test_data/LoomKF_${ic}_${arm}"; mkdir -p "$base"
...
for ic in IC2 IC5 IC3 IC4; do
  run_cell "$ic" base
  run_cell "$ic" loomkf  env FLOW_LOOM_DECOUPLE=1 PLASMC_DHD_KF=1 PLASMC_DW_KF=1
done
echo "ALL CELLS DONE"
