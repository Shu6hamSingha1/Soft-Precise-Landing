#!/usr/bin/env bash
# Yaw e_a convergence sweep (manuscript-combined, IC2 n=5): Omega_a (integral, null steady
# offset) + Gamma_a (reaching). Measures terminal e_a mean/std.
set -u
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"; PROJ="$(cd "$SCRIPT_DIR/.." && pwd)"
N="${N:-5}"; MAXLAUNCH="${MAXLAUNCH:-12}"
run_arm () {
  local name="$1"; shift; local base="$PROJ/test_data/YawConv_IC2_${name}"; mkdir -p "$base"
...
run_arm base
run_arm oa10   env PLASMC_YAW_OMEGA=1.0
run_arm ga10   env PLASMC_YAW_GAMMA=1.0
echo "ALL ARMS DONE"
