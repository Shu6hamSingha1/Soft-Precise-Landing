#!/usr/bin/env bash
# D1 rotz(yaw) a_u-transform IC1 A/B (2026-06-19). N valid reps per arm at IC1=(0,0,5),
...
run_arm () {
  local name="$1"; shift
  local base="$PROJ/test_data/Rotz_IC1_${name}"
  mkdir -p "$base"
...
run_arm baseline
run_arm rotz     env PLASMC_AU_ROTZ_ONLY=1
echo "ALL ARMS DONE"
