...
cleanup() {
  ...
  pkill -9 -f 'parameter_bridge.*world/cross_marker' 2>/dev/null || true
...
WAITED=0
while ! gz topic -l 2>/dev/null | grep -q '/world/cross_marker/clock'; do
  sleep 1
  echo -n "."
  WAITED=$((WAITED + 1))
  if [ "$WAITED" -gt 60 ]; then
    echo " timed out!"
    echo "[run] PX4 SITL did not bring up Gazebo's /world/cross_marker/clock in 60s."
...
start_bg bridge_clock ros2 run ros_gz_bridge parameter_bridge \
  /world/cross_marker/clock@rosgraph_msgs/msg/Clock@gz.msgs.Clock \
  --ros-args -r /world/cross_marker/clock:=/clock

start_bg bridge_pose ros2 run ros_gz_bridge parameter_bridge \
  /world/cross_marker/pose/info@geometry_msgs/msg/PoseArray@gz.msgs.Pose_V \
  --ros-args -r /world/cross_marker/pose/info:=/pose

start_bg bridge_image ros2 run ros_gz_bridge parameter_bridge \
  /world/cross_marker/model/x500_mono_cam_down_0/link/camera_link/sensor/imager/image@sensor_msgs/msg/Image@gz.msgs.Image \
  --ros-args -r /world/cross_marker/model/x500_mono_cam_down_0/link/camera_link/sensor/imager/image:=/image
...
echo
echo "[run] All background processes up. Starting cross_marker_altitude_test.py..."
...
PY_SCRIPT="${PY_SCRIPT:-apps/cross_marker_altitude_test.py}"
