#!/usr/bin/env bash
# Terminal ring-commit A/B: PLASMC_TERMINAL_RING_COMMIT=1 + DESCENT_GATE=1, IC2 perception x5.
SD=/home/shubham/Soft-Precise-Landing/PX4_Gazebo; cd "$SD"
RES="$SD/run_logs/ringcommit.tsv"; printf "rep\txy_err\tclass\tpeak\tRINGCOMMIT\trec\n" > "$RES"
...
for rep in 1 2 3 4 5; do
  ko
  PLASMC_TERMINAL_RING_COMMIT=1 PLASMC_DESCENT_GATE=1 INITIAL_DRONE_ENU="2.0,2.0,5.0" LANDING_AUTOSAVE=1 MAX_ATTEMPTS=5 HEADLESS=1 timeout 235 bash scripts/run_aruco_landing_retry.sh > "$SD/run_logs/ringcommit_$rep.log" 2>&1
  xy=$(grep -aoE 'xy_err=[0-9.]+' "$SD/run_logs/ringcommit_$rep.log"|tail -1|cut -d= -f2)
  cls=$(grep -aoE 'Landing classification: [A-Z_+]+' "$SD/run_logs/ringcommit_$rep.log"|tail -1|awk '{print $3}')
  rc=$(grep -ac 'RING-COMMIT' "$SD/run_logs/ringcommit_$rep.log")
...
PY
  ...
  printf "%s\t%s\t%s\t%s\t%s\t%s\n" "$rep" "${xy:-NA}" "${cls:-NA}" "$pk" "${rc}fire" "$(basename "$L")" >> "$RES"
done
ko; echo "RINGCOMMIT_DONE" >> "$RES"
