#!/usr/bin/env bash
# Catch a death-regime runaway: IC2 perception x8, per-frame lk_disp debug. Keep logs+recs.
SD=/home/shubham/Soft-Precise-Landing/PX4_Gazebo; cd "$SD"
RES="$SD/run_logs/deathreg.tsv"; printf "rep\txy_err\tclass\tpeak\trec\n" > "$RES"
...
for rep in 1 2 3 4 5 6 7 8; do
  ko
  DECODE_DBG=1 INITIAL_DRONE_ENU="2.0,2.0,5.0" LANDING_AUTOSAVE=1 MAX_ATTEMPTS=5 HEADLESS=1 timeout 235 bash scripts/run_aruco_landing_retry.sh > "$SD/run_logs/deathreg_$rep.log" 2>&1
  cls=$(grep -aoE 'Landing classification: [A-Z_+]+' "$SD/run_logs/deathreg_$rep.log"|tail -1|awk '{print $3}')
  xy=$(grep -aoE 'xy_err=[0-9.]+' "$SD/run_logs/deathreg_$rep.log"|tail -1|cut -d= -f2)
...
PY
  ...
  printf "%s\t%s\t%s\t%s\t%s\n" "$rep" "${xy:-NA}" "${cls:-NA}" "$pk" "$(basename "$L")" >> "$RES"
done
ko; echo "DEATHREG_DONE" >> "$RES"
