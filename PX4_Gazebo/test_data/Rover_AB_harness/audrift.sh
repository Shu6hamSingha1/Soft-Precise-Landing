#!/usr/bin/env bash
# A_U DRIFT-TERM investigation: baseline (no dense-recover) + AU_DECOMP_DBG=1, IC2 x5.
# Looking for the "persistent one-directional a_u_y, small zeta, decaying kappa" signature.
SD=/home/shubham/Soft-Precise-Landing/PX4_Gazebo; cd "$SD"
RES="$SD/run_logs/audrift.tsv"; printf "rep\ttag\tclassif_relvel\tminalt_xy\tmin_alt\tpeak\n" > "$RES"
...
for rep in 1 2 3 4 5; do
  ko
  AU_DECOMP_DBG=1 PLASMC_DESCENT_GATE=1 INITIAL_DRONE_ENU="2.0,2.0,5.0" LANDING_AUTOSAVE=1 MAX_ATTEMPTS=5 HEADLESS=1 timeout 235 bash scripts/run_aruco_landing_retry.sh > "$SD/run_logs/audrift_$rep.log" 2>&1
  tag=$(grep -aoE 'classification: [A-Z_+-]+' "$SD/run_logs/audrift_$rep.log"|tail -1|awk '{print $2}')
  crv=$(grep -aoE 'rel_vel=[0-9.]+ m/s \[' "$SD/run_logs/audrift_$rep.log"|tail -1|grep -oE '[0-9.]+'|head -1)
  hon=$(grep -a 'Honest precision @ min-alt' "$SD/run_logs/audrift_$rep.log"|tail -1)
