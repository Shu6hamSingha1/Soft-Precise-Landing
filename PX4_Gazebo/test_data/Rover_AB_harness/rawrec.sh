#!/usr/bin/env bash
# Record ONE landing with raw per-frame PNG dump to visually validate deck overflow.
...
IMG_RECORD_RAW=1 INITIAL_DRONE_ENU="2.0,2.0,5.0" LANDING_AUTOSAVE=1 MAX_ATTEMPTS=5 HEADLESS=1 \
  timeout 235 bash scripts/run_aruco_landing_retry.sh > "$SD/run_logs/rawrec.log" 2>&1
echo "RAWREC_DONE $(ls -td $SD/test_data/Test_Videos/*_raw/ 2>/dev/null | head -1)" >> "$SD/run_logs/rawrec.tsv"
