#!/usr/bin/env bash
# PERCEPTION-1-CHANNEL A/B: isolate whether s (position) or h (flow) drives the terminal bounce.
# Run A: only s from perception, h/hz/yaw/wz from GT. Run B: only h from perception, s/hz/yaw/wz from GT.
SD=/home/shubham/Soft-Precise-Landing/PX4_Gazebo; cd "$SD"
RES="$SD/run_logs/pab.tsv"; printf "run\trep\ttag\tclassif_relvel\tminalt_xy\tmin_alt\tpeak\n" > "$RES"
...
run_batch(){
  ...
  for rep in 1 2 3; do
    ko
    PLASMC_GT_FEEDBACK=1 GT_ABLATE="$ablate" PLASMC_DESCENT_GATE=1 INITIAL_DRONE_ENU="2.0,2.0,5.0" LANDING_AUTOSAVE=1 MAX_ATTEMPTS=5 HEADLESS=1 timeout 235 bash scripts/run_aruco_landing_retry.sh > "$SD/run_logs/pab_${name}_$rep.log" 2>&1
    tag=$(grep -aoE 'classification: [A-Z_+-]+' "$SD/run_logs/pab_${name}_$rep.log"|tail -1|awk '{print $2}')
    crv=$(grep -aoE 'rel_vel=[0-9.]+ m/s \[' "$SD/run_logs/pab_${name}_$rep.log"|tail -1|grep -oE '[0-9.]+'|head -1)
    hon=$(grep -a 'Honest precision @ min-alt' "$SD/run_logs/pab_${name}_$rep.log"|tail -1)
...
run_batch "s-percep" "h,hz,yaw,wz"
run_batch "h-percep" "s,hz,yaw,wz"
ko; echo "PAB_DONE" >> "$RES"
