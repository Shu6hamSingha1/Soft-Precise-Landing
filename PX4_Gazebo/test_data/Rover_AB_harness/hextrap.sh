#!/usr/bin/env bash
# H-EXTRAPOLATION validation: PLASMC_H_EXTRAP=1 (decayed, real-only-fit) vs the default hard-zero.
# baseline (h_extrap OFF, current default) x5, then h_extrap ON x5, IC2 + DESCENT_GATE.
SD=/home/shubham/Soft-Precise-Landing/PX4_Gazebo; cd "$SD"
RES="$SD/run_logs/hextrap.tsv"; printf "run\trep\ttag\tclassif_relvel\tminalt_xy\tmin_alt\tpeak\n" > "$RES"
...
run_batch(){
  ...
  for rep in 1 2 3 4 5; do
    ko
    env "$@" PLASMC_DESCENT_GATE=1 INITIAL_DRONE_ENU="2.0,2.0,5.0" LANDING_AUTOSAVE=1 MAX_ATTEMPTS=5 HEADLESS=1 timeout 235 bash scripts/run_aruco_landing_retry.sh > "$SD/run_logs/hextrap_${name}_$rep.log" 2>&1
    tag=$(grep -aoE 'classification: [A-Z_+-]+' "$SD/run_logs/hextrap_${name}_$rep.log"|tail -1|awk '{print $2}')
    crv=$(grep -aoE 'rel_vel=[0-9.]+ m/s \[' "$SD/run_logs/hextrap_${name}_$rep.log"|tail -1|grep -oE '[0-9.]+'|head -1)
    hon=$(grep -a 'Honest precision @ min-alt' "$SD/run_logs/hextrap_${name}_$rep.log"|tail -1)
...
run_batch "baseline"
run_batch "hextrap" PLASMC_H_EXTRAP=1
ko; echo "HEXTRAP_DONE" >> "$RES"
