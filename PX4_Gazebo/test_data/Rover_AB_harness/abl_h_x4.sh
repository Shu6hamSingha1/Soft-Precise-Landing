#!/usr/bin/env bash
# GT_ABLATE=h confirmation: 4 reps (GT lateral flow, perception rest, ds/dh gates off).
SD=/home/shubham/Soft-Precise-Landing/PX4_Gazebo; cd "$SD"
RES="$SD/run_logs/abl_h_x4.tsv"
...
for rep in 1 2 3 4; do
  ...
  INITIAL_DRONE_ENU="0.0,0.0,5.0" LANDING_AUTOSAVE=1 MAX_ATTEMPTS=5 \
    PLASMC_GT_FEEDBACK=1 GT_ABLATE=h FLOW_DH_MAX=0 FLOW_DS_MAX=0 HEADLESS=1 \
    timeout 235 bash scripts/run_aruco_landing_retry.sh > "$SD/run_logs/abl_h_$rep.log" 2>&1
  xy=$(grep -aoE 'xy_err=[0-9.]+' "$SD/run_logs/abl_h_$rep.log" | tail -1 | cut -d= -f2)
  rv=$(grep -aoE 'rel_vel=[0-9.]+' "$SD/run_logs/abl_h_$rep.log" | tail -1 | cut -d= -f2)
  cls=$(grep -aoE 'Landing classification: [A-Z_+]+' "$SD/run_logs/abl_h_$rep.log" | tail -1 | awk '{print $3}')
