...
OUTBASE="$SD/test_data/Rover_Turning/aulead_ratio"
...
val=0.9_3.5
wz="${val%_*}"; wp="${val#*_}"
for RATIO in ${RATIOS:-0.5 1.0}; do
  ...
  for rep in $(seq 1 "$N"); do
    ...
    env HEADLESS=1 PLASMC_GT_FEEDBACK=1 ROVER_MOTION=1 ROVER_TRAJ=Circular ROVER_SPEED_MULT=1.0 \
        ...
        PLASMC_AU_LEAD=1 PLASMC_AU_LEAD_WZ="$wz" PLASMC_AU_LEAD_WP="$wp" PLASMC_AU_LEAD_RATIO="$RATIO" \
