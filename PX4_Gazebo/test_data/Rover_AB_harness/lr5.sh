#!/usr/bin/env bash
# VERIFY fake-zero fix: same config as lr4 (arm-mask+paired+moment), IC2 x5, debug on.
SD=/home/shubham/Soft-Precise-Landing/PX4_Gazebo; cd "$SD"
RES="$SD/run_logs/lr5.tsv"; printf "rep\ttag\tclassif_relvel\tminalt_xy\tmin_alt\tpeak\tlr_fires\n" > "$RES"
...
for rep in 1 2 3 4 5; do
  ko
  PLASMC_LOOM_RING_ON_LOSS=1 PLASMC_RING_ARM_MASK=1 PLASMC_RING_PAIRED=1 RING_LOOM_SOURCE=moment PLASMC_DESCENT_GATE=1 LOOM_RING_DBG=1 INITIAL_DRONE_ENU="2.0,2.0,5.0" LANDING_AUTOSAVE=1 MAX_ATTEMPTS=5 HEADLESS=1 timeout 235 bash scripts/run_aruco_landing_retry.sh > "$SD/run_logs/lr5_$rep.log" 2>&1
  tag=$(grep -aoE 'classification: [A-Z_+-]+' "$SD/run_logs/lr5_$rep.log"|tail -1|awk '{print $2}')
  crv=$(grep -aoE 'rel_vel=[0-9.]+ m/s \[' "$SD/run_logs/lr5_$rep.log"|tail -1|grep -oE '[0-9.]+'|head -1)
  hon=$(grep -a 'Honest precision @ min-alt' "$SD/run_logs/lr5_$rep.log"|tail -1)
  mxy=$(echo "$hon"|grep -oE 'xy=[0-9.-]+'|head -1|cut -d= -f2); malt=$(echo "$hon"|grep -oE 'min_alt=[0-9.-]+'|head -1|cut -d= -f2)
  nf=$(grep -ac '\[lr\]' "$SD/run_logs/lr5_$rep.log")
