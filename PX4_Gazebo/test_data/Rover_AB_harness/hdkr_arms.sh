...
for val in 0; do
  out="$LOG/hdkr_$val"; mkdir -p "$out"
  for rep in $(seq 1 "$N"); do
    stray_clean
    echo "----- hdkr=$val rep $rep/$N -----"
    env HEADLESS=1 PLASMC_GT_FEEDBACK=1 ROVER_MOTION=1 ROVER_TRAJ=Circular ROVER_SPEED_MULT=1.0 \
        ...
        bash "$SD/scripts/run_rover_landing_retry.sh" > "$LOG/hdkr_${val}_${rep}.out" 2>&1
    rm -f "$SD"/run_logs/px4_*.log 2>/dev/null
    d=$(ls -dt "$out"/*/ 2>/dev/null | head -1)
    [ -z "$d" ] && { echo "[RESULT] hdkr=$val rep$rep: NODATA"; continue; }
...
try:
    ...
    print(f"[RESULT] hdkr={val} rep{rep}: {v} lat={lat[imin]:.3f} min_alt={uz[imin]:.2f} "
          f"| e_mean={emean:.2f} e_rot={erot:+.2f} osc_std={oscstd:.3f} peak={uz.max():.1f}")
except Exception as e:
    print(f"[RESULT] hdkr={val} rep{rep}: ERR {e}")
PY
  done
done
echo "P2INF DONE"
