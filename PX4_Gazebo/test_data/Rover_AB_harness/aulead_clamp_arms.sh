...
OUTBASE="$SD/test_data/Rover_Turning/aulead_clamp"
...
for val in 0.9_3.5; do
  wz="${val%_*}"; wp="${val#*_}"
  out="$OUTBASE/lead_$val"; mkdir -p "$out"
  for rep in $(seq 1 "$N"); do
    ...
    env HEADLESS=1 PLASMC_GT_FEEDBACK=1 ROVER_MOTION=1 ROVER_TRAJ=Circular ROVER_SPEED_MULT=1.0 \
        ...
        PLASMC_AU_LEAD=1 PLASMC_AU_LEAD_WZ="$wz" PLASMC_AU_LEAD_WP="$wp" PLASMC_AU_LEAD_MAX=4.0 \
        PY_TIMEOUT_S=180 MAX_ATTEMPTS=5 LANDING_AUTOSAVE=1 LANDING_OUT_BASE="$out" \
        bash "$SD/scripts/run_rover_landing_retry.sh" > "$LOG/aulead_${val}_${rep}.out" 2>&1
    rm -f "$SD"/run_logs/px4_*.log 2>/dev/null
    d=$(ls -dt "$out"/*/ 2>/dev/null | head -1)
    [ -z "$d" ] && { echo "[RESULT] lead=$val rep$rep: NODATA"; continue; }
    grep -m1 "PLASMC_AU_LEAD" "$LOG/aulead_${val}_${rep}.out" || echo "[WARN] lead=$val rep$rep: AU_LEAD banner MISSING"
