#!/usr/bin/env bash
# Omega_d=[0;0;u_a] inner-loop yaw-rate FF (PLASMC_YAW_OMEGA_D_FF=1) arms:
#   spinff: pure spin 0.48, rover parked, yaw SMC ACTIVE + FF  (acceptance: e_a converges)
#   circff: Circular r=0.8 moving rover, yaw SMC ACTIVE + FF
set -u
...
run_arm() {
  local arm="$1"; shift
  local out="$LOG/ffarm_$arm"; mkdir -p "$out"
  for rep in $(seq 1 "$N"); do
    stray_clean
    echo "----- $arm rep $rep/$N -----"
    env HEADLESS=1 PLASMC_GT_FEEDBACK=1 PLASMC_YAW_ALPHA_FILT=0 PLASMC_YAW_OMEGA_D_FF=1 \
        PY_TIMEOUT_S=180 MAX_ATTEMPTS=5 LANDING_AUTOSAVE=1 LANDING_OUT_BASE="$out" "$@" \
        bash "$SD/scripts/run_rover_landing_retry.sh" > "$LOG/ffarm_${arm}_${rep}.out" 2>&1
...
try:
    ...
    print(f"[RESULT] {arm} rep{rep}: {v} min_alt={uz[imin]:.2f} lat={lat[imin]:.3f} peak={uz.max():.1f} "
          f"| e_a max|{np.abs(ea).max():.0f}| 2ndhalf-mean|{np.abs(ea[k:]).mean():.0f}| end {ea[-1]:+.0f} | ua_max {np.abs(ua).max():.2f}")
...
# acceptance: pure spin, yaw active + FF
run_arm spinff ROVER_MOTION=0 PLASMC_GT_SPIN_WZ=0.48
# then the real thing: Circular moving rover + FF
run_arm circff ROVER_MOTION=1 ROVER_TRAJ=Circular ROVER_SPEED_MULT=1.0
echo "FF ARMS DONE"
