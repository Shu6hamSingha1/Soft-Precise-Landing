#!/usr/bin/env bash
# HEADING-HOLD arm at n reps: Circular r=0.8 wz=0.48, GT-FB, SIGN=+1 (default),
# u_a frozen to 0 via YAW_GAMMA=0 YAW_KAPPA0=0 YAW_OMEGA=0 YAW_N=0 (all four —
# the kappa_a ODE adapts via n_a). Verdict per rep incl. u_a max (must be 0).
set -u
...
for rep in $(seq 1 "$N"); do
  stray_clean
  echo "----- hold rep $rep/$N -----"
  HEADLESS=1 PLASMC_GT_FEEDBACK=1 ROVER_MOTION=1 ROVER_TRAJ=Circular ROVER_SPEED_MULT=1.0 \
    ...
    bash "$SD/scripts/run_rover_landing_retry.sh" > "$LOG/hold_${rep}.out" 2>&1
  rc=$?
  rm -f "$SD"/run_logs/px4_*.log 2>/dev/null
  d=$(ls -dt "$out"/*/ 2>/dev/null | head -1)
  if [ -z "$d" ]; then echo "[RESULT] hold rep$rep: NODATA rc=$rc"; continue; fi
...
try:
    ...
    v = 'ON-PLATFORM' if ok else ('NEAR-MISS' if near else 'MISS')
    print(f"[RESULT] hold rep{rep}: {v} min_alt={uz[imin]:.2f} lat={lat[imin]:.3f} "
          f"peak={uz.max():.1f} ua_max={np.abs(ua).max():.4f}")
except Exception as e:
    print(f"[RESULT] hold rep{rep}: ERR {e}")
PY
done
echo "HOLD ARM DONE"
