#!/usr/bin/env bash
# FLOW_DECODE_CORR test: decode-correspondence flow fallback. IC1 x5.
SD=/home/shubham/Soft-Precise-Landing/PX4_Gazebo; cd "$SD"
RES="$SD/run_logs/decode_flow.tsv"; printf "rep\txy_err\tclass\tpeak\trec\n" > "$RES"
...
for rep in 1 2 3 4 5; do
  ko
  FLOW_DECODE_CORR=1 INITIAL_DRONE_ENU="0.0,0.0,5.0" LANDING_AUTOSAVE=1 MAX_ATTEMPTS=5 HEADLESS=1 timeout 235 bash scripts/run_aruco_landing_retry.sh > "$SD/run_logs/decode_flow_$rep.log" 2>&1
  xy=$(grep -aoE 'xy_err=[0-9.]+' "$SD/run_logs/decode_flow_$rep.log"|tail -1|cut -d= -f2)
  cls=$(grep -aoE 'Landing classification: [A-Z_+]+' "$SD/run_logs/decode_flow_$rep.log"|tail -1|awk '{print $3}')
