...
for val in 5.0; do
  out="$LOG/pxy_$val"; mkdir -p "$out"
  for rep in $(seq 1 "$N"); do
    stray_clean
    echo "----- pxy=$val rep $rep/$N -----"
    env HEADLESS=1 PLASMC_GT_FEEDBACK=1 ROVER_MOTION=1 ROVER_TRAJ=Circular ROVER_SPEED_MULT=1.0 \
        ...
        PLASMC_P_X="$val" PLASMC_P_Y="$val" PLASMC_P_Z=5.0 \
        PY_TIMEOUT_S=180 MAX_ATTEMPTS=5 LANDING_AUTOSAVE=1 LANDING_OUT_BASE="$out" \
        bash "$SD/scripts/run_rover_landing_retry.sh" > "$LOG/pxy_${val}_${rep}.out" 2>&1
    rm -f "$SD"/run_logs/px4_*.log 2>/dev/null
    d=$(ls -dt "$out"/*/ 2>/dev/null | head -1)
    [ -z "$d" ] && { echo "[RESULT] pxy=$val rep$rep: NODATA"; continue; }
...
try:
    ...
    print(f"[RESULT] pxy={val} rep{rep}: {v} lat={lat[imin]:.3f} min_alt={uz[imin]:.2f} "
          f"| e_mean={emean:.2f} e_rot={erot:+.2f} osc_std={oscstd:.3f} peak={uz.max():.1f}")
except Exception as e:
    print(f"[RESULT] pxy={val} rep{rep}: ERR {e}")
PY
  done
done
echo "P2INF DONE"
