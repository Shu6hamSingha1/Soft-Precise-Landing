#!/usr/bin/env bash
# FULL-PERCEPTION baseline (no GT anywhere), corrected checkposts default-on. IC1 x6.
SD=/home/shubham/Soft-Precise-Landing/PX4_Gazebo; cd "$SD"
RES="$SD/run_logs/perc_base.tsv"; printf "rep\txy_err\tclass\tpeak_lat\trec\n" > "$RES"
...
for rep in 1 2 3 4 5 6; do
  ko
  INITIAL_DRONE_ENU="0.0,0.0,5.0" LANDING_AUTOSAVE=1 MAX_ATTEMPTS=5 HEADLESS=1 timeout 235 bash scripts/run_aruco_landing_retry.sh > "$SD/run_logs/perc_base_$rep.log" 2>&1
  xy=$(grep -aoE 'xy_err=[0-9.]+' "$SD/run_logs/perc_base_$rep.log"|tail -1|cut -d= -f2)
  cls=$(grep -aoE 'Landing classification: [A-Z_+]+' "$SD/run_logs/perc_base_$rep.log"|tail -1|awk '{print $3}')
