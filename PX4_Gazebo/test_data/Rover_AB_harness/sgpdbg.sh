#!/usr/bin/env bash
# SAVGOL-PREDICT spike-reconstruction investigation: same config as densedbg2 (dense-recover +
# h-extrap, both now default-relevant), SAVGOL_PREDICT_DBG=1 + AU_DECOMP_DBG=1, IC2 x5.
SD=/home/shubham/Soft-Precise-Landing/PX4_Gazebo; cd "$SD"
RES="$SD/run_logs/sgpdbg.tsv"; printf "rep\ttag\tclassif_relvel\tminalt_xy\tmin_alt\tpeak\n" > "$RES"
...
for rep in 1 2 3 4 5; do
  ko
  PLASMC_DENSE_RECOVER=1 SAVGOL_PREDICT_DBG=1 AU_DECOMP_DBG=1 PLASMC_DESCENT_GATE=1 INITIAL_DRONE_ENU="2.0,2.0,5.0" LANDING_AUTOSAVE=1 MAX_ATTEMPTS=5 HEADLESS=1 timeout 235 bash scripts/run_aruco_landing_retry.sh > "$SD/run_logs/sgpdbg_$rep.log" 2>&1
  tag=$(grep -aoE 'classification: [A-Z_+-]+' "$SD/run_logs/sgpdbg_$rep.log"|tail -1|awk '{print $2}')
  crv=$(grep -aoE 'rel_vel=[0-9.]+ m/s \[' "$SD/run_logs/sgpdbg_$rep.log"|tail -1|grep -oE '[0-9.]+'|head -1)
  hon=$(grep -a 'Honest precision @ min-alt' "$SD/run_logs/sgpdbg_$rep.log"|tail -1)
...
PY
  ...
  printf "%s\t%s\t%s\t%s\t%s\t%s\n" "$rep" "${tag:-?}" "${crv:-?}" "${mxy:-?}" "${malt:-?}" "$pk" >> "$RES"
done
ko; echo "SGPDBG_DONE" >> "$RES"
