#!/usr/bin/env bash
# SITL must-not-regress gate for the _best_pair support-priority fix (landed
# 899d8d26, UNCONDITIONAL -- not a flag) on flat cross_marker, PERCEPTION mode.
# Offline (RobustnessFrameset): fixes 12.5% of ALL frames' pair selection (the
# marker's one-sided stub was getting matched into the main arm pair), zero
# regressions across 7 scenes, centroid position accuracy flat (the stub also
# passes through true center), heading/alpha availability +11-13pts.
# This matters MORE to validate live than ens_ring's gate did: unlike ens_ring
# (opt-in flag), this fix is now ON BY DEFAULT on every flight. Uses the
# CROSS_LEGACY_PAIR_SELECT=1 rollback switch (added alongside the fix) to A/B
# against the exact pre-fix behavior.
# Arms INTERLEAVED per rep so SITL temporal drift is shared, not confounded.
# Shape copied from EnsRing_AB_harness/ensring_ab.sh (docs/SH_REFERENCE.md §10).
#   IC=2.0,2.0,5.0 N=5 bash test_data/PairFix_AB_harness/pairfix_ab.sh
...
# ⛔⛔ PER-REP FOREIGN-SESSION CHECK -- see EnsRing_AB_harness/ensring_ab.sh or
# CmInv_AB_harness/cminv_ab.sh for the full incident/rationale. Checked ONCE
# before the loop is not enough; ko() re-verifies after every cleanup and aborts
# (rather than re-killing) if SITL survives it -- that can only mean something
# else is live.
if [ "${FORCE:-0}" != "1" ]; then
...
RES="$SD/run_logs/pairfix_${TAG}.tsv"
printf "arm\trep\txy_err\tclass\tmin_h\tdetOK\trec\n" > "$RES"
...
for rep in $(seq 1 "$N"); do
  for arm in fixed legacy; do
    LP=0; [ "$arm" = legacy ] && LP=1
    ko
    ...
    env LD_PRELOAD="${QT_PRELOAD:-/home/shubham/cvenv/lib/python3.8/site-packages/PyQt5/Qt5/lib/libQt5Gui.so.5}" \
        ...
        INITIAL_DRONE_ENU="$IC" CROSS_LEGACY_PAIR_SELECT="$LP" \
        timeout 260 bash "$SD/$LAUNCH" \
        > "$SD/run_logs/pairfix_${TAG}_${arm}_${rep}.out" 2>&1
    LG="$SD/run_logs/pairfix_${TAG}_${arm}_${rep}.out"
    ...
    if [ "$L" = "$L0" ]; then rec="NO_NEW_REC"; mh="NA"; dk="NA"; else
      rec="$(basename "$L")"
      read -r mh dk <<<"$("$HOME/ws/scripts/env2025/bin/python3" - "$L" "$SURF" <<'PY'
...
try:
    ...
    print("%.3f %.1f"%(h.min(),dk))
except Exception:
    print("NA NA")
PY
)"
    fi
    printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\n" "$arm" "$rep" "${xy:-NA}" "${cls:-NA}" "$mh" "$dk" "$rec" >> "$RES"
    echo "[pairfix] $TAG arm=$arm rep=$rep xy=${xy:-NA} cls=${cls:-NA} min_h=$mh detOK=$dk"
  done
done
ko; echo "PAIRFIX_DONE" >> "$RES"
