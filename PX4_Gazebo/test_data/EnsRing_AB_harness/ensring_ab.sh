#!/usr/bin/env bash
# SITL flight-outcome gate for CROSS_GATE_MODE=ensemble + CROSS_RING_CONFIRM=1
# ("ens_ring", landed 263a0bb9, DEFAULT OFF) on flat cross_marker, PERCEPTION mode.
# Offline (RobustnessFrameset): base/darkbg bit-unchanged, dim/lowsun accuracy UP,
# inv detOK 99.1->42.2% (silent lie -> mostly-honest refusal, NOT a detection fix).
# This asks the one question offline can't: does ens_ring REGRESS the ORDINARY
# scene it will run on by default going forward? (inv/dim/lowsun benefit needs
# their own Gazebo world variants -- cm_inv etc -- and is NOT tested here; this is
# the must-not-regress gate on the everyday world.)
# Arms INTERLEAVED per rep so SITL temporal drift is shared, not confounded.
# Shape copied from LoomR_AB_harness/loomr_ab.sh (docs/SH_REFERENCE.md §10).
#   IC=2.0,2.0,5.0 N=5 bash test_data/EnsRing_AB_harness/ensring_ab.sh
...
# ⛔ CONCURRENT-SESSION GUARD -- see LoomR_AB_harness/loomr_ab.sh for the full note.
# ko() below does an unconditional kill -9 on every px4/gz/MicroXRCEAgent process.
if [ "${FORCE:-0}" != "1" ]; then
...
RES="$SD/run_logs/ensring_${TAG}.tsv"
...
for rep in $(seq 1 "$N"); do
  for arm in off on; do
    ...
    env LD_PRELOAD="${QT_PRELOAD:-/home/shubham/cvenv/lib/python3.8/site-packages/PyQt5/Qt5/lib/libQt5Gui.so.5}" \
        ...
        > "$SD/run_logs/ensring_${TAG}_${arm}_${rep}.out" 2>&1
    LG="$SD/run_logs/ensring_${TAG}_${arm}_${rep}.out"
...
PY
    ...
    echo "[ensring] $TAG arm=$arm rep=$rep xy=${xy:-NA} cls=${cls:-NA} min_h=$mh detOK=$dk"
  done
done
ko; echo "ENSRING_DONE" >> "$RES"
