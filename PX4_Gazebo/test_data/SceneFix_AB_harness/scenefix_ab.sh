#!/usr/bin/env bash
# SITL flight test: does ens_ring_balance (CROSS_GATE_MODE=ensemble
# CROSS_RING_CONFIRM=1 CROSS_BALANCE_CONFIRM=1, all DEFAULT OFF) help on the
# scenes it was actually designed for -- unlike the must-not-regress gates
# (ensring_ab.sh, pairfix_ab.sh), which only checked the ORDINARY cross_marker
# scene. Only cm_inv has been flight-tested so far (2026-09-04, partial fix:
# ~80% of reps broke out of a stuck-at-start-altitude failure). cm_dim/
# cm_lowsun/cm_col were offline-only until now.
#
# PERCEPTION mode (detector actually gates the flight -- unlike
# RobustnessFrameset, which was recorded under PLASMC_GT_FEEDBACK=1 and never
# let the detector fly). ens_ring_balance chosen as the "on" treatment because
# it was the single best offline combination measured across all 7
# RobustnessFrameset scenes (dim within-0.15 82->88, deepest inv/col
# rejection, no cost on clean scenes).
#
# WORLD is a parameter -- run once per scene:
#   WORLD=cm_dim    IC=2.0,2.0,5.0 N=5 bash test_data/SceneFix_AB_harness/scenefix_ab.sh
#   WORLD=cm_lowsun IC=2.0,2.0,5.0 N=5 bash test_data/SceneFix_AB_harness/scenefix_ab.sh
#   WORLD=cm_col    IC=2.0,2.0,5.0 N=5 bash test_data/SceneFix_AB_harness/scenefix_ab.sh
#
# Arms INTERLEAVED per rep. Metrics set copied from CmInv_AB_harness/cminv_ab.sh
# (max_lat included -- that is what revealed cm_inv's stuck-vs-lost distinction).
SD=/home/shubham/Soft-Precise-Landing/PX4_Gazebo; cd "$SD"
SCENE="${WORLD:?set WORLD=cm_dim|cm_lowsun|cm_col}"
IC="${IC:-2.0,2.0,5.0}"; N="${N:-5}"; TAG="${TAG:-$SCENE}"
LAUNCH="scripts/run_landing_retry.sh"; SURF=0.0
WORLD_ENV="WORLD=$SCENE MARKER_TYPE=cross"
...
RES="$SD/run_logs/scenefix_${TAG}.tsv"
...
for rep in $(seq 1 "$N"); do
  for arm in off on; do
    GM="legacy"; RC=0; BC=0
    if [ "$arm" = on ]; then GM="ensemble"; RC=1; BC=1; fi
    ko
    ...
    env LD_PRELOAD="${QT_PRELOAD:-/home/shubham/cvenv/lib/python3.8/site-packages/PyQt5/Qt5/lib/libQt5Gui.so.5}" \
        ...
        INITIAL_DRONE_ENU="$IC" CROSS_GATE_MODE="$GM" CROSS_RING_CONFIRM="$RC" CROSS_BALANCE_CONFIRM="$BC" \
        timeout 260 bash "$SD/$LAUNCH" \
        > "$SD/run_logs/scenefix_${TAG}_${arm}_${rep}.out" 2>&1
    LG="$SD/run_logs/scenefix_${TAG}_${arm}_${rep}.out"
...
PY
    ...
    echo "[scenefix:$SCENE] arm=$arm rep=$rep xy=${xy:-NA} cls=${cls:-NA} min_h=$mh detOK=$dk max_lat=$ml"
  done
done
ko; echo "SCENEFIX_DONE" >> "$RES"
