...
> **🟢🟢 2026-06-30 (LATEST) — RESIDUAL TERMINAL LIMIT CYCLE characterized; W_U_MAX=2.0 BAKED. Memory [[project_residual_cycle_wumax_bake]].**
...
>   - **`E_z=0.5`** (was 1.0): the Z limit cycle rang 100% INSIDE the boundary layer (`|σ_z|/E_z≤0.2` → sat linearized → **κ's switching damping locked out**; κ IS the damping). Tightening E_z engages it → Z cycle −8%. `E_z<0.5` hard-switch OVER-DRIVES the aggressive cells (NOT noise chatter — σ_z noise is only 0.05; it's sat-saturation on large-σ_z cells; `P_z`↑ softens the over-drive but trades damping). **X/Y cycles are ALSO inside the layer but tightening E_x/E_y makes them WORSE — they're NOISE-PUMPED (s̈ positive feedback), so κ switching FEEDS them.** Z=under-damped (κ helps), X/Y=forced (κ hurts; s̈-drop is the lever). **⚠ OLD-REGIME — RE-TEST PENDING (2026-06-30):** this "κ hurts X/Y / keep E_xy=1.0" verdict was measured when the X/Y cycle was strongly FORCED by the un-smoothed drift (q=10). `VDS_KF_Q=1` (baked 2026-06-30) REMOVED that drift forcing → the relay-feed mechanism scales with the forcing, now small → so reduced `E_xy` is an OPEN test on the new base (q=1 + W_U_MAX=2.0 + Z_REG=0.2), NOT a pre-judged dead-end. Do NOT cite this as binding until re-run. (Pin all 3 axes `PLASMC_E_X/Y/Z` — any single axis bypasses the combined auto-align → all revert to bare 0.8/0.8/0.5.)
