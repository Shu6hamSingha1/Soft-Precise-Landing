...
> **GAIN-SIDE LEVERS MOSTLY EXHAUSTED — but `gamma_s>1.0` (outer funnel) + `KP=12` (re-check at E=1.5 + SEN_FUNNEL) were NOT actually swept (correction 2026-06-10; sweeping now). The binding limit underneath is still the LK dynamic range ~2 m/s. Main next lever: code-level perception.**
