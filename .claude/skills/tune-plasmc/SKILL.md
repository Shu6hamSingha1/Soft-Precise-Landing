...
| **10** | **POST-touchdown attitude divergence → permanent marker loss** (the binding IC1 problem as of 2026-07-09) | Clean precise touchdown (min_alt_xy ≤0.12) → drone sits armed on deck → roll OR pitch escalates 1°→7.5°+ over 0.5-7s → single-frame yaw-feature jumps (43°→117°) → n_corners→0 permanently → endpoint 3-8m | NOT a descent-phase problem, and NOT caused by the KLT relax gate (verified: corruption predates it). Open levers: landed-state commit/disarm (why is the loop still fighting after touchdown?); paired A/B whether `FLOW_FUSE_RING=0` unmasked it. See memory `px4/project_baked_sweep_and_posttouchdown_divergence`. |
...
If the failure is mode 1 → no controller-side tuning fixes it. Mode 2 → already fixed by defaults. Mode 4-7 → tuneable. Mode 3 → secondary to mode 1. **Mode 8b → fix the IC rig, NOT the controller/alpha.** Mode 9 → keep the bake. Mode 10 → the current open frontier (post-touchdown, not descent).
