...
> **🟢🟢🟢 2026-07-02 LATE (LATEST) — the CURVED-TARGET miss is a self-sustained lateral LIMIT CYCLE, fully attributed; HD_KR=0 = the one validated lever. Memory [[project_rover_turning_open]] (the whole causal chain, read end-to-end).**
> - **Mechanism (data-pinned):** ONE SISO loop — (e,ė) → 1/z bearing → parallel branches {Γζ, χ_r·ζ_r, drift χ_r·ζ̇_r/G, θ·sat(σ/E)·κ relay, −ḣ_d, loom×h} → Σ = a_u → **−120° actuation phase** (cross-spectral, delivered vs commanded) → (e,ė). ALL components are ONE signal (corr 0.8–0.99 vs s_e_n/s_dot_meas) — "multi-channel" was multi-BRANCH; pruning/smoothing one branch just re-balances the describing function. Cycle ~1.7 rad/s + harmonics; amplitude set by operating point (5–7 cm straight, 0.3–0.7 m curve, ~1 Hz stationary-terminal = SAME loop, three regimes).
> - **Caps/clamps exonerated** (w_u/tilt/izeta/w_i ≈0% duty); live nonlinearities = sat 29–36% duty + κ ratchet 0.27→1.7. Filter phase recoverable only ~25° (VDS-KF −9/−16°, TAU_IA −14°, GT-h −8°, DHD −6°) vs 120° actuation → **only GAIN attenuation is parameter-tunable** (re-frames the VDS_KF_Q 10→1 win as gain attenuation at cycle freq, not noise smoothing).
> - **Lever sweep (heading-hold Circular n=3):** `HD_KR 0.5→0` **BEST — 1/3 ON-PLATFORM (0.200, first-ever Circular landing)**; removes the 06-29 regression (`_hd_src` switched to FULL h_d, re-differentiating the funnel/k_r term = the s̈-drop violated; the "rate of h_d_noS" comment at controller.py:1500 is STALE); c3 −24%, switching −24%, duty 36→26%, e-rot 1.11→0.85. **Stationary IC gate REQUIRED before baking HD_KR=0** (0.5 was baked on stationary evidence). `P_xy=5` FALSIFIED (adaptive-leakage: κ drains → σ grows → ratchet 3.04, worse). `E_xy=1.5` + `P2INF 2.0/3.0` FLAT/weak (DF re-balances; P2INF also adds a wander outlier).
> - **FRONTIER:** at fixed −120° phase, gain levers trade cycle amplitude ↔ tracking bias (killing the cycle by gain needs ~3× cut = ~3× curve bias). Exits: AU lead compensation on the single shared path (NEW code `PLASMC_AU_LEAD`, unapproved), or platform size. **⛔ K_R=2.5 spent (sharp optimum, was the same phase fix); ⛔ uXRCE-DDS RULED OUT by user — never re-propose.**
