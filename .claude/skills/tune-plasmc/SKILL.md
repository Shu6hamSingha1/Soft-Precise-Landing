...
| **9** | **Ring-fusion loom spike → terminal lateral kick** (FIXED 2026-07-09 by `FLOW_FUSE_RING=0` bake) | h_z spikes to −0.8..−1.9 near touchdown while GT vz≈0, attitude flat, extent flat; `Ring Divergence`/`Ring Opt Flow Ang Vel` already wrong BEFORE any marker loss; endpoint xy balloons far past min_alt_xy | Root: fixed ring radii (41-199px) overlap the marker once `MARKER_EXTENT_PX` >~160px → outer tiers sample the marker (depth-mixing + LK aperture failure on ArUco cells), whole tiers fail together so MAD can't reject. Keep `FLOW_FUSE_RING=0` (default since 2026-07-09). See memory `px4/feedback_ring_fusion_marker_overlap`. |
| **10** | **POST-touchdown attitude divergence → permanent marker loss** (the binding IC1 problem as of 2026-07-09; root enabler now found — see the `improve-perception` skill's touchdown-detection modes) | Clean precise touchdown (min_alt_xy ≤0.12) → drone sits armed on deck → roll OR pitch escalates 1°→7.5°+ over 0.5-7s → single-frame yaw-feature jumps (43°→117°) → n_corners→0 permanently → endpoint 3-8m | NOT a controller-tuning fix — this is a PERCEPTION/detection-layer issue (why doesn't the touchdown detector disarm before this develops?). See the `improve-perception` skill and memory `px4/project_ic1_terminal_kick_root_cause_chain`. |
| **11** | **κ-ratchet on sustained off-center error** | `switch = θ·sat(σ)·κ` term explodes (reconstruct from logged sigma/G/theta/kappa: reach=Γσ/G_d, switch=θ·sat(σ)·κ, resid=a_v−both) while zeta_r/reach stay small; coincides with extent collapse; a_u_xy 25-40 | The adaptive-gain ODE ratchets when σ stays saturated (off-center IC + degrading perception). Known family ([[kappa-4axis-hexy-param-map]]); largely disappeared after the FLOW_FUSE_RING=0 bake removed a σ driver. |

If the failure is mode 1 → no controller-side tuning fixes it. Mode 2 → already fixed by defaults. Mode 4-7 → tuneable. Mode 3 → secondary to mode 1. **Mode 8b → fix the IC rig, NOT the controller/alpha.** Mode 9 → keep the bake. Mode 10 → perception/detection-layer, not a controller-tuning fix — see the `improve-perception` skill.

**If the failure mode involves optical-flow/loom quality, KF filter state, marker detection/tracking, sensor calibration, or the touchdown detector itself (not a gain/parameter), STOP and use the `improve-perception` skill instead of this one.**
...
| KLT relaxed 3/4-corner gate + parallelogram completion | `1` (BAKED 2026-07-09) | `MARKER_KLT_RELAX_GATE` | Accepts 3/4-tracked fallback frames, reconstructs the missing corner (`missing = other1+other2 − diagonal_partner`, 2D-pixels-only, scale-free). Fixes momentary 1-2 frame flicker (100% of that regime's rejections were 3/4). `=0` reverts to strict 4. Memory `px4/feedback_klt_relax_gate_parallelogram`. |
| Corner+ring fusion EKF | `0` (BAKED OFF 2026-07-09; was ON since 06-07) | `FLOW_FUSE_RING` | ⛔ Ring loom corrupts near touchdown (fixed ring radii overlap the grown marker, failure mode 9 above) and it fed the controller's h(t) → terminal kick. The old default-ON rationale ("fused==corner ratio 1.00") never validated the ego/ground split. Keep 0. Memory `px4/feedback_ring_fusion_marker_overlap`. |
| Per-attempt KLT-fallback diagnostic log | always on | — (`"KLT Diag"` key in Img_Data.npy) | n_tracked/gate/reconstructed_idx/accepted per fallback attempt; analyze with `tools/diagnose_klt_gate4.py <rep_dir>`. Segment flicker vs collapse regimes before judging. |
