...
**NC48a–e — descent bootstrap.** gamma_s=1.0 fast-centering caused a 6m **hover** (κ_z too slow to start descent). `KAPPA0_Z 0.31→1.0` bootstraps descent in 0.4–0.9s — but exposed κ_z runaway (107). `KAPPA_MAX_Z=3.0` + **κ-freeze on outlier-containment axes** fix it → **baked** (NC48e: xy_med 2.34m, n=5, 0 TL). [[feedback_descent_bootstrap_fix]]. (Singhal-2025 outlier containment on the barrier, `bf21e52`, feeds the κ-freeze.)
