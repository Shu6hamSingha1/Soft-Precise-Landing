...
> **FIX implemented + A/B'd (2026-08-25): [[project_20260825_cbf_margin_reserve_fix]]** — `CBF_MARGIN_RESERVE` (default 0.0=unchanged, `validate_cbf.py` still 12/12) added to BOTH `cbf_visibility.py`/`cbf_visibility_aruco.py`: Phase 1's box bound was centroid-only by design (`m2=phi_max`, delta2/footprint only used reactively in Phase 2 after decode already failed) — this proactively reserves a fraction of the marker's own measured footprint in Phase 1 too. IC5 ArUco A/B (perception, dtheta=0 confound removed): reserve 0.0 and 0.5 both hit `TARGET_LOST`. **Extended to n=3/arm (2026-08-25): INCONCLUSIVE, not the confident win first reported.** reserve=0.0: 2/3 TARGET_LOST; reserve=1.0: 1/3 TARGET_LOST but that ONE failure was xy=22.46m, the single worst result of the whole investigation — real variance in BOTH arms. IC1/IC2 regression spot-check (n=1, reserve=1.0): no obvious regression (IC1 SOFT+PRECISE 0.083, IC2 SOFT-only 0.190); IC3/IC4 not checked. Mechanism confirmed genuinely engaging (diverging trajectories/theta_cone, not a no-op) and correctly targeted — but n=3 does not support confidently baking reserve=1.0. NOT baked (left at default 0.0). Needs a real n>=5 sweep across {0.5,0.7,1.0} + IC3/IC4 regression before reconsidering.
