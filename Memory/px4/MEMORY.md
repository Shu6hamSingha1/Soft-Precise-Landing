...
> ⭐⭐ [ARCHIVE RE-SCORE: 13.4% of all 'precise' landings were scored MID-AIR](project_20260902_archive_rescore_false_precise.md) — 2026-09-02: re-scored all 4446 saved runs against a SURFACE-relative altitude floor (flat 0.20 m / rover 0.70 m — a legit landing on the 0.4 m rover platform sits ~0.48 m above the rover target-pose origin, so 34 runs were rescued by getting this right). **159 of 1192 `precise` verdicts are unsupported**: 21 ABORTED (a centered IC starts above the marker → `xy_err`~0 at t=0) + 138 whose GT log stopped MID-DESCENT (112 still descending, median last alt 0.486 m — a touchdown detector false-latching at altitude). Bounded to **Jul 19-23 and Aug 22-24**; `ICValidation/20260828` (27/0), `20260829` (103/0) and the post-alpha0 gate `20260831-144626` (13→12, its one flag marginal at 0.211 m) are CLEAN — the IC1-5 stationary result STANDS. `test_data/Final/IC5` is the published casualty ("precise 0.058 m" scored at 0.422 m alt → Final is 1/5, not 2/5, see [[reference_final_landing_recordings]]). FIX (unimplemented): the real defect is that `xy_err` is sampled at the LOOP-EXIT pose (`landing_test.py:835`), not at a verified touchdown. Use an absolute floor on altitude above the LANDING SURFACE (`LANDING_SURFACE_DZ` explicit per world, never inferred) + a surface-agnostic `min_alt > 0.15*alt_start` backstop (catches 45/159, **0/1033 false positives**). ⛔ REJECTED after measuring: a `|dz/dt|` terminal-rest gate (catches 152/159 but rejects **58% of LEGIT landings**) and an IMU contact-spike gate (no separation: 27.0% vs 26.7%) — an earlier version of this line recommended the dz gate on its catch rate alone, which was wrong.
