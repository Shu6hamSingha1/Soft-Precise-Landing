...
> ⭐ [2026-08-31 SESSION — perception-mode landing pushed forward; terminal fly-away root-caused to _dtheta_correction, now removed](project_20260831_perception_mode_landing.md) — fixed the run() list(int) crash (the real blocker), origin-free arm-pixel selection in cross_marker_detector (bbox-centre bearing was circular; recovers detection ~0.73→0.5m), tdV2 flow-freeze false-fire at 4.87m. KEY: the terminal ascent is NOT a spurious loom (verified 3 ways incl. pairwise-distance ratio) — it's `_dtheta_correction` (CBF visibility-conflict lift, always-on, no flag) adding lift at alt 0.4m → self-reinforcing climb. REMOVED it; folded the descent-rate/lateral-margin trade into the joint QP as `CBF_AZ_COST_GAIN` (default 5.0, clamped at −g so it can only slow a descent, never reverse it). CBF_visibility.pdf updated. Perception-mode landing still NOT working end-to-end; CBF change + origin-free detector NOT yet SITL-validated — next: re-run perception IC1.
