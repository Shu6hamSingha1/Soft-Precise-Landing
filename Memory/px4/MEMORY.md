...
> ✅ **PLASMC_YAW_RATE_LAW — BAKED DEFAULT ON (2026-09-09)** [full log](project_yaw_rate_law_sign_bug_and_validation.md) · [ASMC-vs-new mechanism](feedback_yaw_rate_law_vs_asmc_mechanism.md). The new yaw law (direct integrator `d(w_u2)/dt = k_p·e_a + w_z`, bypasses psi_d/e_R) BEATS the ASMC on turning targets: ASMC routes the rate cmd through `e_R[2]=sin(Δψ)` — bounded+non-monotonic, hard-caps `|w_u[2]| ≤ K_R_yaw=0.5 rad/s`, collapses past Δψ=90° → runaway. New law commands the rate directly (clamp ±2.0) and `+w_z` is a free target-rate FF. GT-FB turning gate: `e_a` held 1-4° on a 27°/s spin vs ASMC → −200°. Two sign bugs found+fixed, both frame-derived not tuned: (1) actuation chain inverted (`w_u[2]=+2 → −2.03` achieved); (2) `w_z` convention — `gt_feedback.py:246` now feeds `+w_z,manuscript` (was `−_slope`, an erroneous `alpha_dot` sign) matching the calibrated perception `w_iz`; `WZ_SIGN` retired. Validated n=1 GT-FB (yaw relabel no-op + lateral monotone-converge, zero fly-out incl. IC4). Stationary IC1-5 n=5: **24/25 land, 0 TL** at `FLOW_KF_Q_WZ=1.0 + WZ_SCALE=2.5` (`w_z` is noise-limited not lag-limited; `Q_wz=1.0` rescues IC5). `k_i` = DEAD END (gate freezes `_yaw_rl_ie`). Terminal `e_a` ~15° = gate-freeze artifact (fix = ramp-cmd→0, stationary-only; wrong for a turning target). **BAKED default ON 2026-09-09** (`PLASMC_YAW_RATE_LAW` "0"→"1", `WZ_SCALE` "1.0"→"2.5"; `FLOW_KF_Q_WZ`→1.0 SCOPED to the flag in cross_marker_perception.py so the shared w_z KF / ASMC lateral path is bit-identical when the law is off → gate `20260909-030532` covers the flip, no lateral re-gate). Cross-marker only (non-cross silently falls back to ASMC). Stationary = even trade vs ASMC (25/25 land, 18 vs 16 precise, new wins IC1-3 / ASMC wins IC4-5); its turning-target value still needs the separate LATERAL limit cycle ([[project_rover_turning_open]], `PLASMC_AU_LEAD`) fixed — B4, own session. `PLASMC_YAW_RATE_LAW=0` restores ASMC+psi_d+e_R. `WZ_SCALE` recal CLOSED as not-actionable (2026-09-09): phased `output_cross` data carries no usable `w_z` scale — col-5 `[-y;x]` rank-degenerate with Tx/Ty, isolated `s_wz` R²≤0.05 with filter-dependent SIGN; `PLASMC_YAW_RL_WZ_SCALE` IS the post-KF w_z cal, retune in place ([[feedback_cross_marker_radial_spread_ceiling]] 2026-09-09). Still deferred: ASMC/psi_d removal (keep as fallback).

> ⭐⭐ [STROKE terminal (<0.3 m) refusals = ring radius in σ units, not stroke widths (2026-09-24)](project_20260924_stroke_terminal_ring_radius.md) — `_ring_x` r=2.5σ ≈ 0.7 stroke widths = inside the junction blob. PENDING (see file end; peer owns the module, stroke now default). RING_K=4 offline on PerceptionEvalSet: <0.3 m detOK 84.9→98.3%, poison 16→8, no tag worse; 5-6σ add poison. NOT in src yet. Also: rover runs need meta.json marker_dz=0.5 or the true-bearing ref is wrong.
> ⭐⭐ [rover_cross start-of-descent misses = plate CORNER from the shaded side face (2026-09-24)](project_20260924_rover_platform_corner_detection.md) — dark gate admits the platform's shaded face (V≈90); detector returns the corner at native res; the jump gate then keeps the corner and rejects the true centre. Ring+balance confirm rejects corners correctly (the '15 true lost' was a tracker-state pairing artefact) but can't recover the true cross. Pair retry falsified (only 2 clusters); COMPONENT-level retry + 0.1 area floor prototyped offline; ⛔ SUPERSEDED by the stroke detector (19cdabff) — do not port

> ⭐⭐⭐ [Visibility CBF REBUILT from clean requirements + wired in — IC2-5 A/B PASS (2026-09-09)](project_20260909_visibility_projection_wire_in.md) — `src/visibility_projection.py` (240 lines) replaces `cbf_visibility.py`/`cbf_visibility_aruco.py` + the joint-QP/deliverability-sphere/descent-relief/two-phase-δ/`rho_fov`-cone machinery (~2000 lines → `Obsolete/`). **Tier 1** — a discrete-time one-step CBF-QP (barrier `h_k=φ_k−|c_k|`, constraint enforces `h_k(c_next)≥0` on the predicted feature, min-norm = minimal intervention) keeping the measured cross-marker CENTRE inside `φ = R/(2f)·(1−CBF_BUFFER_FRAC)` on the REAL camera plane (`a_d[2]` fixed; `Le = −(Lw@M)` — sign NEGATED vs the old machinery, the old one was backwards for a down-camera, verified vs an independent pinhole oracle). **Tier 2** `descent_ease` (NOT a CBF — a soft self-releasing governor; `CBF_DESCENT_EASE`, default on): scales only the downward part of `a_z` by `g_z∈[g_min,1]` on the measured time-for-`|c|`-to-reach-the-edge — external trigger, post-Tier-1, bounded scale, self-releasing (none of the `CBF_AZ_COST_GAIN` failure modes). Scale-free/depth-free. Wire-in `82fa9c16` (controller −289 net lines); kept the freshness-gated centre source, z-guard, `CBF_LPF_BEFORE`, downstream `|I_a|≤A_CAP` + lean cap; drift-off pull-back → per-axis buffer bump; `CBF_OVERFLOW` permanently False. **Gate `test_data/VisProjGate/20260909-005328` (worktree @ `d380901c` = old joint-QP as the OLD arm), IC2-5 n=5: both 20/20 land / 0 TL; NEW pooled mean xy 0.14 vs OLD 0.24, precise 11 vs 8, max rel_vel 1.47 vs 1.95; NEW wins IC3/IC4, IC2/IC5 a wash.** No regression, tighter+softer at half the code. **WHY NEW>OLD (traced from the bundle):** median is a WASH (both CBFs idle on clean approaches). NEW wins the TAIL — the old joint-solve/relief/pullback/θ_cone-floor stack THRASHED on marginal off-center approaches: (1) intervened when it shouldn't (OLD `dtheta_az` fired 4-5% of terminal frames vs NEW `vis_active` 0% — fought the SMC's re-centering), (2) θ_cone chattered 3-5× → κ ratchet (`a_u` 32-55 vs 5-6, κ 0.5 vs 0.1), (3) descent-relief added terminal edge-exposure pressure. **All OLD tail failures end at `s_e_n≈0.9-1.0` = the unresolved off-center lateral-convergence wall** ([[project_20260824_crossmarker_offcenter_convergence_wall]]); NEW does NOT fix that wall (still hits it once, IC5r2) — it stops the CBF from worsening it. New logs `vis_gz(t)`/`vis_active(t)`; dropped `rho_fov(t)`/`d_min_fov(t)`/`dtheta_az(t)`/`jqp_resid_*(t)`. Validator `tools/validate_visibility_projection.py` (now **15/15**). ⚠ driver bug: both arms autosaved to the main tree's `Landing_Test/` (set `LANDING_OUT_BASE` per arm for any 2-tree A/B). **SINCE THE ORIGINAL WIRE-IN (all in the topic file's UPDATE sections): Tier 1 is now ONE convex QP** (`e63751e2`) — visibility half-planes + deliverability ball `‖y‖≤√(A_CAP²/a_z²−1)` folded in (`I_a` feasible by construction) + penalised slack `CBF_VIS_RHO=2000` (graceful, never infeasible); projected-Newton + circle-refine solver. **Moving-target lead `τ·d` LIVE + BAKED** (`28e4417b` frame fix — `h_xy` is identity-mapped, not swapped; `e1b094e8` `condition_drift` — `rel_resid` gate / median-3 / LPF / `|d|≤CBF_DRIFT_MAX=0.5` clamp; **`CBF_DRIFT_TAU` default 0→0.15, baked on CBF-behaviour not SP**: `τ=0` and `τ=0.15` both hold the stationary safe set 100%, but reactive-only `τ=0` leaves a moving-target gap — 278 centre-off-sensor frames / 24 rover reps — that `τ·d` closes). One law, no scenario branching. Follow-up hardening (open): overfill gate on `d`. New logs `vis_slack/vis_drift/vis_c(t)`. ArUco 4-corner path still out (module is cross-marker only). → judge CBF changes by [[feedback_dont_judge_cbf_by_sp]].
>
> ⛔ [Judge a visibility-CBF change by CBF BEHAVIOUR, not SP](feedback_dont_judge_cbf_by_sp.md) — 2026-09-09 (user correction, 2nd instance of the SP-is-the-wrong-metric lesson; cf. line for the KF-fusion flag). Called the `CBF_DRIFT_TAU=0.15` confirm gate a "failure" on P+S 11 vs 14 + one 2.80 m/s IC4 TD, and reverted. WRONG: at n=5 SITL, SP is bimodal noise and most "regressed" reps had `vis_active=0` (CBF inert). Judge by: safe set held (frames with `|c|` past the physical FoV edge — must be ~0), trigger correctness (fires on predicted breach; early fire on real drift = correct anticipation, not a false positive), safe-input minimality, reaction lag. Judged that way `τ=0.15` held the stationary safe set 100% (= `τ=0`) and closed a moving-target gap → BAKED. Match the metric to what the component GUARANTEES.

> ⭐⭐⭐ [Line-width loom investigation (width itself: LANDED+validated) + origin_ratio ⛔⛔ PROVEN UNSUITABLE FOR CROSS-MARKER, reject on principle (2026-09-08)](project_20260908_line_width_loom_investigation.md) — width-loom: found+fixed 2 real bugs in the existing width machinery, replaced with a direct mask-thickness scan, **validated 0.89-1.00 corr with GT altitude across all 5 ICs (7 reps), landed as shadow-mode `"Width Loom Px"`** (not consumed by control). `origin_ratio` double-gate: caused a confirmed crash regression on every off-center IC (2 sessions, 12/12 correlation); reverted (`CROSS_TZ_VETO_R_MULT` 1e6->1.0). Relative-drop/streak fix tested against 15 real off-center reps: decisively failed (24-45% false-veto rate). **Root-caused to a PROVABLE architectural mismatch, not a tuning gap: cross-marker's flow points have no fixed physical identity across frames** — measured point-count churn predicts `origin_ratio` volatility monotonically in both centered and off-center real flight data. **Reject origin_ratio-based Tz gating for cross-marker permanently, on principle** — `CROSS_TZ_VETO_R_MULT=1.0` stays default.
>
> ⛔⛔ **THREAD CLOSED 2026-09-09 — line-width / extent / size-derived loom RATE for `h_z`: DEAD END, exhaustively chased.** See [[project_20260908_line_width_loom_investigation]] (top block). Summary: **static line-width VALUE** is good (0.89-1.00 vs GT alt) and `width_loom_from_detection` now defaults to a **geometry method** (`CROSS_WIDTH_GEOM=1`, `bf812f1f`: `2√3·std(arm-inlier transverse residual)`, gated on arm-perpendicularity + junction-in-frame, `None` on fail; bounded 12-35px terminally vs mask-scan 14-201) — SHADOW-ONLY. **Pure line-width RATE** (`"Width Loom Rate"`) is dead (~0.22 corr w/ GT loom in the .5-2m band, noise elsewhere): the VALUE fits position at R²>0.95 but its residual is a low-freq drift whose derivative swamps the loom signal (ruled out: lag, binarisation, KF tuning, direction-wobble, V-frame leveling). **Extent-fused `"Scale Loom Rate"`** (`0.3·ln w + 0.7·ln extent`, extent is the workhorse) tracks GT loom 0.84-0.98 in the .5-2m band, live-parity confirmed — **but 3 `h_z`-fusion forms (naive / +overfill-gate / +affine-debias+band+loose-r) ALL regressed the IC1-5 gate** (hard landings, then blown lateral xy; `h_z` couples into the middle-loop SMC c-term + sliding surface, and pinv `h_z` is already good enough that any perturbation only costs accuracy). **`CROSS_SCALE_RATE_FUSE` default-OFF is PERMANENT.** **Terminal window (<~0.35m) is unrecoverable for ANY size-derived loom** (line-width, extent, moment-loom): ~70-80% of the logged `<0.5m` band is post-touchdown (parked, GT loom≈0); the genuine last ~0.3m has a real SIGN INVERSION — the marker fragments / exits the FOV, arm fits latch onto background, `corr(width, inlier_count)=+0.65`. Extent saturates (318px = frame). **Useful range of any width-based loom: ~2m → ~0.35m only**; below that the touchdown detector (`_touchdownDetectV2`: n_corners/extent/flow-freeze — none use `h_z`) already owns the regime. Kept (all shadow/diagnostic): the 4 log channels, `_wgeom_arm`+geometry width, `_wloom_*` mask-scan fallback (`CROSS_WIDTH_GEOM=0`), the `_scale_fuse_*` machinery behind the OFF flag, `tools/overlay_width_loom_rate.py`, and the `tools/gt_optical_flow.py` 3-bug fix (real correctness fix, outlives the thread). **Do not re-open the SIZE-derived RATE without a different observable (not apparent size / its derivative) or a different consumer (not a KF measurement of live `h_z`).** **✅ WHAT ACTUALLY FIXED THE ORIGINAL SPIKE (baked `7e9843ae`):** a **loom-channel innovation gate** on the hw-KF — down-weight a loom measurement only when it is BOTH statistically surprising (NIS `= y²/S > CROSS_LOOM_NIS_GATE=25`, `y` = residual beyond the KF's own predicted loom-accel trend) AND physically implausible (`|y|/dt > CROSS_LOOM_SLEW_MAX=12` /s; clean flight tops ~5-8, GT `|dloom/dt|` p95 ~0.2, the spike ran 20-260), debounced (accept after `CROSS_LOOM_GATE_MAX_STREAK=6` consecutive trips) — plus a hard abs clamp `|h_z| ≤ CROSS_LOOM_ABS_MAX=20` (= the moment-loom `sol[2]` clip) for NaN/grazing blowups. **`CROSS_LOOM_INNOV_GATE=1` default-ON; `=0` restores pre-gate.** Validation: stationary n=3 IC2-5 (`ICValidation/20260909-162140`) = clean wash (12/12 land, 0 TL, 0 fly-away, xy ~0.06); offline it turned a spiky rep's `>2m` GT-loom corr −0.07→+0.83 and caught a +1601 blowup. 7-profile moving-rover analysis (`RoverCBFSweep/20260909-163929`): trip rate 0.7-1.8% of descent frames, `h_z` bounded [−2.4,+3.4] on 6/7 trajectories, abs clamp never fired; 6-frame debounce ceiling reached on most rover trajectories (~0.16s coast, benign); Lissajous the one soft spot (+9.34 raw `h_z` partway through a 445/s jump, still 2× under the clamp). Peer cross-link: `rel_resid`/`cond(A)` from `_solve_jacobian` is a ready-made scenario-agnostic "h-solve confidence" signal if a shared h_z+h_xy trust gate is ever built (rover thread, when unblocked).

> ⚠ [CBF joint-QP: descent-rate-relief self-inflation fix (`CBF_JQP_RELIEF_REF_AZ0`) PROVISIONALLY rejected — gate base was loom-confounded, needs a clean re-gate](project_20260908_jqp_relief_refaz0_gate_failed.md) — 2026-09-08. `CBF_visibility.pdf` S4 open #2 (relief↔box fight): the relief measured box-suppression against the RUNNING `_az_now`, which the relief itself inflates each outer iterate → `Ia_z` ratchets to `-g` on any box bind. Fix = measure Δθ at the FIXED unconstrained `a_z`. Offline: `validate_cbf.py`+`test_joint_qp()` 17/17 (that path had ZERO coverage before), 926/926 synthetic cases retained more descent authority. **A/B (`test_data/JQP_ReliefRefAz0_AB/20260908-160523`, n=5/arm/IC): mean xy_err higher at all 4 ICs (1.08→1.68 m), TL 4→7.** ⚠ **CONFOUND (peer session): that gate ran on a base WITH `c3a46d1a`'s origin_ratio loom-veto bug — off-center `h_z` frozen/corrupted positive in ~70% of the reps, BOTH arms.** Matched+interleaved so direction may survive but effect size is unreliable. Fixed base = `e173b05c`. **NEEDS A CLEAN RE-GATE** before the rejection is settled (queued; SITL lane held by peer). **LOOM-INDEPENDENT mechanism that stands (`f61c0608`): box↔relief DEADLOCK** — the `I_a[2]`-pinned-at-`-g` signature is specific to `relief_on` (off-arm pin fraction ~0), so the relief term is doing it, not the frozen loom: proportional+persistent relief fires every frame the box binds → pins `I_a[2]` at `-g` → `B_T`→0 → descent freeze → box still binds → relief still fires. Left **default OFF**. **STANDS regardless of the re-gate:** the per-outer-iterate residual instrumentation (`7b81ac1f`) and `test_joint_qp()`. `CBF_visibility.pdf` S4 #1 + fixed-iterate-budget-no-early-exit remain OPEN.

> ⛔ **h_ref IS CONSTANT (user-mandated 2026-09-08, `1473bd48`).** The two runtime descent-reference scalers — `PLASMC_DESCENT_GATE` (|s_e_n|-smoothstep × `_h_ref`) and `PLASMC_DTHETA_HREF` (exp(−dtheta/scale) × `_h_ref`, AZ visibility filter v3) — are REMOVED from `controller.py`. Both were default-OFF and never passed a gate, so it's a no-op at every real config (`h_ref_eff` was already `== self._h_ref`). `h_d`'s descent term now unconditionally uses the constant. No env var can reintroduce a time-varying descent reference. Do NOT add one. (The "slow descent to buy the lateral loop time" family — `_dtheta_correction`, `DTHETA_HREF`, `DESCENT_GATE`, `CBF_AZ_COST_GAIN` — has failed in every form; see [[project_20260908_visibility_cbf_simplification_audit]], [[feedback_descent_softness]].)

> ⭐ [Visibility-CBF simplification audit — the joint-QP layer is the accretion](project_20260908_visibility_cbf_simplification_audit.md) — 2026-09-08, prompted by "did we overcomplicate this?". **Certified core (barrier + `L_w@M` + one box QP + `th_safe→rd3`, ~40 lines) is fine, leave it.** The `CBF_JOINT_QP` layer (2026-08-29, ~152 lines) exists to host (a) a deliverability sphere that **binds 0.0%** (4/9889 gate frames; 81-97% of cap normally) and (b) a descent-rate relief that has **failed in 3 forms** (`_dtheta_correction` removed, `PLASMC_DTHETA_HREF` obsolete, `CBF_AZ_COST_GAIN`+`REF_AZ0` deadlock) and **never won a SITL gate**. Algebra + 3000-case test: **joint QP ≡ theta-path when the sphere doesn't bind and relief is off** (`worst |Δth_safe|=0.064 rad`, just iteration-order noise). Phase-2 fires 0.1%. Legacy `rho_fov`/`d_min_fov` cone dormant (`PLASMC_LFOV=0`) — but the `_cbf_drift_off`→`p_10_eff` pullback it feeds IS live on IC5, keep that. Simplification list ranked by safety: (1) kill the relief, (2) collapse joint-QP→theta-path + one `arccos(a_z/A_CAP)` clip, (3) bake `CBF_LW_ROT`/`CBF_RD3_DIRECT`, delete `=0` forks, (4) retire the frozen cone scaffold, (5) delete `CBF_HZ_AWARE_DRIFT`/`CBF_MARGIN_RESERVE`. Each still needs an IC2-5 cross-marker SITL gate. NONE done — analysis only.

> ⭐ [NULL RESULT: CROSS_LOOM_R_SCHEDULE gives NO flight-outcome gain — keep it OFF](project_20260903_loom_r_schedule_ab.md) — 2026-09-03, n=5 interleaved SITL A/B on flat `cross_marker` IC2: **3/5 precise BOTH arms**, median xy 0.078 vs 0.083, terminal vz identical (0.120 vs 0.123). Engagement verified (`Loom R Mult` max 4.000 on every ON rep, 1.000 on every OFF rep) so the arms really differed. The offline gain (1.3-2.0 m loom r +0.309→+0.405) did NOT convert. ⚠ One consistent signal: vz at 1.3-3.0 m ~15% FASTER on ON (0.931→1.073), the under-brake direction [[feedback_pinv_tol_loom_scaling]] warned of; it did not reach the terminal band. ⭐ MECHANISM CONFIRMED: below ~1.3 m the loom is COHERENTLY CORRUPTED (190 pts, coverage 2.19, cond 7.8 — all healthy — yet r vs GT +0.013), so no noise model can help; a fix must change WHAT IS MEASURED. Also: `w_x`/`w_y` are NOT observable from the background-texture field (|r|<=0.37, Tx↔Wy column cosine 0.90-0.99 all the way down — the wider point spread does NOT break the 2026-08-08 aliasing; consistent with `_sensor_cal_hw` already zeroing those rows).
...
> ⭐⭐ [318-run CONTROLLER population analysis — kappa/sigma REFUTED, CBF inactive, residual is control-bound](project_20260903_controller_population_analysis.md) — 2026-09-03: **kappa decays FURTHER in successes** (end/start 0.212 landed vs 0.890 failed) and **sigma max is HIGHER in failures** — so "the gain leaks away" is refuted; an n=1 trace had said the opposite. CBF inactive (`dtheta_az≈0`, `az_joint_delta=0`). Discriminator = **lateral divergence, 2% landed vs 66% failed**, but diverged≠failed: low-detOK runs diverge LEAST (19%) because they abort early. **36% of runs at ≥95% detOK still diverge (only 44% land) → control-bound residual, cause NOT identified.** ⚠ Funnel identities (user corrected me twice): there is **NO visibility funnel** (visibility = the CBF); `p_r`=combined-barrier position funnel driving σ via ζ_r; `p_s`=SUPERSEDED outer-loop path but still LIVE with an active integrator (`PLASMC_SEN_FUNNEL` default 1); `p`=flow funnel. ⚠ **ζ_r unloaded (r_r~0.098) is BY DESIGN** (`PR0=10`+`XIR=0.10` keep `S_r~0.05` to avoid edge-forcing; **`PRINF` is inert, never binds**). ⛔ FRAMING: **GT-FB already lands moving 3/3 @0.47 m/s and recovers a 0.74 m transient — this is NOT a control tuning problem and the record said so before I started.** Speed was also over-read as causal; it isn't.
> ⭐⭐⭐ [ROBUSTNESS EVAL SET — lighting is FINE; `inv` fails SILENTLY (99.1% detOK, 0% usable)](feedback_cross_detector_robustness_requirement.md) — 2026-09-03: `test_data/RobustnessFrameset` (7 variants, 2743 frames, GT-FB flight, `CROSS_RING_OVERLAY_DBG=0` clean). **CORRECTED baseline** (the first was contaminated — the scorer clamped frames past the GT-log end, 16-42%/variant; DEFLATED clean scenes and FLATTERED `inv`; now guarded by `CROSS_GT_WINDOW_STRICT` in the tool): base 100%/97%, dim 100%/81%, bright 95%/96%, lowsun 100%/97%, darkbg 99.6%/94%, col 62.4%/83%, **inv 99.1% detOK / 0% within-0.15 / 109.9 px off at EVERY band.** ⭐ **Lighting is already met — 8× sun range, grazing sun, dark ground all fine; stop looking there.** ⛔ **`inv` LIES**: `inRange(V≤100)` keeps 94.8% (the PLATE), mask = plate with cross as HOLES, Hough finds arm-like hole boundaries, nothing downstream objects — any detect-rate health metric scores it perfect. **Both front-end attempts FAILED and are default-off in-tree with results**: `CROSS_GATE_MODE=contrast` (Lab local-deviation) regressed clean scenes because the PLATE BOUNDARY is genuinely high-contrast; `CROSS_STROKE_VALIDATE=1` (ridge/valley vs step) left `inv` at 26% and worsened its error. Timing is NOT the constraint (2.6-9.0 ms vs a 33.3 ms budget). NEXT: stage 3 geometry-first confirm, not a 4th gate.
> ⭐⭐⭐ [PRE-EXISTING `_best_pair` BUG FOUND + FIXED (unconditional): the stub got matched into the main arm pair in 41.6% of ALL frames](feedback_cross_detector_robustness_requirement.md) — 2026-09-04, user challenged a flawed "score longer arm only" heuristic ("why not both, they intersect at the same center") and was right. Real cause: a hard support-gate in `_best_pair` (2026-08-01 code, unrelated to this week's work) returns the FIRST support-passing pair, never comparing it against a much-better under-supported one — this marker is an X + a one-sided stub 45° off each diagonal, and the bug let the stub win 12.5% of ALL frames (measured, not the other 29.1 points which have no near-90 pair available at all, a separate Hough-recall problem). **FIXED with soft-penalty scoring** (support becomes a tiebreaker not a veto) — zero regressions across all 7 RobustnessFrameset scenes, insensitive to the exact penalty value. Centroid accuracy stays FLAT (the stub also passes through the true center, so even the wrong pair lands near it) but **heading/alpha availability jumps +11-13pts** on clean scenes (was silently `None` whenever the stub got eaten by a separate downstream step that excludes whatever `(i,j)` already consumed). A 5th confirm stage, `CROSS_BALANCE_CONFIRM=1` (default off, span-balance: a cross center sits INSIDE each arm's span, a corner is the ENDPOINT), landed on top and is complementary to ring — combined `ens_ring_balance` gives dim within-0.15 82→**88** plus deeper inv/col rejection, no new cost elsewhere. **2026-09-05 SITL must-not-regress gate (n=5): fixed 4/5 precise (median xy 0.061) vs legacy 1/5 precise (median xy 0.212), detOK 100% both arms — no regression, fix comes out ahead (n=5, not proof of improvement, but nothing to hold it back on).**
> ⚠ [SITL flight test on cm_dim/cm_lowsun/cm_col: NONE support flipping the confirm stages on](feedback_cross_detector_robustness_requirement.md) — 2026-09-05, n=5/arm, ens_ring_balance vs baseline, perception mode. `cm_dim`: 0/5 landed EITHER arm, confounded by the separate terminal-overfill stall. `cm_lowsun`: no meaningful difference (both arms land reliably, `FAIL` label was misread at first — real controlled landings just missing a strict 0.1m threshold, corrected after checking the raw log). `cm_col`: genuinely MIXED — `on` wins 3/5 reps clearly (median xy ~half of off's) but its WORST rep is a 15.4m fly-away with detOK collapsing to 34% (worse than off's worst, 3.7m/58%) — a real typical-case-gain-vs-tail-risk trade-off, not a clean win. VERDICT: `ens_ring_balance` and all component flags STAY DEFAULT OFF. Plausible col mechanism: being more selective about rejecting bad detections can occasionally leave NO usable measurement during a rough patch — worse than noisy-but-present. NEXT: investigate a rescue path for when every candidate gets rejected, before re-testing at higher n.
> ⭐⭐ [5 independent GT-FB flights on col: the margin-gated rescue improves detOK on EVERY SINGLE REP, zero accuracy cost](feedback_cross_detector_robustness_requirement.md) — 2026-09-05, test_data/GtfbMulti_col/ (5x PLASMC_GT_FEEDBACK=1 flights, ~2200 frames total, ~5x the single RobustnessFrameset col recording). Under GT-FB the trajectory is fixed regardless of detection, so every variant scores against the SAME real flight — no closed-loop confound, unlike the small-n perception-mode SITL A/B above. detOK: ring_balance 54.3%->60.8% mean (+6.5pt, every rep individually positive), ens_ring_balance 55.5%->60.3% mean (+4.8pt, every rep positive). within-0.15 flat to +1pt, NO regression in any of 10 rep-comparisons. Materially stronger validation than the perception-mode test could give (no outliers either direction — not noise). Complements, does not contradict, the earlier finding of a real-but-partial closed-loop tail-risk reduction: this shows the rescue reliably improves what the detector REPORTS; closed-loop LANDING outcome still needs more evidence. Rescue stays landed; ens_ring_balance overall stays DEFAULT OFF.
> ⭐⭐⭐ [Innovation-gated adapt FALLBACK landed AND BAKED DEFAULT ON — Kalman-style fusion of baseline+adapt](feedback_cross_detector_robustness_requirement.md) — 2026-09-05, user-requested ("fuse estimates from multiple approaches using a Kalman-filter method"). baseline/adapt are genuinely INDEPENDENT front-ends (unlike ring/balance/geom, which gate the SAME estimate); adapt detects far more (90.5% vs 65.2% detOK on col) but is far less accurate (77.0% vs 83.4% within-0.15) when it fires. Naive fallback-when-silent is a WASH; gating the fallback on agreement with a held-last-good prediction (a KF innovation test) turns it into a real gain. ⚠⚠ HONEST GAP: idealized offline estimate said EVERY rep improves (82.8% pooled); verified via the REAL process_frame() end-to-end, the shipped result is net-positive but weaker (79.0%->82.2%, 3/5 reps positive, 2/5 mildly negative) -- the idealized estimate assumed adapt runs continuously, the shipped fallback only runs sporadically. Also fixed a real bug found during verification: updating the held reference from an accepted FALLBACK (not just primary) let a less-accurate estimate compound into the next frame's gate, causing a live regression the offline test never had. Generalization checked on rover_IC2/IC4 (oblique clutter, not lighting/colour): both mildly positive, zero regressions. **2026-09-05 FIRST SITL FLIGHT TEST (n=5/scene): ordinary cross_marker must-not-regress PASSED with a bonus (on 3/5 precise vs off 1/5, median xy 0.081 vs 0.105, detOK 100% both arms). cm_col firmed up to n=15 (10 more reps pooled): the effect shrank substantially from the n=5 snapshot -- mean xy is now essentially a WASH (1.947 off vs 1.938 on), on tighter in only 9/15 paired reps, one clear bad outlier for on this batch (xy=4.984, TARGET_LOST). What DOES hold: both arms show the SAME collapse rate (2/15) but on's collapses are milder (66-70% vs 33-45%) -- matching the ring+balance rescue's own col pattern (severity reduced, not eliminated). Neither arm landed in any of 15 reps. Report col as a mild, uncertain positive, NOT the clear win n=5 suggested. The clean win remains the ORDINARY-scene must-not-regress result (untouched, not re-run). ⛔ **User correction (2026-09-05): SP/xy_err is the WRONG metric for this flag** — landing outcome is confounded by control dynamics + the terminal-overfill wall, neither touched by this perception-layer fix; the right metric is altitude-banded detOK. Re-scored offline from the SAME n=15 recordings above: `off` has ZERO valid-detection frames below 2.0m across all 15 flights (every flight loses the marker before getting that close); `on` reaches 1.3-2.0m (n=36, 94.4% detOK there). Reframes the fix as extending survive-with-detection RANGE, not raising in-band detOK — this is a real, band-specific gain the outcome-level wash was never positioned to see. **BAKED DEFAULT ON 2026-09-05 (`f1e93c9a`) by explicit user direction**, ahead of live dim/lowsun coverage (offline-only for those) — `=0` reverts.**
> ⭐⭐⭐ [ROOT CAUSE of `inv` FOUND BY VISUALISING IT: the detector fits the PLATE'S OUTER EDGES and returns a plate CORNER as the junction](feedback_cross_detector_robustness_requirement.md) — 2026-09-03. Rendered overlay on `inv` at 5/4/3/2 m: green arm on the plate's top edge, blue on its left edge, detected centre on the TOP-LEFT CORNER, GT mid-plate. ⭐ Explains EVERYTHING at once: 99.1% detOK/0% usable (a corner is a legitimate intersection), ~110 px error (half a plate-width), and why all 4 stage-3 sub-tests are non-discriminative — **a rectangle's corner is EXACTLY 90°, its two edges are mask boundaries of IDENTICAL width, and it lies INSIDE both spans**. ⛔⛔ LESSON: 4 mechanisms were reasoned from a mental model of this failure and 2 of those models were WRONG when measured; a dozen annotated frames settled it in one pass — **VISUALISE THE FAILURE BEFORE WRITING THE FIFTH MECHANISM**. ✅ FIX THAT FOLLOWS: `CROSS_RING_CONFIRM=1` (default OFF) — a junction has arms RADIATING, a corner has two edges MEETING, so count MASK CROSSINGS on a ring (polarity-agnostic, scale-free). `base` min 6 / mode 10, 46% of `inv` ≤2 → at T=6, **72% catch for 0% cost on base+lowsun**. Best combo `ens_ring`: `inv` detOK 99.1→**42.2%** AND accuracy UP on dim (81→86) and lowsun (97→98), ≤1.6% cost. ⛔ **`inv` within-0.15 is STILL 0% — this fixes the failure MODE (silent lie → honest refusal), NOT detection. Do not report it as solving `inv`.** **2026-09-04 UPDATE: n=5 SITL flight-outcome gate on ordinary `cross_marker` (must-not-regress) PASSED — off 4/5 precise xy 0.086, on 3/5 precise xy 0.077, detOK ~100% both arms, no directional signal (both single failures comparable magnitude, consistent with known lateral stochasticity). The claimed inv/dim/lowsun BENEFIT still needs its own gate on the cm_inv/cm_dim/cm_lowsun world variants (exist, unconfirmed live) — NOT yet run, still DEFAULT OFF.** **2026-09-04 `cm_inv` PERCEPTION-MODE FLIGHT TEST (n=5/arm, detector actually flying, not GT-FB): baseline is 5/5 STUCK at the ~5m start altitude (confidently-wrong plate-corner detection = a STABLE wrong setpoint, not noise — the live confirmation of the offline root cause). `ens_ring` breaks that floor in 4/5 reps (2 nearly touch down), 1/5 reproduces baseline exactly — a real but INCOMPLETE fix. Neither arm actually LANDED in any rep. **FIRMED UP 2026-09-04: n=15/arm pooled across 3 batches — off 13/15 (87%) stuck at the floor (2/15 break out into OTHER bad outcomes, not progress); on 3/15 (20%) reproduce the floor, **12/15 (80%) break out with real descent** (3/15 near-touchdown <0.12m). Neither arm landed in any of 15 reps. ⛔⛔ SEPARATE FINDING: the harness's concurrent-session guard was UNSAFE — checked once before the loop, ko() then did an unconditional kill -9 on every rep with NO re-check, so a second session starting mid-run would be killed SILENTLY (user caught this by direct challenge; could not prove after the fact whether it happened). FIXED in all three *_ab.sh harnesses (9bd79574): ko() now aborts instead of re-killing if SITL survives its own cleanup — that can only mean something else is live. Applies to any future harness of this shape. Still DEFAULT OFF.** Attempts 1-4 (contrast, stroke-validate, ensemble, geom-confirm) all in-tree default-OFF with numbers; geom-confirm is FALSE BY DESIGN here (`STUB_REL_ANGLE_DEG=45` → 41% of CORRECT base detections are a stub+arm pair) and the virtual-plane version was tested too (moves it ~0.1°, do not re-try).
> ⭐⭐⭐⭐ [USER REQUIREMENT: perception must be robust to LIGHTING / COLOUR / TEXTURE / noise](feedback_cross_detector_robustness_requirement.md) — 2026-09-02 user-stated ACCEPTANCE BAR, stricter than "make rover_cross land". Retires patch-a-gate-at-a-time. The pipeline hard-codes TWO absolute assumptions, both "the marker is the only dark thing": `inRange(V<=100)` (the image) and the `centroid_mismatch` mask-centroid proxy (the mask). Target = the locked design: contrast-based POLARITY-AGNOSTIC segmentation → perpendicular stroke-profile validation (ridge/valley vs STEP) → geometry-first confirm, which removes the centroid proxy entirely. ⛔ Two validation constraints: score on ACCURACY + FLIGHT OUTCOME never detect-rate alone, and **the eval set is the bottleneck — robustness is currently UNMEASURABLE** (DetectorFrameset is all one lighting/polarity/texture). BUILD THE EVAL SET FIRST.
> ⭐⭐⭐ [SPAN RESCUE now DEFAULT ON `a53a5f63` — SITL-gated on flat+clutter+rover](project_20260902_spanrescue_sitl_gate.md) — 2026-09-03, interleaved n=5/arm, 0 flakes: **flat PRECISE 0/5→3/5** (xy median 0.143→0.046), **clutter TARGET_LOST 5/5→1/5** (detOK ~11→~53%), rover detOK 48→98% / TARGET_LOST 3-5→0-5. **Overfill rescues 0 in EVERY run** (ceiling holds; pre-ceiling 12.3/12.6 m clutter fly-aways gone). ⭐ MECHANISM: the rescue does NOT improve the fit, it stops DISCARDING good ones — `centroid_mismatch` checks the intersection against the MASK CENTROID which fused foreign structure drags off the junction; rejecting the frame leaves the controller with NO measurement, so the "fly-aways" were the drone flying BLIND, not control instability. The ceiling covers the opposite failure (at overfill the centroid legitimately drifts, so admitting a frame that fails even the ramped 0.72 tolerance is worst-case AT touchdown). ⭐ It helps CLEAN scenes too — flat is 100% detOK in BOTH arms, so the gain is the LATE APPROACH (1-3 rescues/run). ⛔ NOT fixed: clutter still doesn't land (holds at 1.3-4.9 m), and a rover 49.8 m fly-away had ZERO overfill rescues. Raw data `test_data/SpanRescue_Gate/`; revert `CROSS_CENTROID_SPAN_RESCUE=0`.
> ⛔ [SITL needs a Qt5 LD_PRELOAD until the packages are fixed](project_20260902_spanrescue_sitl_gate.md) — 2026-09-02/03: PX4 spawns Gazebo via the `gz sim` CLI, which aborts on `libQt5Quick → undefined symbol QDoubleValidator::validate` because system `libqt5gui5` is esm3 while `libqt5quick5` is esm1 (split ESM stream). `apt --only-upgrade` CANNOT fix it. **Workaround: `LD_PRELOAD=~/cvenv/lib/python3.8/site-packages/PyQt5/Qt5/lib/libQt5Gui.so.5`** (wired into `spanrescue_ab.sh`; anything else launching SITL needs it too). ⛔ My two diagnoses were wrong and are stamped in the file: "stale gz server on world switch" (it was ORDERING — time confounded with condition; interleave) and "broke at 21:41" (no dpkg activity; an accidental preload most likely just stopped being present). The "maybe headless" hypothesis was tested and refuted.
...
> ⭐⭐ [Joint-QP likely does NOT converge near touchdown — first concrete evidence, kappa ratchets into a_u=1274](project_joint_qp_nonconvergence_kappa_ratchet.md) — 2026-09-04: `theta_cone` chatters frame-to-frame (0.16→0.71→0.10→1.02) under a saturated FoV margin, `I_a` swings in sync, `kappa` ratchets 0.09→0.59, terminal blow-up. GT-feedback (not perception noise), yaw signals calm (not Q8-related). Confirms the convergence gap flagged-not-fixed in `CBF_visibility.tex` §4. NOT yet shown to be the general cause — needs a logged residual + corpus check.
> ⭐⭐⭐ [Q8 step 2: existing PLASMC_YAW_OMEGA_D_FF FIXES the turning-rover ceiling ramp-windup](project_q8_omega_d_ff_fixes_ceiling.md) — 2026-09-04, n=3/arm @0.48 rad/s, attribution-verified: e_a lag -136°→-19°, rate tracking 11%→107%, xy 0.844→0.122, tight across all reps. Works by adding AFTER -K_R@e_R (bypasses the sin(Δψ) ceiling) using the ASMC's OWN rate (not an independent estimate, so no double-counting). Scope: constant-rate targets AT the ceiling ONLY — not a general feedforward, not yet gated for a default flip.
> ⛔⛔ [Q8 step 1b: FF is ACTIVELY HARMFUL with real headroom, not just useless at the ceiling](project_q8_yaw_ff_harmful_with_headroom.md) — 2026-09-04, n=3/arm at 0.30 rad/s: `off` already converges e_a≈0-3° unaided; `ff` induces a 60-70° standing lag (u_a −0.196→−1.647, likely double-counting the ASMC's own type-1 tracking of a constant-rate disturbance). NO regime found where FF helps. Do NOT build a w_z feedforward that ADDS to the ASMC — narrows the w_z idea to ONLY bypassing the sin(Δψ) ceiling at high turn rates.
> ⛔ [Q8: perfect ω_t feedforward does NOT fix turning-rover yaw — the ceiling is K_R_yaw·sin(Δψ)≤0.5 rad/s](project_q8_yaw_ff_dead_sin_ceiling.md) — 2026-09-03: arms indistinguishable with the EXACT injected rate fed forward (e_a −159.8° vs −162.1°). `e_R,z`=sin(Δψ) saturates at ±1 so `w_u[2]` is hard-capped at ±0.5 — downstream of ψ_d, so no FF can reach it; and non-monotonic (needs Δψ≥74° to match 0.48, weaker past 90°). NOT windup. Don't bake the knob, don't raise K_R_yaw. w_z regulation survives only because it BYPASSES the sin().
> ⭐⭐ [Thrust deliverability bound was WRONG in TWO places — fixed + BAKED](project_thrust_sphere_bug_and_bake.md) — 2026-09-03 `CBF_SPHERE_TRUE_THRUST` default-ON: `|I_a+g*e3|<=A_CAP` bounds VEHICLE accel (zero at hover), admitting 2.39 g on a 1.389 g vehicle. Existed in `cbf_visibility.py`'s sphere AND `controller.py`'s downstream cap — fixing one left a 15.14 m/s² command passing. A_CAP's own comment was the wrong spec, which is why it was coded wrong twice. Validated as an INVARIANT: 17 over-cap samples → 0.
> ⭐ [d_s=0 HOLDS for the cross marker — stronger than the paper's argument](project_ds_zero_holds_for_cross_marker.md) — 2026-09-03: the arm-line intersection is EXACTLY tilt-invariant (homographies preserve incidence), vs the N=4 centroid which is not (they aren't affine). Verified numerically: intersection err flat 0.01-0.08 px across 0-40° tilt vs centroid 0.0→1.35 px. The stub never enters `s`. Don't add a d_s term; contamination is an OUTLIER regime to gate, not a bounded disturbance.
> ⛔ [ALL-`landed=NO` sweeps: two environment causes, neither is the code under test](feedback_gz_needs_qt5_ld_preload.md) — 2026-09-03: (1) gz needs `LD_PRELOAD=.../PyQt5/Qt5/lib/libQt5Gui.so.5` (run_aruco_landing.sh lacks it; the one-off harnesses have it) → "Timed out waiting for Gazebo world"; (2) an ORPHANED SITL holding udp:8888 → "bind error errno 98" — see [[feedback_check_concurrent_sitl_before_launch]] 2026-09-03 §, incl. why `pkill -f wrapper.sh` leaves the child harness alive and why `grep -av grep` is load-bearing. Cost 2.5h / 4 dead runs.
> ⭐⭐ [WHOLE-PROJECT ARC — read for orientation](project_overall_arc_and_phase_history.md) — MATLAB sim -> PX4 perception tuning (largely cal/gain-bug CONFOUNDED) -> GT-feedback as a PERMANENT ablation tool (not a past phase) -> marker evolution driven first by CONTROL failures -> cross-marker -> now rover + robustness; parallel HARDWARE track since 07-10. Corrects the natural "control done -> perception now" summary.
> ⭐ [TWO independent output cals — cross-marker recalibrated for 320x240, ArUco NOT](project_two_output_cals_aruco_vs_cross.md) — 2026-09-03: "we recalibrated the output cal" means `cross_marker_perception.py:797` (08-28); `img_data.py:186` still sits at the 07-17 640x480 cal. **User DECLINED the ArUco recal — do not re-raise.** Carries the CALIB_FREQ_HZ lever if it is ever wanted.
...
- [⭐⭐⭐ 2026-09-18 CONFIRMED: `PLASMC_YAW_RATE_LAW` default (kp=0.3) is the root cause of the cross-marker rover r=0.8 curve failure](project_20260916_curve_qgate_revalidation.md) — discriminator A/B (n=4/arm, `WORLD=rover_cross MARKER_TYPE=cross ROVER_CIRCLE_R=0.8`, GT-FB, no lead; every rep confirmed on the real curve via Kåsa fit `r_fit` 0.85-0.89 m): **default (`YAW_RATE_LAW=1 kp=0.3`) landed 0/4** (final xy_err 0.76-1.20 m, uav_z 0.25-1.67 m — crashes through the platform) **vs forced `PLASMC_YAW_RATE_LAW=0` landed 4/4** (xy_err 0.035-0.128 m, uav_z consistently 0.51-0.52 m — correct altitude, near-baseline precision). ⛔ 2026-09-21 user correction: do NOT force `PLASMC_YAW_RATE_LAW=0` as the working fix — that was diagnostic-only; the law's feedforward (perception-derived target yaw rate) is needed to actually track a turning target, which pure ASMC heading-hold cannot do. Correct fix/how to apply: re-tune `PLASMC_YAW_RL_KP` down from 0.3 (MATLAB validated `0.02`, unvalidated on PX4 — not yet run). Confirms the same-day cross-pollinated MATLAB lead (`P.yrl_kp=0.3` independently found to destabilize Circular tracking via an image-position/orientation coupling runaway; MATLAB commits `6a7d7dfb`/`ee7bbd46`) — PX4's cross-marker rover runs had this yaw-rate-law active (gated to `MARKER_TYPE=cross`) the whole time because the "heading-hold" recipe only zeroed the (inactive on cross-marker) ASMC gains, so those tests were never actually heading-hold. Also explains the marker-type asymmetry that two now-retracted hypotheses (GT pose-jitter/chase-cam — physically invalid under Gazebo LOCKSTEP sim time, [[project_20260826_chasecam_resolution_bump]]; cross-marker yaw/alpha sign convention — already resolved 2026-09-09, [[feedback_gtfb_wz_sign_bug]]) were chasing: it's a yaw-control gain-path selection, not perception quality. **Open follow-up:** re-run the `cycle_isolation` gain-revert sweep on cross-marker WITH `PLASMC_YAW_RATE_LAW=0` forced (the earlier ArUco version was null but confounded). Separately: ALL ArUco-rover curve results from 2026-09-16/17 (r=0.8/r=10 QGATE A/Bs, the old cycle-isolation sweep, `d380901c` worktree test, camera-resolution test, `edb546f0` bisect) were DELETED at user instruction — ArUco is obsolete for the moving-rover scenario, do not cite their numbers. Surviving code fact: `ROVER_TRAJ=Circular`'s DEFAULT (`ROVER_CIRCLE_R=10`, commit `b816fea0`) is a gentle 9-14 m/11-28° arc, not a curve — set `ROVER_CIRCLE_R=0.8` for the real turning-target case. All stationary IC1-5 work is unaffected (no rover involved).
...
- [⭐⭐ B4 AU_LEAD: 2-term perception+magnitude gate restores baseline parity (18/25, p=0.797 — NOT 'beats'); audited 2026-09-16, two claims retracted (2026-09-09→12)](feedback_aulead_stationary_regresses.md) — concurrent A/B `WORLD=cross_marker`, IC1-5 n=5. **PERCEPTION path:** baseline 20/25 precise (xy 0.02-0.09 m); `PLASMC_AU_LEAD=1 RATIO=0.5` (ω_z/ω_p 0.9/3.5) **2/25 precise (xy 0.11-0.21 m, 2-6× worse)**, all land, no TL/fly. Perception log dive first blamed a control-side κ-ratchet HF pump. **GT-FEEDBACK A/B REFUTES that** (no regression IC1-4, κ decays, AU_LEAD even rescues fragile GT-FB IC5) ⇒ real mechanism: the ×3.9 HF lead amplifies terminal extent-saturation centroid/flow corruption. κ-ODE dead-zone ([[feedback_adaptive_law_noise_behavior]]) confirmed this is upstream of κ. **FIX 1 (2026-09-12): `PLASMC_AU_LEAD_QGATE`** attenuates the lead by `MARKER_EXTENT_PX/frame_min` fill-fraction — fixed IC1-4 (13/20 ≥ baseline 12/20, vs 3/20 ungated) but **IC5 (3 m start, steepest descent) stayed 0/5**: its FIRST 30-40% of flight already runs `|I_a_raw_xy|` ~2× hotter than other ICs (early-phase mean 0.89 vs 0.35-0.50), unrelated to extent (which hasn't saturated yet) — the lead over-amplifies that. **FIX 2 (same day): 2nd independent gate term on `|I_a_raw_xy|` itself** (`PLASMC_AU_LEAD_QGATE_MAG_LO=0.5`/`_HI=1.2`, combined gate = product of both terms) — **IC5 FIXED: 0/5→5/5, mean xy 0.051 m.** Full result (bundle `ICValidation/20260912-040029`): IC1 3/5, IC2 4/5, IC3 5/5, IC4 1/5 (one hard rep that never reached the surface, gate barely engaged, extent tiny — signature doesn't match the AU_LEAD pattern, looks like an unrelated flake, unconfirmed at n=5), IC5 5/5 → **18/25 total, beats the 17/25 ungated baseline.** ⚠ **OPEN, load-bearing before any bake:** the curved-target use case this lead was built for ([[project_rover_turning_open]]) runs on a SUSTAINED `|I_a_raw|`~1.0-1.5 — the SAME range the new magnitude gate suppresses; instantaneous magnitude can't tell IC5's transient spike from the curve's sustained demand. MUST re-validate the curved-target benefit with both gate terms on before baking; if neutered, the magnitude gate needs a persistence/duration test (rolling mean, still-transient latch) not instantaneous `|I_a_raw|`. Also: retest IC4 at higher n. ⚠ `run_ic_validation.sh` doesn't set WORLD/MARKER_TYPE → always export both. **⛔ 2026-09-16 AUDIT ([[feedback_session_20260909_12_audit]]) RETRACTS TWO CLAIMS FROM THIS ENTRY:** (1) the GT-FB 'AU_LEAD rescues IC5' half is CONFOUNDED (GT-FB arms ran unmatched `CBF_DRIFT_TAU` 0.0 vs 0.15 — a peer re-baked the default between them); (2) '18/25 beats 17/25' is p=1.000 — the identical baseline spans 17-20/25 across 3 runs, so <3/25 is noise. Also IC5 is the EASIEST perception IC (15/15 baseline), not the hardest; IC4 is (8/15). [[feedback_reject_on_single_failure]] [[feedback_adaptive_law_noise_behavior]]
- [⚠ κ-ODE noise behaviour + KAPPA_DZ dead-zone: real floor, TESTED, does NOT help landings (2026-09-11)](feedback_adaptive_law_noise_behavior.md) — the translational κ-ODE growth term `θNG·|σ|` uses RAW rectified `|σ|` (no dead-zone/filter) → perception noise gives `E[|σ|]≈0.8·σ_n>0` = standing forcing → κ settles at floor `θG·0.8σ_n/P`, not 0. Measured (IC1-4 xy, HF std of σ): GT-FB σ_n≈0.002 (κ decays); perception ramps 0.006→0.037→0.057 over descent; +AU_LEAD → 0.093 (~1.6×) + sat-rail onset → END κ flips decay→+0.198 growth. **Added `PLASMC_KAPPA_DZ_{X,Y,Z}` (dead-zone on growth term, default 0 = bit-identical). 4-arm gate n=5: DZ=0.06 ENGAGES (halves END κ-growth +0.11→+0.05, κ_peak 0.22→0.12) but landings 16/25 vs 19/25 baseline (IC4 regressed 2/5→0/5, δ starves terminal-braking κ build-up) and does NOT rescue AU_LEAD (D 4/25 ≈ C 2/25 — AU_LEAD's damage is upstream of κ per the GT-FB refutation).** Verdict: κ noise floor is real, dead-zone provably suppresses it, but it's a SECONDARY lever — do NOT bake `KAPPA_DZ`; left in code inert. Levers ranked in-file (dead-zone / P_xy leakage / N_xy bandwidth / E_xy). Bundles `test_data/ICValidation/{20260911-003120, -005506, -011851}`.
...
- [⭐ NEGATIVE: the 2026-09-10 XIR/XI2 handoff does NOT transfer to SITL (tested 2026-09-11)](feedback_xir_xi2_handoff_refuted_sitl.md) — docs/HANDOFF_xir_xih_moving_target.md candidate PLASMC_XIR_{X,Y}=0.20 + PLASMC_XI2_{X,Y}=0.20 (vs baked 0.10/1.0). GT-FB rover-world A/B (n=3, moving+static, IC1+IC2): baseline ALREADY lands 3/3 precise on moving IC2 (xy~0.04, terminal s_e_n~0.1, no growth-back) → the MATLAB funnel-deficit failure does not reproduce in SITL. Candidate: moving IC2 2/4 precise (p_r collapses ~1.9), moving IC1 1/3 DETONATION (kappa_xy 0.5→30=cap, a_u_xy 1.3e5, xy 12m = the [[project_ic1_kappa_leakage_drift_20260721]] fly-away the handoff flagged), static neutral-to-worse. KEEP XIR=0.10 / XI2=1.0; moving-precision lever is NOT the funnel rates. Reinforces [[feedback_matlab_gains_not_portable]], [[feedback_px4_gains_authoritative]].
- [⭐ NEGATIVE: the thm:translational R_k(t) compatibility-margin hypothesis does NOT explain Xi_r's non-portability (tested 2026-09-12)](feedback_rk_compatibility_hypothesis_refuted_sitl.md) — computed R_k(t)=C_k(t)/|eta_nu_z(t)| directly on test_data/XirXi2_RoverGTFB/ SITL logs (not inferred). min R_k(t)>1 holds in 22/24 reps, 1.5-5.6x margin, same structural pattern as MATLAB (conservative C_k(t)<1 ~100% of flight, tight R_k(t)>1 throughout) — the ONE dip (moving_base_IC1 rep0, R_x→0.746) is a touchdown-window transient (last 0.3s of a 10s run), not a breach precursor (no target_lost in any of the 24 runs). Candidate arm (XIR=0.20/XI2=0.20) has LARGER R_k margins (3.5-5.6x vs base 1.5-2.3x) yet performs WORSE (IC1 detonation at R_x=1.18, still >1) — margin doesn't track outcome. Xi_r's real non-portability cause is elsewhere (likely [[project_ic1_kappa_leakage_drift_20260721]]), not this boundedness condition.
- [⭐ CBF-QP slack validated on PX4 (needed, tail-only); MATLAB unloggable from existing data (2026-09-14)](feedback_cbf_qp_slack_validated_px4_unlogged_matlab.md) — 352-rep scan of `vis_slack(t)`/`vis_active(t)`: successful landings (n=146) engage the Tier-1 QP 31% of the time but slack stays ~0 (2/146 >0.1, max 0.19) — the hard constraint alone almost always suffices. Failed reps (n=206) engage 61% and slack grows large (p90 0.62, max 22.1) but ALWAYS at the very end of an already-diverging trajectory (checked 5 worst: target_lost, xy up to 12.3, the moving_cand_IC1 kappa-detonation rep) — slack is a SYMPTOM of a bad state, not a rescue. Design intent (graceful degradation over hard infeasibility) genuinely validated for the tail. MATLAB: `cbf_visibility.m` computes `s_star`/`cbf_ok` but `run_simulation.m` never logs it — zero .mat datasets carry it, can't be checked without a code addition + a user-run gate.
- [⭐ FIRST moving-target GT-FB result on cross-marker (rover_cross): 3/3 PRECISE Circular, far beats the stale July ArUco baseline (2026-09-16)](project_20260916_rovercross_gtfb_circular_first_result.md) — no moving+GT-FB+cross-marker data existed before (every prior moving-GT-FB result, incl. the July 2026-07-02/03 campaign and my own XirXi2_RoverGTFB sweep, defaulted to `rover_aruco`; `cross_marker`/`rover_cross` files postdate July by a month). `test_data/RoverCross_GTFB_Circular/`: baseline config (no AU_LEAD), IC2, Circular, `WORLD=rover_cross MARKER_TYPE=cross`, n=3 → **3/3 precise** (xy 0.043-0.081m, 0 target_lost, min_alt~0.51m, confirmed ~4.5m real motion span) — vs the July ArUco baseline's 0/4 (1.0-1.7m miss, yaw-ramp windup, [[project_rover_turning_open]]). Attributed to `PLASMC_YAW_RATE_LAW` baked default-ON 2026-09-09 (a week after July) fixing the exact windup mechanism that broke the old baseline — NOT purely "cross-marker transfers cleanly," landing on a since-fixed bug. n=3/IC2/Circular-only; don't over-generalize to other trajectories/ICs without testing.
...
- [⭐ test_data/Final/ — curated IC1-5 landing recordings, REPLACED 2026-09-23 (85faa9f1)](reference_final_landing_recordings.md) — now GT-FB rover_cross-world cross-marker, 5/5 SoftPrecise (xy 0.009-0.030m, rel_vel 0.095-0.111m/s), 1st launch attempt each, all clear the manuscript's strict 0.08m/0.2m/s thresholds. Replaces the perception-mode set (1/5 verified precise per the 2026-09-02 correction, kept in the file as history + recoverable from git pre-85faa9f1) — the GT-FB set landing all 5 cleanly at the same spawn positions is evidence IC1/3/4's old misses were perception-side, not control-law.
- [alpha_0 vs alpha_ref](reference_alpha0_vs_alpharef.md) — alpha_0 = perception CALIBRATION offset (raw principal angle -> 'alpha, 0=aligned'); alpha_ref (=_s_d[3]) = the CONTROL SETPOINT the yaw controller drives alpha to (default 0, DES_ALPHA). Different layers.
- [Yaw-rate law vs ASMC — the sin(dpsi) ceiling mechanism](feedback_yaw_rate_law_vs_asmc_mechanism.md) — ASMC routes the yaw-rate cmd through e_R[2]=sin(dpsi), bounded+non-monotonic, caps |w_u[2]| at K_R_yaw=0.5; new law deletes psi_d/e_R, commands the rate directly + -w_z is a free target-rate FF. Measured on the 0.48 rad/s spin gate.
- [⭐ Visibility-CBF + touchdown-detect + soft-touchdown session (2026-09-17→22): axis-fix BAKED, flow-freeze BAKED, soft-touchdown perception bug CONFIRMED+UNFIXED](project_20260917_visibility_predictor_residual.md) — SESSION CLOSED, index at file top. (2) axis-transposed CBF barrier found+fixed+SITL-validated (96271ba6). (3) touchdown-detect flow-freeze false-positive found+fixed+SITL-validated (2177670b, 22%→0%). (4) the ~5.6x reconstruction gap that drove mechanism-hunting was the INVESTIGATION'S OWN dt bug (fixed a1ffbf02) — the terminal h_z error itself is real (GT+video confirmed, marker overfills frame) but 5 candidate fixes all ruled out/insufficient; converges on a flow-Jacobian model-linearization limit, not yet proven; touchdown-detect already ignores h_z here by design.
- [⭐ Rover moving-target SP investigation 2026-09-22](project_20260922_rover_moving_sp_investigation.md) — static 5/5 SP; Circular R=0.8 0/35 across law/KP/retune/law-off; R>=1.6 8/9 precise 0/9 soft; ~1 s lateral lag + ungated descent -> terminal a_u blow-up. Supersedes the 09-18 '4/4 law-off' discriminator (it zeroed YAW_*).
- [⭐ FIXED+VERIFIED+RE-RUN: rover_drive.py now paces off sim /clock, not wall-clock](project_20260922_rover_drive_wallclock_pacing_bug.md) — was racing ahead of Gazebo's true sim time, so measured GT rover speed diverged 28-250% from commanded value. Re-run of [[project_20260922_rover_moving_sp_investigation]]'s sweeps under the fix: Sinusoidal's earlier catastrophic 0/3 FAIL was a CLOCK ARTIFACT (now 3/3 precise); EightShape now lands 2/3 SOFT+PRECISE (first moving-target SP found); Circular R=0.8 at EXACTLY MATLAB's own validated wz=0.48 baseline still fails 0/3 -- cleanest evidence yet of a real PX4/MATLAB platform gap; Lissajous still shows an unresolved ~3x speed-tracking gap even post-fix (separate mechanism, not this bug).

- [⭐ Lissajous RESOLVED (k=0.1, SP achieved) + CBF-vs-divergence-trigger mechanism finding](project_20260922_lissajous_cbf_and_divergence_mechanism.md) — ⛔ CORRECTED by the entry below (SP rep = parked rover). Was: going slow enough (~0.14 m/s) landed genuine SOFT+PRECISE (xy=0.013m), live default. Mechanism dig along the way: GT-FB does not isolate the CBF (marker_center_px is live-perception); vis_active tracks failure severity across reps at higher speeds, but a within-rep trace shows the actual divergence trigger is h_d_z pinned regardless of lateral error + I_a_z cannibalized by I_a_xy — the SAME mechanism as the main rover investigation, just never triggered at low-enough speed. Other profiles (Circular/Sinusoidal/EightShape) may not have the same easy speed-floor fix — still need controller.py (descent gating / protect I_a_z) for a general solution.
- [⭐ RESOLVED: Ackermann rover park/loop bug -> ROVER_CTRL=vel fix -> genuine 2:3 Lissajous promoted to Final/VISTA-GT](project_20260922_ackermann_rover_loops_not_tracking.md) — root cause: PX4 offboard POSITION mode parks within NAV_ACC_RAD=0.5m and loops at ~2 m/s (invalidated all Lissajous_final speed-retune conclusions; always compute target speed from Ground_Truth.npy, never trust ROVER_SPEED_MULT). Fixed via rover_drive.py velocity-tracking (ROVER_CTRL=vel); then found+fixed a drone-tilt crash at high speed (ROVER_LISS_RADIUS_MULT), chase-camera framing (ROVER_VEL_ROT_DEG), and a w1:w2~1:1 ellipse-not-Lissajous shape bug (corrected to 2:3). Final validated config gives a precise landing (xy=0.111m) with a genuinely curving path -- read top-to-bottom, includes 2 corrected self-made math errors (period formula, false cusp-caused-crash claim).
- [⭐ 2026-09-23 SESSION MAP: final videos + 4-baseline campaign + cho2022 root cause](project_20260923_session_summary_final_videos_baselines.md) — start here: what was delivered (VISTA-GT symbol-aligned videos; LIN2022/ZHANG2026/LIN2023/CHO2022-GT with manifests; landed 6/10, 10/10, 4/10, 1/10, 0 precise+soft), bugs fixed (montage argmin touchdown, B_T saturation, stale zf 0.3->0.2), what's still open (cho2022 Linear landing cause unverified, n=1 noise, use_sq_comp dead code), and process lessons (background SITL, never broad pkill -f, check for peer batches, verify before writing).
- [Manuscript video symbol convention: h->nu, ᴵ calligraphic font, no PiP captions](project_20260923_manuscript_video_symbol_convention.md) — VISTA-GT's 10 montage/overlay_h videos regenerated to match `Soft_Precise_Landing/Figures/rover_cross_circular_*` (`mathtext.fontset="cm"` REQUIRED for `\mathcal{I}` to render calligraphic; `h=` -> `ν=`; PiP bottom-left `pip_label`/`pip_label2` captions removed from `tools/make_landing_montage.py`). Hit + caught a concurrent-session Lissajous-recording-replacement mid-batch; see [[feedback_recurring_analysis_mistakes]].
- [⭐ Comparative-baseline campaign: 40/40 recorded, cho2022 0/10 descent-stall](project_20260923_comparative_baseline_campaign.md) — lin2022/zhang2026/lin2023/cho2022 x IC1-5+Static/Linear/Sinusoidal/Circular/Lissajous, ONE attempt each (no SP gate), under `test_data/Final/{LIN2022,ZHANG2026,LIN2023,CHO2022}-GT/`. zhang2026 lands all 10 (imprecise, xy 2.6-12.2m); lin2022/lin2023 land ~half; **cho2022 hits an identical descent-stall abort on ALL 10 cases** — see the root-cause entry below.
- [⭐ ROOT-CAUSED: cho2022 never lands — converges to stable hover, never reaches physical contact; PLUS the Ls/B_T blow-up mechanism near touchdown](feedback_cho2022_never_lands_rootcause.md) — Cho2022's FF-IBVS law is a pure feature-error regulator with a fixed ~0.4m depth setpoint; once converged, `I_a` settles to EXACT hover (verified in Control_Data) and just parks there. Baseline runs also bypass `self.PLASMC()` entirely, so the loom-inversion touchdown latch never runs; the only remaining landing signal (PX4 LandedState / accelerometer impact spike) needs GENUINE physical contact, which a law converging smoothly to zero velocity never produces. Other baselines land because their laws overshoot past hover into hard contact (zhang2026 rel_vel up to 18.5 m/s). **Follow-up:** quantified WHY a shallower depth target's diagnostic rep landed via a B_T blow-up to millions — `Ls` cond number grows ~40x (10→434) as z→0.01m floor (not enough alone); `use_sq_comp` is dead code (N always 5, never 4); the real smoking gun is `accel_to_rate_thrust`'s `B_T` dividing by `cos(roll)*cos(pitch)` of the ACTUAL attitude with only a `1e-6` guard — a near-gimbal-lock real tilt event explodes it (back-of-envelope math matches the observed ~-2.9M almost exactly). Unique to cho2022 among the 4 baselines (only one with a raw unratioed image-Jacobian 1/z division). **2026-09-23 later: B_T saturation FIXED** (`accel_to_rate_thrust` now clips to [-11.08,31.22]N, matching the downstream thrust_norm clip exactly) and re-tested — B_T confirmed bounded (was ±3M, now [-1.91,+3.17] across a full flight) but cho2022 STILL aborts (expected — the blow-up was a symptom, not the cause). **Also fixed a separate user-caught stale constant:** `BASELINE_ZF` default was 0.3, MATLAB's `Constants.m` canonically defines `zf=0.2` — fixed (affects both cho2022 AND lin2023, both `needs_features=True`). The `0.01` z-depth clamp floor was checked too and DOES match MATLAB exactly (not stale).
- [⭐ FIXED: make_landing_montage.py touchdown-detection bug (argmin(uz) on never-landed runs desyncs plots vs video)](feedback_montage_touchdown_argmin_bug.md) — user-reported plot/chase-cam mismatch, worst on cho2022 (0/10 landed). Root cause: argmin(uz) locked onto sensor noise during a flat hover instead of a real touchdown, truncating BOTH the plots and the montage's own video length (CHO2022-GT/Circular: 24.6s→37.0s after fix). Fix gates the trim on `Ground_Truth.npy['SoftPrecise'].get('xy_err') is not None` (the harness's own landing signal) instead of raw argmin. 17 montages regenerated; also surfaced LIN2022-GT/IC3 missing its onboard recording entirely (file dropped, not left broken). Shared-tool fix — applies to future VISTA-GT regens too.
- [New tool: 2x2 baseline-comparison montage per target profile (chase-cam+plots only, Times New Roman, white title + yellow small-caps status tag)](project_20260923_baseline_2x2_comparison_montage.md) — `tools/make_baseline_comparison_montage.py` -> `test_data/Final/Baseline_Comparison/<profile>_2x2.mp4`; panel order PBVS-PPC/PBVS-AEDO/IBVS-PPC/FF-IBVS; result: 0 soft-precise/hard-precise/soft-imprecise across all 20 reps used, every panel hard-imprecise or aborted
- [⭐ Perception-s terminal gap = close-range s NOISE (<0.5 m), NOT staleness](project_20260924_terminal_perception_rate_drop.md) — sperc 6/25 vs GT 25/25 manuscript SP; GT s held at perception timing (PLASMC_GT_S_HOLD=stamp) = 25/25; the 49→12 Hz terminal rate drop is real but harmless
- [⭐ Stroke detector: STATIONARY SOLVED (25/25 SP, xy=GT), moving rate-bound; deck GT marker-height bug FIXED](project_20260924_stroke_detector_rewrite.md) — CROSS_DETECTOR=stroke; sperc stationary 6/25->25/25; Sinusoidal 2.5 m->0.17 m (gt 0.04), limited by 48-80 ms live frame gap; deck GT used marker_dz 0.5 (true 0.201) — fixed in run_rover_landing.sh
- [⭐ RESUME HERE: pending tasks after perception-s investigation (defaults baked)](project_20260925_pending_perception_s_tasks.md) — n=5 pure-perception gate, moving pure-perception, out-of-process perception, re-record deck videos
