...
> ⭐⭐⭐ [Line-width loom investigation (width itself: LANDED+validated) + origin_ratio veto (⛔ REGRESSED, PERMANENTLY DEFAULT-OFF — do not re-attempt) (2026-09-08)](project_20260908_line_width_loom_investigation.md) — width-loom: found+fixed 2 real bugs in the existing width machinery, replaced with a direct mask-thickness scan, **validated 0.89-1.00 corr with GT altitude across all 5 ICs (7 reps), landed as shadow-mode `"Width Loom Px"`** (not consumed by control). Width-loom RATE (KF derivative for control use): built, swept 64 q/r combos, **no usable signal found — negative result**. `origin_ratio` double-gate: the restructuring **CAUSED A CONFIRMED CRASH REGRESSION on every off-center IC** (2 independent sessions, 12/12 correlation) — reverted (`CROSS_TZ_VETO_R_MULT` 1e6->1.0, no-op). Attempted relative-drop+streak fix, then tested it against a REAL 15-rep off-center dataset (IC2/3/4, n=5 each): **DECISIVELY FAILED — 24-45% false-veto rate across a full threshold/streak/smoothing sweep, on CLEAN validated landings.** `origin_ratio` genuinely swings by large relative factors on sub-second timescales throughout ordinary off-center flight — this is real signal volatility, not noise a smarter gate can filter out. **`CROSS_TZ_VETO_R_MULT=1.0` is now the PERMANENT verdict, not a holding pattern — do not re-attempt origin_ratio-based Tz vetoing in any form; the underlying `M0/||c0||^2` quantity structurally conflates off-center position with conditioning collapse.** If Tz reliability needs fixing, the width-loom direction (position-independent by construction) is the path, once its rate-signal gap is solved.
