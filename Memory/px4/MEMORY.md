...
- [⭐ FIXED+VERIFIED: rover_drive.py now paces off sim /clock, not wall-clock](project_20260922_rover_drive_wallclock_pacing_bug.md) — was racing ahead of Gazebo's true sim time via asyncio.sleep() accumulation, so measured GT rover speed diverged 28-250% from commanded ROVER_SPEED_MULT/formula value; affected every rover-profile speed number logged via this launcher, incl. [[project_20260922_rover_moving_sp_investigation]]'s trajectory sweep (treat those speed thresholds as provisional until re-measured). Fix verified live: Sinusoidal measured 0.579 vs commanded 0.576 m/s (was 0.74).
