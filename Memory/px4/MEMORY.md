...
- [⛔⛔⛔ OVERTURNED 2026-09-17 — my "the curve is no longer open" re-stamp was WRONG; the limit cycle is ALIVE on HEAD and every Sep "curve" test ran on a NON-CURVE](project_rover_turning_open.md) — `ROVER_TRAJ=Circular` stopped driving a circle on 2026-07-03, commit **`b816fea0`** (`ROVER_CIRCLE_R` 0.8 m→10 m, its own comment *"only the path curvature drops ~12x"*). **Circle-fit on GT Target Pose, independently verified by me (Kåsa, n=30):** July `yawhold_arm_n3`/`aulead_*` **r=0.87-0.88 m, 230-242° swept** vs EVERY Sep run (`cycle_isolation`, `qgate_revalidation`, `worktree_d380901c`, `XirXi2`, `RoverCross`) **r=9.9-13.9 m, 13-28°** — a gentle arc. HEAD unmodified + `ROVER_CIRCLE_R=0.8` → **cycle RETURNS** (median |e_rot| 0.70, July's band, one rep missing by 7.20 m, `test_data/Rover_Turning/r08_confirm/`). ⇒ "curve lands now" = the r=10 GENTLE ARC lands (already-solved straight-line case); "cycle gone" = **null stimulus, not null effect**; "cause is outside the repo" (`e7882829`) = FALSE, it's in-repo, that bisect was anchored by DATE onto a commit 11 h after the change; the camera-SDF hypothesis is separately FALSIFIED (640×480 on HEAD → e_rot +0.04..+0.07) — my loop-timing table predicted that correctly. **The July analysis is NOT superseded — it is the live description of a live problem** (χ>Wτ pump, branch audit, AU_LEAD exit, bias↔cycle frontier all stand). ⚠ **To exercise the cycle you MUST set `ROVER_CIRCLE_R=0.8`; default r=10 is a smooth-tracking test, NOT interchangeable.** ⛔ METHOD LESSON (mine): my comparability check used heading-sweep from `unwrap(atan2(dy,dx))` on gradients — it reported the near-STRAIGHT Sep paths as turning MORE (16-26 rad) than July's real circles, because on a straight path heading is noise and unwrap accrues spurious 2π. I had called that metric noisy in the same session and leaned on it anyway. **A comparability check that licenses pooling two datasets must be a robust statistic.** Overturn by `soft-precise-landing-29` (`360e3bca`); primary record [[project_20260916_curve_qgate_revalidation]]. Original July entry follows ↓
...
- [⭐⭐⭐ CURVE RE-VALIDATION 2026-09-16: QGATE does NOT neuter the curve (BLOCKER CLEARED) — but the curve LIMIT CYCLE IS GONE from the baseline](project_20260916_curve_qgate_revalidation.md) — GT-FB Circular, campaign recipe verbatim, n=4/arm (`test_data/Rover_Turning/qgate_revalidation/`). **(1) Gate is harmless on the curve:** gated vs ungated lead — e_mean 0.253 vs 0.250 (p=0.72), touchdown lat p=0.16, osc_std p=0.49, both **4/4 ON-PLATFORM**; gate transmits ~33% through the 0.8-3.5 m window, enough to keep the benefit. The feared magnitude-gate conflict does NOT materialise — measured tracking-window `|I_a_raw|` median is **0.44-0.49**, not the 1.0-1.5 the old memory recorded, so the magnitude term sits mostly open (0.73). ⇒ AU_LEAD + both QGATE terms is defensible for the rover scenario. **(2) ⚠ PREMISE CHANGED:** the no-lead baseline now lands the curve — **3/4 ON-PLATFORM, lat 0.130 m, e_rot +0.05, osc_std 0.033** vs July's **0/4, 1.0-1.7 m, e_rot +1.11, 0.06-0.07**. The self-sustained rotating cycle AU_LEAD was built to damp is **essentially absent** on the current stack; the lead is now a modest refinement (e_mean 0.345→0.250, −27%, p=0.0001), not a rescue. **⛔ CAUSE STILL UNIDENTIFIED (I claimed the rewrite did it — REFUTED by worktree test, see below).** Isolation sweep (`test_data/Rover_Turning/cycle_isolation/`), each July→today lateral change reverted one at a time, no lead: `CBF_DRIFT_TAU`=0, `P_xy`=1.5, `P2INF_xy`=1.0, `XI2_xy`=0.7 — **all four NULL, 13/13 ON-PLATFORM, e_rot never above +0.10**. Positive mechanism: the OLD cone rotated the lateral command **0.345-0.473 rad at 100% duty** through the tracking window vs the new QP's **0.058-0.106 rad, 0-16% active** — and the original campaign itself named that cone *"the DF that caps growth"* / *"amplitude is authority-set"*. A limit cycle needs a nonlinearity to set its amplitude; the chattering cone was it. ⛔ **WORKTREE TEST AT `d380901c` REFUTES THAT:** old cone verified live at July magnitude (theta_cone 0.358-0.478) yet **e_rot +0.05..+0.21, NOT July's +1.11** — the rotating cycle does NOT reproduce on pre-rewrite code either. What the rewrite DOES do on the curve is **eliminate tail variance** (OLD 1/4 ON-PLATFORM, lat 0.018-5.90 m bimodal, e_mean sd 0.691; NEW 3/4, lat 0.030-0.352, e_mean sd **0.006**) — matching the peer's stationary VisProjGate "NEW wins the TAIL" finding. ⚠ n=4, nothing reaches p<0.05 (Fisher 0.486, Welch 0.077-0.117). **TWO phenomena were conflated:** (a) July's rotating cycle — gone in BOTH old and new code, cause UNKNOWN; (b) old-stack tail-thrashing — fixed by the rewrite. AU_LEAD redundancy stands directionally (cycle is gone regardless of cause) but NOT on an established mechanism. Data `test_data/Rover_Turning/worktree_d380901c/`. ⛔⛔⛔ **2026-09-17 — ALL OF THE ABOVE CURVE WORK RAN ON A NON-CURVE.** `ROVER_TRAJ=Circular` has not driven a real circle since **`b816fea0` (2026-07-03 12:13) set `ROVER_CIRCLE_R` 0.8m→10m** (target paths measure 9.4-12.1 m radius / 11-24° arc vs July's 0.86 m / 234°). **Today's HEAD with `ROVER_CIRCLE_R=0.8`: median|e_rot|=0.70, r_fit 0.88 m — CYCLE RETURNS**, July's band, one rep missing 7.20 m (`test_data/Rover_Turning/r08_confirm/`). **The curved-target limit cycle is ALIVE on HEAD — never fixed, never re-tested.** OVERTURNS: "cycle is gone", "AU_LEAD is redundant", "QGATE doesn't neuter the curve" (p=0.72 — measured on a straight path, **question OPEN again**, rerun the 3-arm A/B at r=0.8), and the bisect-abort conclusion below. **All STATIONARY work is unaffected.** ⚠ To exercise the cycle you MUST set `ROVER_CIRCLE_R=0.8`; default r=10 is a gentle-arc test. ⛔⛔ ~~BISECT ABORTED — CAUSE IS NOT IN THE REPO~~ **(SUPERSEDED — it IS in the repo: `b816fea0`. My July anchor was picked by date and landed 11 h after the change, so 'endpoint fails to reproduce' meant WRONG ANCHOR, not out-of-repo):** the exact July commit `edb546f0` re-run TODAY gives **NO-CYCLE (median |e_rot|=0.13)** vs its own archived July data at **+0.58..+1.10, 27/27 cycle**. Same commit, different behaviour ⇒ out-of-repo cause; a commit bisect would falsely converge on the earliest commit. Leading hypothesis: the **camera SDF** (outside the repo, 640×480→320×240 on 2026-08-27) — every cycling run records `resolution (480,640)`, every non-cycling run `(240,320)`, and the cycle was lag-pumped (`χ > Wτ`), with that change made specifically to raise `process_frame()` rate ~15-23→~38 Hz. Decisive test (unrun, needs a temporary shared-install edit): restore the SDF to 640×480 and re-run HEAD. ⚠ **A git worktree does NOT reconstruct a historical experiment — validate bisect endpoints first.** ⚠⚠ **SECOND observational-log-diff mechanism claim refuted by a controlled test this session** (the κ-ratchet story fell to the GT-FB A/B). Treat log-diff mechanism inferences as hypotheses, never findings. ⛔ **My offline pre-check predicting the gate would kill the curve (qg≈0.01) was WRONG by 2×** — it divided July 640×480 extents by today's 320×240 `frame_min`. **Renormalise every pixel-domain quantity across the 2026-08-27 resolution change**; GT-derived metrics (e_mean/e_rot/osc_std/lat) are resolution-independent and safe. Caveats: n=4, GT-FB only, real rover perception still broken. [[project_rover_turning_open]] [[feedback_session_20260909_12_audit]]
> **⭐⭐ UPDATE 2026-09-17 (later): the r=0.8 re-run is IN and it settles the QGATE question.** Combined n=6-8/arm, stimulus verified (Kasa fit r_fit 0.86-0.91m throughout). C_gated: **7/7 landed, ZERO failures, lowest terminal peak command of all arms** (Ia_xy peak mean 1.95, even below baseline 2.42). B_ungated: **2/8 real failures** incl. a 20.7m fly-away, terminal peak up to 7.59 (July detonation range). Landing-rate delta not independently significant yet (Fisher p=0.47) but the terminal-peak-command separation IS (MWU p=0.003 vs gated, p=0.04 vs baseline). **Verdict: AU_LEAD_QGATE does its intended job at the real cycle scale** — removes the ungated lead's fly-away risk while keeping its tracking benefit. Supports AU_LEAD+QGATE for turning targets (GT-FB only; real rover perception still separately broken). Also fixed a harness bug (stale-directory false-success on a crashed rep) — [[feedback_recurring_analysis_mistakes]] §17.
> **⛔⛔ UPDATE 2026-09-17 (later still): the r=0.8 QGATE result above was measured on the WRONG WORLD.** Per §18, the harnesses never set `WORLD`/`ROVER_MODEL`/`MARKER_TYPE`, so it ran on ArUco, not cross-marker. Re-run on `WORLD=rover_cross ROVER_MODEL=rover_cross MARKER_TYPE=cross`, n=5/arm: **ALL THREE arms land only ~1/5** (incl. the no-lead baseline, terminal peak Ia_xy ~6.2-6.7 vs ArUco's 2.0-4.0) — so this is NOT an AU_LEAD/QGATE problem, it is a shared control/perception issue on the moving cross-marker rover. Marker-mount-height mismatch RULED OUT (rover_cross is geometrically identical to rover_aruco). Root cause not yet found — likely MARKER_EXTENT_PX-driven logic (touchdown detect v2 / terminal commit / CBF gating), which stays LIVE under GT-FB and sees cross-marker's noisier detection. **Blocks any AU_LEAD conclusion on cross-marker until fixed.** > **⛔⛔ UPDATE 2026-09-18: the "GT pose timestamp jitter / chase-cam" root cause is FULLY RETRACTED.** Physically invalid under Gazebo LOCKSTEP sim time (render speed cannot create sim-time gaps), and the `dt<=0` measurement was on `apps/landing_test.py`'s own diagnostic recording loop (polling a cached `/clock` value at wall-clock cadence — a `dt=0` row is just the recorder polling faster than the clock ticked), NOT the loop inside `controller.py`'s `Controller` thread that actually feeds `gt_feedback.py`. Chase-cam resolution is not implicated — already validated safe at 1920x1440 ([[project_20260826_chasecam_resolution_bump]], two prior bumps, zero down-cam degradation). **Root cause of the cross-marker rover's genuine descent-tracking divergence is OPEN again.** ⛔ The yaw/alpha-sign-convention candidate is ALSO RETRACTED (2026-09-18, user correction) — that bug class was found, flip-flopped, and finally resolved 2026-09-09 ([[feedback_gtfb_wz_sign_bug]], superseded block; [[project_yaw_rate_law_sign_bug_and_validation]]), validated against calibrated perception `w_iz`, and is baked into the current `gt_feedback.py` already read in full — not marker-specific, not open. **Remaining candidate:** the live `gt_feedback.py` call site inside `controller.py`'s `Controller` thread (never inspected). Read `gt_feedback.py` in full: NO `MARKER_TYPE` branch anywhere, pure function of `(uav_pose, target_pose, t)`; both `landing_platform` joints are `type=fixed` in both SDFs. So checked raw `Ground_Truth.npy` timestamp spacing directly: ArUco `dt<=0` rate ~2-3%, **cross-marker ~15-21%** (7-10x). `gt_feedback.py`'s `_slope` velocity/yaw-rate estimator (feeds `h`, `w_z`) is a least-squares fit over exactly these timestamps — a burst of duplicates corrupts it (near-zero `denom`), and `w_z` leaks into the LATERAL command via the documented `cross(w_i,s)` transport term, explaining the baseline-only (no-AU_LEAD) divergence. Reconnects the chase-camera hypothesis closed too fast at the control-loop level (100Hz clean both worlds) — the GAZEBO POSE-PUBLISH cycle is a SEPARATE clock from the control loop and can stutter under the cross world's 1920x1440 chase-cam render load without touching the control loop's own timer. Not yet proven (needs a SITL rep with the chase-cam reverted to 640x480, cheap, not yet run) but best-supported hypothesis with a mechanism and a numeric signature. [[project_20260916_curve_qgate_revalidation]]
...
- [Visibility predictor residual measured — horizon is ~144 ms, b=0.15 thin in the tail, τ·d is median-only](project_20260917_visibility_predictor_residual.md) — 1-step prediction is WORSE than no-motion; p99/p99.9 residual exceeds the buffer (1.2% of frames); τ·d = 10% median, 0 tail. τ is a PLANT property, not a scenario one.
