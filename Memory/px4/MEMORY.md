...
> ⭐⭐⭐ [Line-width loom investigation (width itself: LANDED+validated) + origin_ratio ⛔⛔ PROVEN UNSUITABLE FOR CROSS-MARKER, reject on principle (2026-09-08)](project_20260908_line_width_loom_investigation.md) — width-loom: found+fixed 2 real bugs in the existing width machinery, replaced with a direct mask-thickness scan, **validated 0.89-1.00 corr with GT altitude across all 5 ICs (7 reps), landed as shadow-mode `"Width Loom Px"`** (not consumed by control). Width-loom RATE (KF derivative): initial 64-combo sweep gave no usable signal — but root-caused (not re-swept blind) to 2 real bugs in the width measurement itself (`_WLOOM_MAX_STEPS=60` pinned width flat for the whole terminal descent from ~frame 311 onward on a real rep; the 2-arm "median" degenerates to a non-robust average). Fixed both (cap raised to 300, added a 2x arm-agreement check) and re-swept: **all 4 tested reps now POSITIVE for the first time** (best: q=10/r=0.005, 0.15/0.18/0.49/0.69, mean 0.38, vs the old defaults' -0.30 mean). Real progress, not yet control-ready (well short of static width's own 0.89-1.00, only 4 reps tested, weaker reps not yet individually root-caused). `origin_ratio` double-gate: caused a confirmed crash regression on every off-center IC (2 sessions, 12/12 correlation); reverted (`CROSS_TZ_VETO_R_MULT` 1e6->1.0). Relative-drop/streak fix tested against 15 real off-center reps: decisively failed (24-45% false-veto rate). **Root-caused to a PROVABLE architectural mismatch, not a tuning gap: cross-marker's flow points have no fixed physical identity across frames (unlike ArUco's same-4-corners-every-frame decode) — `RESAMPLE_PERIOD_S` periodically re-diversifies the tracked set by design, and point count churns every single frame regardless (measured: 70,70,54,38,29... frame to frame). Quantified: `|delta point-count|` predicts `origin_ratio` volatility monotonically in BOTH centered (median |dln ratio| 0.45-0.57) and off-center (0.10-0.37) real flight data — the noise is architecture-level and, if anything, WORSE when centered. Centered flights only "worked" under the old veto because their baseline ratio (58-71) sits far above threshold; off-center's baseline (0.4-0.9) sits AT it, so identical noise crosses constantly.** No threshold variant can ever separate real collapse from routine churn in this metric's value space — **reject origin_ratio-based Tz gating for cross-marker permanently, on principle, and treat this as a caution against ANY future point-position-statistics conditioning metric for cross-marker's flow source.** `CROSS_TZ_VETO_R_MULT=1.0` stays default. Width-loom (measures a physical mask property, no point-identity dependency) is structurally immune to this failure mode and remains the path forward, once its rate-signal gap is solved.
