...
- [⭐⭐ Lissajous: CBF correlates with failure severity but is NOT the divergence trigger](project_20260922_lissajous_cbf_and_divergence_mechanism.md) — GT-FB does not isolate the CBF (marker_center_px is live-perception, confirmed); vis_active tracks failure severity across reps (63%/51%/27% bad vs 3.4% good at k=0.4) but a within-rep trace shows the actual t~2.7s divergence trigger is h_d_z pinned regardless of lateral error + I_a_z cannibalized by I_a_xy — the SAME mechanism as the main rover investigation. Speed retunes (k=1.0→0.4→0.2→0.1) delay but don't prevent it (0/5 SP at k=0.2 combined). Trajectory-level retuning is the wrong lever; fix belongs in controller.py (descent gating / protect I_a_z), untried.
