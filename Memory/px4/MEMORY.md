...
> ⭐⭐⭐ [Line-width loom investigation (width itself: LANDED+validated) + origin_ratio veto (⛔ REGRESSED, now DEFAULT-OFF) (2026-09-08)](project_20260908_line_width_loom_investigation.md) — width-loom: found+fixed 2 real bugs in the existing width machinery, replaced with a direct mask-thickness scan, **validated 0.89-1.00 corr with GT altitude across all 5 ICs (7 reps), landed as shadow-mode `"Width Loom Px"`** (not consumed by control). Width-loom RATE (KF derivative for control use): built, swept 64 q/r combos, **no usable signal found — negative result, don't re-attempt the same untuned hypothesis**. `origin_ratio` double-gate: the restructuring **CAUSED A CONFIRMED CRASH REGRESSION on every off-center IC** (2 independent sessions, 12/12 correlation) — `origin_ratio` reads persistently low for any off-center approach regardless of tracking health (it's the marker's real frame offset, not a health signal), so the hard veto froze h_z for the whole descent -> unbraked crash. Attempted relative-drop+streak fix reduces but does NOT eliminate the risk (real origin_ratio noise on centered data creates a genuine noise-vs-sensitivity tension unresolved by centered-only testing). **`CROSS_TZ_VETO_R_MULT` DEFAULT CHANGED 1e6->1.0 (no-op)** to unblock — restores exact pre-fix Tz data flow. ⛔ Do not raise it again without a fresh gate covering BOTH a real off-center IC and a reference-like collapse.
