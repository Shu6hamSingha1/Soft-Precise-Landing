...
> ⭐⭐⭐ [Line-width loom investigation + origin_ratio fix — ALL LANDED, verified on real data (2026-09-08)](project_20260908_line_width_loom_investigation.md) — deep dive on terminal-overfill `h_z` corruption. Found+fixed 2 real bugs in the existing width machinery (self-referential `_robust_fit_line` pruning collapse; fixed-fraction `_band` admitting a second real structure at close range) and ruled out a 3rd (downscale quantization, negligible). Replaced the whole point-cloud pipeline with a direct mask-thickness scan — **validated 0.89-1.00 corr with GT altitude across all 5 ICs (7 reps), landed in `cross_marker_perception.py` as a shadow-mode logged signal (`"Width Loom Px"`, `width_loom_from_detection()`), live-wiring verified (0.886 corr matching offline)**. Tilt-leveling checked and resolved as a false alarm (narrow-altitude-range/touchdown-noise artifact, not a real foreshortening bug — leveling correctly had zero effect). `line_points_i_raw`/`line_points_j_raw` fields also landed in `cross_marker_detector.py` (additive). Separately: **`origin_ratio`'s gate-fail-fallback bug FIXED** — was nested behind moment-loom's own point-count floor (missed the low-point-count regime the reference failure actually collapsed through); restructured to veto BOTH Tz estimators (KF r[2] inflation, not a value swap) as soon as the solve's own floor is met — verified firing on real data (14/361 and 100/391 solve calls vetoed across two reps), `h_z` stays bounded, no more NaN/blow-up risk. **Only remaining open item: a control-ready width→loom RATE signal** (KF-based derivative on `ln(width)`, reusing `_kf_step` — not yet designed/implemented, width itself is validated but its time-derivative for control has not been).

> ⛔ [CBF joint-QP: descent-rate-relief self-inflation fix (`CBF_JQP_RELIEF_REF_AZ0`) FAILED its IC2-5 SITL gate](project_20260908_jqp_relief_refaz0_gate_failed.md) — 2026-09-08. `CBF_visibility.pdf` S4 open #2 (relief↔box fight): the relief measured box-suppression against the RUNNING `_az_now`, which the relief itself inflates each outer iterate → `Ia_z` ratchets to `-g` on any box bind (the terminal descent-stall chain). Fix = measure Δθ at the FIXED unconstrained `a_z`. Offline clean (`validate_cbf.py` +`test_joint_qp()` 17/17 — that path had ZERO coverage before; 926/926 synthetic cases retained more descent authority). **Cross-marker n=5/arm/IC A/B (`test_data/JQP_ReliefRefAz0_AB/20260908-160523`): REJECT — mean xy_err regressed at ALL 4 ICs (1.08→1.68 m total), target-loss 4→7 (worse/equal every IC), xy>2 m reps 2→7.** The self-inflating relief empirically buys the descent-slowing off-center ICs need; the "principled" version under-relieves. Same lesson as `Rz_p90b` / `CBF_MARGIN_RESERVE` / backstep-h_d. Left **default OFF** as an A/B hatch, stamped ⛔ in the code — do NOT re-try/bake. **STANDS regardless:** the per-outer-iterate residual instrumentation (`7b81ac1f`: `jqp_resid_final/max/rising/converged(t)` in `Control_Data.npy` + `tools/analyze_joint_qp_convergence.py` corpus scan — proxy result: P(blow-up\|chatter)=0.50 vs 0.05) and `test_joint_qp()`. `CBF_visibility.pdf` S4 #1 + the fixed-iterate-budget-no-early-exit question remain OPEN. **ROOT CAUSE (traced `f61c0608`): box↔relief closed-loop DEADLOCK** — the corrected relief is proportional+persistent, so when the FoV box binds every frame at a tight off-center touchdown it pins `I_a[2]` at `-g` → `B_T`→0 → descent freezes 4-8s at 0.4-1.5m; the freeze doesn't fix the off-center condition so the box keeps binding so relief keeps firing. The buggy self-inflation is unstable → spikes → breaks its own trigger → an accidental dither that never deadlocks. Terminal metrics regressed-vs-matched: mean relief 0.73 vs 0.09, `I_a[2]` pinned at `-g` up to 71% of frames vs ~0, `a_u` max to 1666.
