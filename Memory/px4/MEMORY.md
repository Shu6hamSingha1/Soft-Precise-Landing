...
- [⛔⛔ RE-STAMPED 2026-09-17 — "TURNING-rover OPEN" IS STALE: the curve LANDS now (~30/33 on-platform at 1-9 cm), cycle gone, cause OUT-OF-REPO and still unidentified](project_rover_turning_open.md) — Sep GT-FB Circular (~0.5 m/s, heading-hold) pooled over `Rover_Turning/{cycle_isolation,qgate_revalidation}` + `XirXi2_RoverGTFB/moving_base` + `RoverCross_GTFB_Circular`: **~30/33 on-platform, xy 1-9 cm, `min_rel_z` 0.490-0.504 = genuine pad landings** (July: typical arm 0/3 at 1-8 m; `yawhold_arm_n3` `min_rel_z`=0.089 = landed BESIDE the pad). Cycle disappearance is NOT explained by any repo commit — the exact July commit run TODAY shows no cycle ([[project_20260916_curve_qgate_revalidation]], `e7882829`; a bisect would falsely converge — don't). Peer's leading hypothesis is the out-of-repo camera SDF (640×480→320×240, 08-27) acting via reduced lag; **⚠ constrained by my 2026-09-17 measurement: these are GT-FB runs and the logged CONTROL-LOOP timing is unchanged (July 94.4 Hz / dt_p95 18.3-18.7 ms vs Sep 100 Hz / 18.0-18.5 ms) — the loop was never starved, so "slower loop ⇒ more lag" is not supported; cause stays OPEN.** The visibility rewrite killed the **TAIL not the cycle** (old stack bimodal 0.018-5.90 m, `e_mean` sd 0.691 vs new 0.006 — replicates the stationary [[project_20260909_visibility_projection_wire_in]] tail finding on a moving target). **Both July "exits" are now non-binding:** AU_LEAD (no-lead 3/4 ≈ ungated 4/4 ≈ gated 4/4) and `CBF_DRIFT_TAU` — ⛔ **that τ line CORRECTED same day**: τ=0's win was an xy/SP argument, the wrong metric for a CBF. Same-metric audit (`e9a3d081`, third session) **independently re-derived by me with its committed `tools/scan_vis_safeset.py`, all 8 figures exact**: the "278 sensor-exit frames" figure behind the τ=0.15 bake is a **τ=0-arm-only count** (off-vs-lead raw 2.24% vs 2.10% wash; conditioned 0.47% vs 1.36%, lead worse) ⇒ stated rationale UNEVIDENCED but the value is fine (n=2/cell, duration-confounded); better justification = τ is the PLANT's attitude-realization horizon ~144 ms (IQR 112-288), a plant NOT scenario property ⇒ "moving-target lead" is mis-framed, applies to stationary too. ⚠ **SCOPE-CORRECTED `647ffec1` (author's own downgrade, NOT re-derived by me — no tool committed with it, unlike the scanner above): τ·d's benefit is SMALL** — 54.1% of frames, mean 9.9% (p50 0.0194→0.0169), and **no tail improvement** (over-buffer 1.235%→1.319%); gT²/6Z is exact but small vs total residual, noise/unmodelled dynamics dominate. ⭐ Higher-value target than τ: buffer b=0.15 covers p95 (0.084 vs [0.133,0.178]) but **p99 0.194 / p99.9 0.307 EXCEED it**, over-buffer 1.2% of frames — a residual-sized buffer now has a number behind it ([[project_20260917_visibility_predictor_residual]]). STILL OPEN: the **speed wall** (July ≤1.09 m/s reliable, binds 1.56 — [[project_rover_speed_sweep]]) untested since; GT-FB only; n=3-4/arm. Original July entry follows ↓
