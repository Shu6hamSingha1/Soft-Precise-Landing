...
- [⭐⭐⭐ dtheta az-visibility-filter is SELF-DEFEATING via an attitude-history feedback loop through cbf2_filter's th_curr -- not a gain-tuning problem, A_CAP saturation ruled out with hard data (2026-08-24)](project_20260824_dtheta_az_filter_self_defeating_feedback.md) — PLASMC_DTHETA_AZ_GAIN sweep {5,10,20,40} n=3 IC5 cross-marker: correction magnitude scales correctly with gain (4x range, I_a[2] shift -0.6->-2.75) and dtheta fires 65-98% of frames, but xy_err/rel_vel show NO improving trend. Root cause: BOTH dtheta and theta_cone (CBF's own solved lean magnitude) shrink monotonically with gain (12.08->6.69deg) -- extra lift changes realized attitude history, which feeds back into cbf2_filter's th_curr (current-tilt) reference next cycle, shrinking the QP's own solved theta_safe. The mechanism undermines the exact lateral authority it exists to protect. Fix needs to break the loop (anchor th_curr against DESIRED not realized attitude, or decouple), not adjust gain. IC2's earlier positive A/B trade-off (better xy_err, worse rel_vel vs the old loom-margin approach) likely reflects IC2's much rarer dtheta firing (~20% vs IC5's ~65-98%) limiting how much this loop bites there.
