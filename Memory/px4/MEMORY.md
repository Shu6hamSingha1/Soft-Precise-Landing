...
> ⭐⭐⭐ [ROBUSTNESS EVAL SET BUILT — lighting is FINE, COLOUR is the weakness](feedback_cross_detector_robustness_requirement.md) — 2026-09-03: `test_data/RobustnessFrameset` (7 variants, 2743 frames, GT-FB flight so the detector doesn't gate it, `CROSS_RING_OVERLAY_DBG=0` verified clean). Baseline @ `a53a5f63`: **dim/bright/lowsun/darkbg all 96-100% detOK with accuracy ≥ base — an 8× sun range does NOT break it, so stop looking there.** ⛔ **`inv` (polarity flip) is the DANGEROUS case: 82.3% detOK but 53.9 px error, only 23% within-0.15 — CONFIDENTLY WRONG**, because `inRange(V≤100)` keeps 94.8% of pixels (the PLATE) and Hough fits plate edges. ⛔ `col` (chromatic iso-V, gate keeps 0.0%) degrades with RANGE: 95% @4-6 m → **5% @0.7-1.3 m**. PASS CRITERIA for front-end work: `inv` within-0.15 up from 23% (hard), `col` low band up from 5% (graded), no regression on the other five, scored on within-0.15 + flight outcome NOT detOK. ⚠ n=1/variant, synthetic textures.
