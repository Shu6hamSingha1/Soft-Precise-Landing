...
> ⭐⭐ [SITL GATE: the span rescue's offline detOK gain did NOT translate to flight](project_20260902_spanrescue_sitl_gate.md) — 2026-09-02: clutter detOK 5-7→22-73% while outcomes got **WORSE** (median xy ~1.9→~5.6 m, two 12 m fly-aways). rover_cross: acquisition decisively fixed (detOK 48→98%, TARGET_LOST 3/5→0/5) and blind dives become stalls ABOVE the pad (Mode A→B), but **0/5 land** either arm and a 49.8 m fly-away persists. `CROSS_CENTROID_SPAN_FILL_MAX=0.6` removes every overfill rescue as designed but ⛔ did NOT fix the fly-aways — the overfill hypothesis explained one outlier's TIMING, not the mechanism. **KEEP DEFAULT OFF.** **DECISION: flip to default ON is AGREED IN PRINCIPLE but HELD (user) until SITL is back and the flat+clutter confirmation lands, so the default ships WITH its evidence.** ⛔⛔ **SITL IS DOWN since 2026-09-02 ~21:41** — every launch in BOTH worlds fails `Timed out waiting for Gazebo world`; a direct manual launch fails too (so NOT the harness); needs machine-level gz/Qt repair. ⛔ RETRACTED: the "stale gz server on world switch" theory (flat-after-rover 7/9 vs flat-after-flat 3/10) — a hardened gz cleanup did NOT fix it and the rover probe then failed identically, so the failure is GLOBAL and that pattern was just ORDERING (time confounded with condition; interleave conditions).
