...
- [⭐ Visibility-CBF + touchdown-detect session (2026-09-17→22): axis-fix BAKED, flow-freeze BAKED, soft-touchdown OPEN](project_20260917_visibility_predictor_residual.md) — SESSION CLOSE at file top. (2) axis-transposed CBF barrier found+fixed+SITL-validated (96271ba6, one image axis was inert). (3) touchdown-detect flow-freeze false-positive found+fixed+SITL-validated (2177670b, 22%→0%). (4) soft-touchdown root cause only PARTIALLY identified after 5 mechanisms tested (ill-conditioning falsified, grazing-rays/KF-rate-buildup real but insufficient, gyro resolved but doesn't close a ~5.6x gap) — N_z tuning correctly abandoned, MATLAB peer retune not yet ported.
