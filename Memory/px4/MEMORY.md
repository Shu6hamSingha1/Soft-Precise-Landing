...
> ⭐⭐⭐ **PICK UP HERE (new session, 2026-09-05)**: [PLASMC_YAW_RATE_LAW — sign bug found+fixed, GT-feedback VALIDATED (incl. beyond the old ceiling), real-perception NOT yet safe](project_yaw_rate_law_sign_bug_and_validation.md) — the new yaw-rate law (direct integrator on w_z, bypasses psi_d/e_R for yaw) had an actuation-chain sign bug (commanding +2.0 rad/s achieved -2.03 rad/s), found by MEASURING the actual GT yaw rate, not by guessing twice. Fixed form `Δw_u2=k_p·e_a−w_z`. GT-feedback: e_a -137°→-0.5°to-0.9° at the old 0.48 rad/s ceiling, AND -0.6°to-1.0° at 0.60 rad/s (unreachable by any prior mechanism). Real perception: REGRESSES — w_z inherits the documented terminal-overfill corruption. **Gate + w_z SIGN FIX done (2026-09-08). Root cause of the pre-overfill spin-up: perception w_z (`self._w_i[-1][2]`) = manuscript rotational flow ≈ +alpha_dot, the OPPOSITE sign to the GT-FB w_z (`-alpha_dot`) the law was validated against → `-w_z` term became positive feedback → drone yawed 1-2 full turns. Verified vs GT body yaw rate. Fix: `PLASMC_YAW_RL_WZ_SIGN` (default −1 on perception, +1 under GT-FB) + `PLASMC_YAW_RL_WZ_SCALE`. IC1-5 n=3: 15/15 land, 0 TL, 8/15 precise, all soft — on par with baseline ASMC. Residual e_a ~10-22° (3× w_z magnitude deficit). NEXT: sweep WZ_SCALE (~2-3)/k_i; then n=5 + turning-target gate.**
