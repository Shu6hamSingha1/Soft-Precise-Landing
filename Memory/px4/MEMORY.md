...
- [⭐ B4 AU_LEAD: perception-quality-gated lead FIXES IC1-4 (13/20 precise, ≥ baseline); IC5 still open (2026-09-09→12)](feedback_aulead_stationary_regresses.md) — concurrent A/B `WORLD=cross_marker`, IC1-5 n=5. **PERCEPTION path:** baseline 20/25 precise (xy 0.02-0.09 m); `PLASMC_AU_LEAD=1 RATIO=0.5` (ω_z/ω_p 0.9/3.5) **2/25 precise (xy 0.11-0.21 m, 2-6× worse)**, all land, no TL/fly. Perception log dive first blamed a control-side κ-ratchet HF pump (`I_a_raw` pk 4.7→14.6, κ pk 0.21→0.41, κ-growth +0.08→+0.37, RATIO clamp pinned at 0.5·\|I_a_raw\|). **GT-FEEDBACK A/B (same config, bundles `20260909-221339`/`-224018`) REFUTES that:** IC1-4 xy 0.013→0.015 (no regression), κ DECAYS in both arms, `I_a_raw` pk +0.01, CBF idle — and AU_LEAD **RESCUES** the fragile GT-FB IC5 (0/5→5/5 precise). ⇒ real mechanism: the ×3.9 HF lead **amplifies terminal extent-saturation centroid/flow corruption** (318 px both arms); κ-ratchet etc. are driven symptoms. A κ-ODE dead-zone (`PLASMC_KAPPA_DZ_*`, [[feedback_adaptive_law_noise_behavior]]) confirmed this is upstream of κ — gating κ-growth alone does NOT rescue AU_LEAD. **⭐ FIX 2026-09-12: `PLASMC_AU_LEAD_QGATE`** (default ON w/ AU_LEAD) attenuates the lead delta by `MARKER_EXTENT_PX/frame_min` fill-fraction (scale-free ramp 0.55→0.85→0, no depth/altitude). 3-arm A/B (`ICValidation/{20260912-023429,-030211,-032559}`): **IC1-4 FULLY RECOVERED — 13/20 precise (≥ baseline 12/20), vs 3/20 ungated.** IC5 (3 m start, steepest/shortest ~9 s descent) still 0/5 despite the gate engaging identically (terminal `au_lead_qgate`→0 confirmed) — IC5's full-lead-window command runs ~2.3× hotter than IC2's (`I_a_xy` 1.13 vs 0.49 mean), an open threshold-tuning item, not yet fixed. VERDICT: perception-quality-gated lead is the viable path for 4/5 ICs; still needs an IC5 fix, the curved-target benefit re-validated WITH the gate on, and a real-rover pass before any bake. ⚠ `run_ic_validation.sh` doesn't set WORLD/MARKER_TYPE → defaults to stale-cal ArUco world; always export both. [[feedback_reject_on_single_failure]] [[project_rover_turning_open]] [[feedback_adaptive_law_noise_behavior]]
