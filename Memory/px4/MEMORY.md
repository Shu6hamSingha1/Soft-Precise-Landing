...
>
> **Width-loom RATE (KF derivative): the "0.38 mean, all-4-positive breakthrough" reported mid-session did NOT survive re-validation and is RETRACTED.** ⛔⛔ **`tools/gt_optical_flow.py` — the canonical GT-scoring reference used for EVERY correlation this whole thread — had 3 real bugs, found 2026-09-08 (user flagged the first from project-history instinct; comparing side-by-side against the maintained sibling `gt_feedback.py` surfaced the other two):** (1) stale `Z_REG=0.01` regularizer (live value elsewhere is 0.2); (2) **missing camera/marker mount-offset correction** — used raw `target_origin - uav_base_link` instead of `marker - camera`, an **11x relative depth error at touchdown** (rep1: raw zB=0.014m vs corrected 0.162m) — the dominant bug, concentrated exactly in the terminal window under dispute; (3) unclamped signed depth + a stale `abs(zB)>=0.1` altitude gate that `nan`-blanked the reference below 0.1m. All 3 fixed to match `gt_feedback.py`'s convention exactly. **Re-ran the REAL production detector (`cross_marker_detector.detect()`, not an approximation) on the raw `IMG_RECORD` frames for all 7 reps, against the corrected reference: terminal-window (alt<0.5m) mean correlation dropped to 0.06 (range -0.10 to +0.29) — WORSE than the retracted 0.38.** The earlier "breakthrough" was very likely fitting the corrupted reference's own artificial ramp-then-nan shape, not real loom. Whole-descent correlation (0.10-0.31, positive in all 7 reps) says the signal isn't pure noise, just not solving the terminal-window problem. **Honest status: width-loom RATE is NOT control-ready and this thread's two rejected hypotheses (point-count churn, tilt-leveling) were never wrong per se — they were chasing a broken reference.** Static width (no derivative) is unaffected and remains solid. Width-loom (measures a physical mask property, no point-identity dependency) is structurally immune to the origin_ratio failure mode and remains the path forward for the VALUE signal — the RATE signal needs a new idea, not more tuning of the current KF/detection machinery. **➡ 2026-09-09 UPDATE — the "new idea" landed: EXTENT FUSION.** ⚠ `"Scale Loom Rate"` is ~70% `MARKER_EXTENT_PX`, NOT line-width — pure line-width (the separate `"Width Loom Rate"` channel) tested weak (~0.14 mid-band); extent alone carried ~0.73, so the 0.3 width term is just a small robustness add. Don't call `"Scale Loom Rate"` "loom rate from line-width" (that's `"Width Loom Rate"`, which didn't pan out). `scale_z = 0.3*ln(width) + 0.7*ln(MARKER_EXTENT_PX)` (both fresh) → gentle CV-KF (Q=1.5/R=0.03) → `-d/dt` gives corr with GT loom **>2m mean 0.90 / worst 0.77, 0.5-2m mean 0.80 / worst 0.68**; <0.5m still ~0 but **fails SAFE toward 0** (pinv h_z spikes there). Confirmed terminal loss is NOT lag (GT-shift sweep flat), NOT binarisation (grayscale scan a wash), NOT regressable (residual autocorr 0.5-0.9, unexplained by tilt/extent/coff/ncorners); CA-KF worse. Landed SHADOW-MODE as `getLogData()` `"Scale Loom Rate"` (env `CROSS_SCALE_RATE_A/_KF_Q/_KF_R`; backup `Obsolete/src/cross_marker_perception_pre_scalerate_20260909.py`). **CONFIRMED LIVE (2026-09-09, 2 fresh HEADLESS off-center SITL reps IC2/IC3, real perception, user-authorised):** `"Scale Loom Rate"` is in `getLogData()`; live↔offline-replay parity corr **0.95-0.97** (RMSE ~0.05) — the `process_frame` path is faithful; **live 0.5-2m band corr 0.84-0.91** (vs live width-only 0.16-0.33); <0.5m still sign-unstable per-rep but bounded |rate|≤1.15. Also de-risked offline on 20 real off-center descents (`20260908-182815` IC1-4, extent-only limit): 0.5-2m corr 0.85-0.98. **IC5 breaks the signal** (0.5-2m negative, |rate|→1.7) — separate unsolved blocker, don't force-fit. **➡ WIRE-IN LANDED DEFAULT-ON (2026-09-09, `c843a7a1`), user-directed, classified as a PERCEPTION-estimator change (controller.py untouched — still reads `getOptFlowAngVel()[2]`).** Mechanism: a 2nd sequential scalar KF correction on `_hw_kf_x[2]` at the end of `_kf_update_hw`, measurement `= scale_rate / _sensor_cal_hw[2,2]` (0.9513; scale_rate is on the calibrated scale, `_hw_kf_x` is raw), value-only. Guards: `_scale_fuse_on` · hw-KF init & NOT frozen · scale KF init & `_scale_rate_measured_this_frame` (skip on coast → collapsing terminal estimate can't drag h_z) · `|scale_rate|≤CROSS_SCALE_FUSE_CLAMP`(1.0, rejects IC5). Env: **`CROSS_SCALE_RATE_FUSE=0` = instant full revert** (verified bit-identical `_hw`), `CROSS_SCALE_FUSE_R`(0.05, tighter than `FLOW_KF_R=0.1` so it can pull a spike down over ~2-3 frames). New log `"Scale Fuse Z"`. **⛔⛔ GATED AND REVERTED TO DEFAULT-OFF same day (`3137d4cd`).** IC1-5 n=3 gate on the fusion (`test_data/Multi_IC/20260909-022319`, on `cd6dc57f` which also added `CROSS_SCALE_FUSE_MAX_EXT=310` after the sanity A/B): **3 PRECISE-only / 1 SOFT+PRECISE / 8 FAIL / 1 NOT_LANDED**, rel_vel to 1.44 m/s, one false-touchdown 2.55 m up. Clean concurrent **fuse-OFF** gate on the same `visibility_projection` control code minutes earlier (`test_data/ICValidation/20260909-021038`): IC2/3/4 all SOFT+PRECISE, xy ~0.017 m, rel_vel ~0.02. **Touchdown DETECTOR not implicated** — `_touchdownDetectV2` (default for cross-marker) never reads h_z (n_corners/extent/flow-freeze; every rep latched `[overfill]`/`[flow-freeze]`). The regression is in the **descent-rate control** h_z feeds: `scale_rate` is well-CORRELATED (0.84-0.98) with GT loom but not UNBIASED, and fused at `r=0.05` it biased loom-setpoint tracking → fast/erratic arrival. Machinery + shadow logs + env flags all KEPT. Methodology miss: the sanity A/B's 3-4× xy degradation (fuse-ON 0.06-0.08 vs fuse-OFF 0.019) was already a reject per [[feedback_reject_on_single_failure]]. **➡ DE-BIAS RETRY ALSO FAILED (`246e8260` → marked dead-end `ff242d97`).** Characterised the bias (`GT_loom=a·sr+b` per band): NOT a constant gain — `.5-2m` `a≈0.41,b≈−0.22`, `>2m` `a≈0.10` (sr ~10× hot in the engage/warmup region). Retry = band-limit `150≤ext≤310` + affine de-bias `0.41·sr−0.22` + `r` 0.05→0.3. IC1-5 gate: `rel_vel` recovered (1.44→0.24-0.38) but **xy blew out on the CENTERED IC1** (0.146, 0.329 → 2/2 FAIL, killed on reject rule). Mechanism: `h_z` couples into the middle-loop SMC c-term `−(h·e3)h` + sliding surface, so any perturbation moves the lateral solution; pinv `h_z` is already good enough that there's nothing to gain. **VERDICT: feeding `scale_rate` into `h_z` via a KF measurement is a CONFIRMED DEAD-END (3 forms tried). `CROSS_SCALE_RATE_FUSE` default-OFF is PERMANENT.** The shadow `"Scale Loom Rate"` (0.84-0.98 in-band corr with GT loom) stays as a diagnostic only; any revisit needs a fundamentally different consumer, not a continuous h_z correction.
