...
- [⚠ B4 AU_LEAD stationary gate: FAILS under perception (2/25) but GT-FB shows NO regression → perception-noise amplification, not a control defect (2026-09-09)](feedback_aulead_stationary_regresses.md) — concurrent A/B `WORLD=cross_marker`, IC1-5 n=5. **PERCEPTION path:** baseline 20/25 precise (xy 0.02-0.09 m); `PLASMC_AU_LEAD=1 RATIO=0.5` (ω_z/ω_p 0.9/3.5) **2/25 precise (xy 0.11-0.21 m, 2-6× worse)**, all land, no TL/fly. Perception log dive first blamed a control-side κ-ratchet HF pump (`I_a_raw` pk 4.7→14.6, κ pk 0.21→0.41, κ-growth +0.08→+0.37, RATIO clamp pinned at 0.5·\|I_a_raw\|). **GT-FEEDBACK A/B (same config, bundles `20260909-221339`/`-224018`) REFUTES that:** IC1-4 xy 0.013→0.015 (no regression), κ DECAYS in both arms, `I_a_raw` pk +0.01, CBF idle — and AU_LEAD **RESCUES** the fragile GT-FB IC5 (0/5→5/5 precise). ⇒ real mechanism: the ×3.9 HF lead **amplifies terminal extent-saturation centroid/flow corruption** (318 px both arms); κ-ratchet etc. are driven symptoms. VERDICT: stays default-OFF for the live path; deployability gate is **"feature measurement clean"** (extent/corner-count/flow-resid — cf. `_solve_jacobian` `rel_resid`/`cond`), NOT "no 1/Z" — a perception-quality-gated lead is the viable turning-target path, needs building+testing. Perception bundles `test_data/ICValidation/{20260909-191246, -193638}`. ⚠ `run_ic_validation.sh` doesn't set WORLD/MARKER_TYPE → defaults to stale-cal ArUco world; always export both. [[feedback_reject_on_single_failure]] [[project_rover_turning_open]] [[feedback_adaptive_law_noise_behavior]]
