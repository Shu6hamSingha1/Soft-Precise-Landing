...
- [⛔⛔ 2026-09-18: ALL ArUco-rover curve test data from 09-16/17 DELETED at user instruction (ArUco is obsolete for the moving-rover scenario); what survives is code-level + genuinely cross-marker](project_20260916_curve_qgate_revalidation.md) — `ROVER_TRAJ=Circular`'s DEFAULT (`ROVER_CIRCLE_R=10`, commit `b816fea0` 2026-07-03) is a gentle 9-14 m/11-28° arc, not a curve — code fact, marker-independent, still stands; set `ROVER_CIRCLE_R=0.8` for the real turning-target case. **Cross-marker rover at r=0.8 (`WORLD=rover_cross MARKER_TYPE=cross`, GT-FB, n=5/arm, data KEPT) genuinely fails ~1/5 regardless of AU_LEAD** (baseline 1/5, ungated-lead 1/5, gated-lead 1/5; touchdown `|s_e_n|` up to 5.85, 2/5 reps hit literal hard impacts >50 m/s²) — not an AU_LEAD/QGATE problem, a shared control/perception issue. Marker-mount-height mismatch ruled out (`rover_cross` is geometrically identical to `rover_aruco`). **Two root-cause candidates tried and RETRACTED:** GT `/pose` timestamp jitter / chase-cam render contention (physically invalid under Gazebo LOCKSTEP sim time; measurement was on `landing_test.py`'s own diagnostic loop, not the one feeding `gt_feedback.py`; chase-cam already validated safe at 1920×1440, [[project_20260826_chasecam_resolution_bump]]); cross-marker-specific yaw/alpha sign convention (already resolved 2026-09-09, [[feedback_gtfb_wz_sign_bug]], baked into current `gt_feedback.py`, not open). **⭐⭐⭐ NEW LEAD (2026-09-18, cross-pollinated from a same-day Windows/MATLAB session):** MATLAB independently found `P.yrl_kp=0.3` (= `PLASMC_YAW_RL_KP`, PX4's default) destabilizes Circular tracking via an image-position/orientation coupling runaway at elevated target speed — fixed there by lowering to `0.02` (`MATLAB` commits `6a7d7dfb`/`ee7bbd46`). PX4's cross-marker rover run resolved `YAW_RATE_LAW=True, YAW_RL_KP=0.3` — the exact unstable value — because the yaw-rate-law is `MARKER_TYPE=cross`-gated (falls back to ASMC on ArUco) and the "heading-hold" recipe only zeroed the (inactive on cross-marker) ASMC gains, so the cross-marker curve tests were never actually heading-hold. This also explains the marker-type asymmetry the two retracted hypotheses were chasing. **Untested on PX4 — discriminator not yet run: `PLASMC_YAW_RATE_LAW=0` forced vs default, same r=0.8 recipe.** All stationary IC1-5 work is unaffected (no rover involved). Superseded prior entries in this slot (2026-09-16/17 QGATE-on-ArUco results, the cycle-isolation sweep, the `d380901c` worktree test, the camera test, the `edb546f0` bisect) — their underlying data was deleted; do not cite their numbers.
