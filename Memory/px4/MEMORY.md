...
> ⛔ [STAGE 3 + ensemble BOTH TRIED 2026-09-03 — the locked design is SPENT](feedback_cross_detector_robustness_requirement.md) — geometry-first confirm REGRESSED clean scenes (base detOK 100→80%, rejecting ACCURATE frames) because `|angle−90|` on clean `base` is BIMODAL with **41% at 35-55°, centred on `STUB_REL_ANGLE_DEG=45`** — the marker is a 3-armed cross WITH A STUB, so a stub+arm pair is a CORRECT detection. ⚠ NOT a frame bug: the virtual-plane version moves it ~0.1° (tested, do not re-try). ⛔ `inv`'s WRONG detections are MORE perpendicular (62-67% <15°) than base's right ones (58%) → no two-line self-consistency test can separate them. ⭐ Ensemble: `inv` 99.1/0 → 42.6/0 (silent lie → partly LOUD failure) and it DOES pick the right channel/polarity on 60% of inv frames, so segmentation is no longer the constraint. NEXT: visualise WHAT inv locks onto before writing a fifth mechanism.
