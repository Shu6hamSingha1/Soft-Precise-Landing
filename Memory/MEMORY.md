...
- [⚠ PRE-FLIGHT CHECKLIST — analysis mistakes Claude repeats here](px4/feedback_recurring_analysis_mistakes.md) — run the checks BEFORE concluding: log↔log time alignment; confirm pairings against RECORDED CONTENT — names have lied (§2) and mtime has lied (§13), neither is authoritative; rover `uav.z−target.z≈0.49` MEANS LANDED; a catch rate needs its false-positive rate and a log line its base rate; re-check the baseline (concurrent sessions). ~85/200 memories are corrections — recording a trap does NOT prevent repeating it. **+2026-09-16/17/18 §10-20:** ⭐ verify a mechanism's PRECONDITIONS before presenting it as a root cause — sim-time vs wall-clock, which loop produced a number, has this exact mechanism already been tested (§20); ⭐ verify the STIMULUS, not just the code — `ROVER_TRAJ=Circular` silently stopped driving a circle (`ROVER_CIRCLE_R` 0.8→10) and invalidated four controlled experiments; the ROVER launcher also defaults to ArUco not cross-marker (`WORLD=rover_cross ROVER_MODEL=rover_cross MARKER_TYPE=cross` needed explicitly, even under GT-FB — a whole session's curve tests ran on the wrong marker); log-diff mechanism claims are HYPOTHESES (2 of 3 refuted by the first controlled test); a worktree rebuilds CODE not the EXPERIMENT (validate bisect endpoints — out-of-repo camera SDF); identical baseline spans 17-20/25 so <3/25 is noise (quote a Fisher p); archived-run mtime ≠ run date (parse the dir name); renormalise px across 640×480→320×240; pin env vars — a peer re-baked a default BETWEEN two arms of my A/B.
